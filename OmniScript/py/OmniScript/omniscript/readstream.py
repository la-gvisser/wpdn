...
class ReadStream(object):
    ...
    def find(self, value):
        """Find a value in the entire stream of bytes. Return the offset where the
        value is found."""
    ...
    def read(self, length):
        """Read a number of bytes and adavance the current position."""
        value = struct.unpack_from("=%ds" % length, self.data, self.offset)[0]
    ...
    def read_short(self):
        """Read a short, 2-byte, signed value and advance the current position."""
        value = struct.unpack_from("=h", self.data, self.offset)[0]
        self.seek(2, os.SEEK_CUR)
        return value

    def read_ushort(self):
        """Read a short, 2-byte, unsigned value and advance the current position."""
        value = struct.unpack_from("=H", self.data, self.offset)[0]
        self.seek(2, os.SEEK_CUR)
        return value

    def read_int(self):
        """Read an integer, 4-byte, signed value and advance the current position."""
        value = struct.unpack_from("=i", self.data, self.offset)[0]
        self.seek(4, os.SEEK_CUR)
        return value

    def read_uint(self):
        """Read an integer, 4-byte, unsigned value and advance the current position."""
        value = struct.unpack_from("=I", self.data, self.offset)[0]
        self.seek(4, os.SEEK_CUR)
        return value

    def read_long(self):
        """Read a long, 8-byte, signed value and advance the current position."""
        value = struct.unpack_from("=q", self.data, self.offset)[0]
        self.seek(8, os.SEEK_CUR)
        return value

    def read_ulong(self):
        """Read a long, 8-byte, unsigned value and advance the current position."""
        value = struct.unpack_from("=Q", self.data, self.offset)[0]
        self.seek(8, os.SEEK_CUR)
        return value

    def read_double(self):
        """Read a long, 8-byte, floating-point value and advance the current position."""
    ...
    def read_string(self, count=0):
        """Read a short length and string value and advance the current position."""
        if count == 0:
            length = self.read_ushort()
            if length == 0:
                return ""
            if length > 512:
                raise OmniError('String is too long: %d' % length)
        else:
            length = count
        value = struct.unpack_from("=%ds" % length, self.data, self.offset)[0]
        self.seek(length, os.SEEK_CUR)
    ...
    def read_unicode(self, count=0):
        """Read an integer length and string value and advance the current position."""
        if count == 0:
            ...
            if (length % 2) != 0:
                raise ValueError
            if length > 512:
                raise OmniError('Unicode String is too long: %d' % length)
        else:
            length = count
        data = struct.unpack_from( "%dH" % ((length / 2) - 1), self.data, self.offset)
        self.seek(length, os.SEEK_CUR)
