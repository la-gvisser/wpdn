	...
	bool				bAddPI )
{
	...
	static const CLSID			clsid_DomDoc60 = { 0x88D96A05, 0xF192, 0x11D4, {0xA6, 0x5F, 0x00, 0x40, 0x96, 0x32, 0x51, 0xE5} };
	static const CLSID			clsid_DomDoc40 = { 0x88D969C0, 0xF192, 0x11D4, {0xA6, 0x5F, 0x00, 0x40, 0x96, 0x32, 0x51, 0xE5} };
	static const CLSID			clsid_DomDoc30 = { 0xF5078F32, 0xC551, 0x11D3, {0x89, 0xB9, 0x00, 0x00, 0xF8, 0x1F, 0xE2, 0x21} };
	hr = spXmlDoc.CoCreateInstance( clsid_DomDoc60 );
	if ( FAILED(hr) )
	{
		ATLTRACE( _T("Could not create MSXML2 6.0 DOM document, hr=0x%8.8X, trying 4.0...\n"), hr );
		hr = spXmlDoc.CoCreateInstance( clsid_DomDoc40 );
		if ( FAILED(hr) )
		{
			ATLTRACE( _T("Could not create MSXML2 4.0 DOM document, hr=0x%8.8X, trying 3.0...\n"), hr );
			hr = spXmlDoc.CoCreateInstance( clsid_DomDoc30 );
			if ( FAILED(hr) )
			{
				ATLTRACE( _T("Could not create MSXML2 3.0 DOM document, hr=0x%8.8X, giving up!\n"), hr );
				return hr;
			}
		}
	}
	...
	try
	{
		...
		if ( hr == S_OK )
		{
			...
			if ( !bFirstChildTextNode )
			{
				...
				ThrowIfFailed( spDoc->createNode( CComVariantEx( NODE_TEXT ), NULL, NULL, &spNewNode ) );
				CString	strValue = _T("\r\n");
				if ( strIndent.GetLength() > indentOrigLen )
				{
					strValue += strIndent.Left( strIndent.GetLength() - indentOrigLen );
				}
				ThrowIfFailed( spNewNode->put_nodeValue( CComVariantEx( strValue ) ) );
