...
public:
	...
	inline CMediaSpec& operator=( const CMediaSpec& inMediaSpec )
	{
		if ( &inMediaSpec != this )
		{
			// Looks dangerous, but isn't.
...
public:
	...
	static UInt32		ToProtoSpecInstID( const TMediaSpec& spec );

	static CMediaSpec	FromProtoSpecID( UInt16 inProtoSpecID );
	static UInt16		ToProtoSpecID( const TMediaSpec& spec );

	static CMediaSpec	FromIPv4Address( UInt32 inIPv4Address );
	static UInt32		ToIPv4Address( const TMediaSpec& spec );
	
	static CMediaSpec	FromTCPUDPPort( UInt16 inPortNumber );
	static UInt16		ToTCPUDPPort( const TMediaSpec& spec );
