	...
	const XCompare& rhs, int /*iFlag*/ ) const
{
	...
	pCompare->QueryInterface( TStringID, (void**)&pString );
...
#elif _LINUX
		nLength = vsnprintf( m_pData, m_nCapacity - 1, inFormat, args );
#endif
...
template< class Y >
X_RESULT TString< Y >::CalcHash( int hashSize ) const
{
...
TString< Y >::Mid( Y iFirst, Y nCount ) const

{
	...
	return( TString( m_pData + iFirst, nCount ) );
...
template< class Y >
bool TString< Y >::Replace( TCHAR cOld, TCHAR cNew )
{
