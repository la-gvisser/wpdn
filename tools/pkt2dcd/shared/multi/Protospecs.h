...
//	Protospecs.h
// =============================================================================
//	Copyright (c) 2001-2013 WildPackets, Inc. All rights reserved.
...
namespace ProtoSpecs
{
	...
	HRESULT PROTOSPECS_API GetPacketProtoSpecNative( TMediaType inMediaType, TMediaSubType inMediaSubType,
							UInt32 inPacketFlags, const UInt8* inPacket, UInt16 inPacketSize,
							PROTOSPEC_INSTID& outID, SProtospecInfo* payInfo, int& nInfoCount );

	HRESULT PROTOSPECS_API GetPacketProtoSpecCombined( TMediaType inMediaType, TMediaSubType inMediaSubType,
							UInt32 inPacketFlags, const UInt8* inPacket, UInt16 inPacketSize,
							PROTOSPEC_INSTID& outID, SProtospecInfo* payInfo, int& nInfoCount );

	HRESULT PROTOSPECS_API SetUseNativeProtoSpecs( bool bUseNativeProtospecs );
...
#define MAKE_PSINSTID(inInstance, inPSID)	((PROTOSPEC_INSTID) (( (inInstance) << 16 ) | (inPSID) ))
	...
	HRESULT PROTOSPECS_API FindProtospecInstanceW(LPCWSTR pszPath, PROTOSPEC_INSTID& outID );	// for expample, _T("IEEE 802.3//SNAP//IP//TCP//HTTP")
	HRESULT PROTOSPECS_API FindProtospecInstanceA(LPCSTR pszPath, PROTOSPEC_INSTID& outID );	// for expample, _T("IEEE 802.3//SNAP//IP//TCP//HTTP")
	HRESULT PROTOSPECS_API FindProtospecInstanceByID(PROTOSPEC_ID* payIDs, int nLength, PROTOSPEC_INSTID& outID );	// for expample, payIDs[] = { 1, 9, 1000, 1400, 1418 }
#ifdef UNICODE
	...
	HRESULT PROTOSPECS_API BuildPSParentPathW(PROTOSPEC_INSTID nInstanceID, LPWSTR pszBuff, int cchBuff);	// for expample, _T("IEEE 802.3//SNAP//IP//TCP//HTTP")
	HRESULT PROTOSPECS_API BuildPSParentPathA(PROTOSPEC_INSTID nInstanceID, LPSTR pszBuff, int cchBuff);	// for expample, _T("IEEE 802.3//SNAP//IP//TCP//HTTP")
#ifdef UNICODE
