...
//	ChannelMgr.cpp
// =============================================================================
//	Copyright (c) 2004-2012 Wildpackets, Inc. All rights reserved.
...
CChannelManager::CChannelManager(const CChannelManager& other) :
m_payChannelData(NULL),
m_type(kMaxType)
{
...
CChannelManager::SetChannels(const void* pData, SInt32 nCount)
{
	...
	m_payChannelData->InvalidateSort();
	m_payChannelData->Sort();
	return true;
}
...
CChannelManager::ScanfWirelessChannel(LPCTSTR psz)
{
	...
	_stscanf(psz, _T("%d - %d(%3s)"), &nChannel, &nFrequency, szBand);
...
CChannelManager::StringToBand(LPCTSTR psz)
{
	...
	if ((0 == _tcscmp(psz, _T("ac"))) || (0 == _tcscmp(psz, _T("802.11ac"))))
	{
		// there are actually four possibe values, but they all just say "ac"
		return PEEK_802_11_AC_BW80_PRIMARY_0_BAND;
	}
	...
	if (*psz == _T('u'))
	{
		...
		if (*psz != _T('0') && *(psz + 1) == 0 && _istdigit(*psz))
		{
			...
			long lval = wcstol(psz, nullptr, 10);
#else
			long lval = strtol(psz, nullptr, 10);
#endif
			return PEEK_802_11_UNKNOWN_5_BAND + lval - 1;
		}
	}
...
TArrayEx<UInt32>&	outRates)
{
	...
	case PEEK_802_11_N_40MHZ_HIGH_BAND:
	{
		...
		else
		{
			static const UInt32 ayRates[] = { 2, 4, 11, 12, 13, 18, 22, 24, 26, 27, 36, 39, 48, 52, 54,
				72, 78, 79, 81, 96, 104, 108, 117, 130, 154, 162, 163, 208, 210, 216, 234,
				243, 260, 270, 324, 432, 486, 540, 600, 720, 810, 900 };
			outRates.SetCount(COUNTOF(ayRates));
			for (UInt32 n = 0; n < COUNTOF(ayRates); n++)
				outRates[n] = ayRates[n];	// rates are stored in 0.5Mbps
		}
	...
	{	// all A, B, G, and N rates
		...
		// N : 1, 2, 5.5, 6, 6.5, 9, 11, 12, 13, 13.5, 18, 19.5,     24, 26, 27,     36, 39(78), 39(79), 40.5, 48, 52, 54, 58.5, 65, 78, 81, 81.5, 104, 105, 108, 117, 121.5, 130, 135, 162, 216, 243, 270, 300, 720, 810, 900(pulled from Atheros 802.11n driver)
		static const UInt32 ayRates[] = { 2, 4, 11, 12, 13, 18, 22, 24, 26, 27, 36, 39, 48, 52, 54,
			72, 78, 79, 81, 96, 104, 108, 117, 130, 154, 162, 163, 208, 210, 216, 234,
			243, 260, 270, 324, 432, 486, 540, 600, 720, 810, 900 };
		outRates.SetCount(COUNTOF(ayRates));
...
CChannelManager::GetDefault80211Bands(TArrayEx<UInt32>&	outRates)
{
	...
	outRates.AddItem(PEEK_802_11_AC_BW80_PRIMARY_0_BAND);
	outRates.AddItem(PEEK_802_11_AC_BW80_PRIMARY_1_BAND);
	outRates.AddItem(PEEK_802_11_AC_BW80_PRIMARY_2_BAND);
	outRates.AddItem(PEEK_802_11_AC_BW80_PRIMARY_3_BAND);
...
//---------------------------------------------------------------------
bool
ChannelUtil::IsValidChannel(
UInt32	nBand,
SInt16	nChannel)
{
	map<UInt32, set<SInt16> >::iterator	mapItr = m_bandDefaultChannelMap.find(nBand);
	if (mapItr == m_bandDefaultChannelMap.end())
	{
		mapItr = m_bandDefaultChannelMap.insert(m_bandDefaultChannelMap.begin(), pair<UInt32, set<SInt16> >(nBand, set<SInt16>()));

		TArrayEx<SWirelessChannel> channels;

		UInt32 nCount = CChannelManager::GetDefault80211Channels(nBand, kMediaDomain_Null, false, channels);
		for (UInt32 i = 0; i < nCount; i++)
			mapItr->second.insert(channels[i].Channel);
	}

	set<SInt16>::iterator	setItr = mapItr->second.find(nChannel);
	return (setItr != mapItr->second.end());
}
