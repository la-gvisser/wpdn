...
//	XMLDoc.cpp
// =============================================================================
//	Copyright (c) 1999-2011 WildPackets, Inc. All rights reserved.
...
#if TARGET_OS_WIN32
#ifndef OPT_XXMLDOC_USE_MSXML_PARSER
#define OPT_XXMLDOC_USE_MSXML_PARSER 1
#endif
#endif

#if defined(OPT_XXMLDOC_USE_MSXML_PARSER) && (OPT_XXMLDOC_USE_MSXML_PARSER == 1)
	...
	const TCHAR*	inChildName ) const
{
	...
	{
		const XXMLElement*	pChild = m_pChildren[x];
		if( _tcscmp(pChild->GetName(), inChildName) == 0 )
		{
			return pChild;
		}
	}
	...
	const TCHAR*	inChildName )
{
	...
	for( SInt32 x = 0; x < nChildren; x++ )
	{
		XXMLElement*	pChild = m_pChildren[x];
		if( _tcscmp(pChild->GetName(), inChildName) == 0 )
		{
			return pChild;
		}
...
{
	// Get nth attribute.
	if( inIndex >= 0 && inIndex < m_nAttributeCount )
	{
	...
	const TCHAR* inAttributeName ) const
{
	...
	for( SInt32 x = 0; x < m_nAttributeCount; x++ )
	{
		const XXMLAttribute*	pAttribute = m_pAttributes[x];
		if( _tcscmp(pAttribute->GetName(), inAttributeName) == 0 )
		{
			return pAttribute;
		}
	}
	...
	const TCHAR* inAttributeName )
{
	...
	for( SInt32 x = 0; x < m_nAttributeCount; x++ )
	{
		XXMLAttribute*	pAttribute = m_pAttributes[x];
		if( _tcscmp(pAttribute->GetName(), inAttributeName) == 0 )
		{
			return pAttribute;
		}
...
bool
XXMLElement::RemoveChild( XXMLElement* inChild, bool bForceDelete /* = false */)
{	// find the element
...
bool
XXMLElement::DetachChild( XXMLElement* inChild )
{	// find the element
	for (SInt32 n = 0; n < CountChildren(); n++)
		if (m_pChildren[n] == inChild)
		{	// realloc the array
...
bool
XXMLElement::RemoveAttribute( XXMLAttribute* inAttrib, bool bForceDelete /* = false */)
{	// find the element
