...
public:
	...
	static void		ParseList( const string& inData, UInt16 inId, CBlockAttributeList& outAttribs );

public:
	;		CBlock() : m_nType( TaggedFile::kCaptureFileUnknownBlock ),
				m_nFlags( 0 ), m_nLength( 0 ) {}
	...
	virtual bool	Read( CCaptureReader& inFile );
	virtual bool	ReadData( CCaptureReader& inFile );
...
public:
	...
	wstring			FormatAppVersion() const { return m_AppVersion.Format(); }
	string			FormatAppVersionA() const { return m_AppVersion.FormatA(); }
	wstring			FormatProductVersion() const { return m_ProductVersion.Format(); }
	string			FormatProductVersionA() const { return m_ProductVersion.FormatA(); }
	...
	bool			ReadData( CCaptureReader& inFile );
	...
	bool			Write( CCaptureWriter& inFile );
};
...
protected:
	void	Build( CBlockAttributeList& outAttribs );
	bool	BuildChannelList( string& outChannelList );
	bool	BuildRateList( string& outRateList );

	void	Parse( size_t inLength, const byte* inData );
	void	Parse( const CByteVector& inData ) {
		Parse( inData.size(), &inData[0] );
	}
	bool	ParseChannel( const string& inChannel, tChannel& outChannel );
	void	ParseChannelList( const string& inChannels );
	void	ParseRateList( const string& inRates );
...
public:
	...
	string		GetAdapterAddress() const { return m_AdapterAddr; }
	string		GetAdapterName() const { return m_AdapterName; }
	string		GetCaptureId() const { return m_CaptureId; }
	string		GetCaptureName() const { return m_CaptureName; }
	...
	string		GetEngineAddress() const { return m_EngineAddr; }
	...
	string		GetMediaDomain() const { return m_MediaDomain; }
	...
	void			SetAdapterAddress( const string& inAddress ) { m_AdapterAddr = inAddress; }
	void			SetAdapterName( const string& inName ) { m_AdapterName = inName; }
	void			SetCaptureId( const string& inId ) { m_CaptureId = inId; }
	void			SetCaptureName( const string& inName ) { m_CaptureName = inName; }
	void			SetChannelList( const std::vector<tChannel>& inChannels ) { m_ayChannels = inChannels; }
	void			SetComment( const string& inComment ) { m_Comment = inComment; }
	...
	void			SetEngineAddress( const string& inAddress ) { m_EngineAddr = inAddress; }
	void			SetEngineName(const string& inName ) { m_EngineName = inName; }
	void			SetFileIndex( UInt32 inIndex ) { m_FileIndex = inIndex; }
	void			SetHost( const string& inHost ) { m_Host = inHost; }
	void			SetLinkSpeed( UInt32 inSpeed ) { m_LinkSpeed = inSpeed; }
	void			SetMediaDomain( const string& inDomain ) { m_MediaDomain = inDomain; }
	void			SetMediaType( tMediaType inType );
	void			SetOwner( const string& inOwner ) { m_Owner = inOwner; }
	...
	void			SetTime( const string& inTime ) { m_Time = inTime; }
...
public:
	...
	void			SetId( const string& inId ) { m_strId = inId; }
...
public:
	;		CPeekCapture( tMediaType inMediaType = kMediaType_Ethernet )
...
public:
	;		CPeekCaptureReader()
		:	CCaptureReader( CF_FORMAT_PEEK, CF_ACCESS_UNKNOWN )
	{}
	virtual	~CPeekCaptureReader() {}

	virtual void	CopyMetaData( const CFileMetaData& inData ) {
		CPeekCapture::CopyMetaData( inData );
	}
...
{
public:
	;		CPeekCaptureWriter( CF_ACCESS_TYPE inAccess, tMediaType inType )
