...
public:
	static bool		Parse( size_t inLength, const byte* inData,
		const CAttributeIdList& inIds, CBlockAttributeList& outAttribs );
	static bool		Parse( const CByteVector& inData, const CAttributeIdList& inIds,
		CBlockAttributeList& outAttribs ) {
		if ( inData.size() > 0 ) {
			return Parse( inData.size(), &inData[0], inIds, outAttribs );
		}
	}
...
public:
	...
	UInt32				GetType() const { return _byteswap_ulong( m_nType ); }
...
public:
	...
	virtual void	CopyMetaData( const CCaptureFile& inOther ) {
		if ( inOther.GetFileFormat() == kFileFormatPeek ) {
...
public:
	;		CPeekCaptureReader()
		:	CCaptureReader( kFileFormatPeek )
	{}
...
public:
	;		CPeekCaptureWriter( int inAccess, tMediaType inType )
		:	CCaptureWriter( kFileFormatPeek, inAccess )
		,	CPeekCapture( inType )
	{}
