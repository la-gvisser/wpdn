	...
	size_t		inLength )
{
	...
	if ( spBuf.get() == nullptr ) return strResult;
	 ...
	 tTimeFormat inFormat /*= kFormat_Nanoseconds*/ )
{
	...
	switch ( inFormat ) {
		...
		case kFormat_Milliseconds:
			ssTime << L"." << std::setw( 3 ) << (tf.fNanosecond / 1000000) << L"Z";
			break;

		case kFormat_Nanoseconds:
		default:
			ssTime << L"." << std::setw( 9 ) << tf.fNanosecond << L"Z";
			break;
	}
...
CFilePacket::Compare(
	...
	UInt32				inFlags /*= kFlag_None*/,
	wstring*			inError /*= nullptr*/ )
{
	if ( &inOther == this ) return true;
	if ( inOther.m_MediaType.fType != m_MediaType.fType ) {
		if ( inError ) {
			*inError = L"Media Types don't match";
		}
		return false;
	}
	if ( inOther.m_MediaType.fSubType != m_MediaType.fSubType ) {
		if ( inError ) {
			*inError = L"Media Sub Types don't match";
		}
		return false;
	}
	if ( inOther.m_nLength != m_nLength ) {
		if ( inError ) {
			*inError = L"Lengths don't match";
		}
		return false;
	}
	if ( !(inFlags & kFlag_NoTimeStamp) ) {
		if ( inOther.m_nTimeStamp != m_nTimeStamp ) {
			if ( inError ) {
				*inError = L"Time Stamps don't match";
			}
			return false;
		}
	}
	...
	if ( IsWireless() ) {
		if ( inOther.m_nChannelNumber != m_nChannelNumber ) {
			if ( inError ) {
				*inError = L"Channel Numbers don't match";
			}
			return false;
		}
		if ( inOther.m_nDataRate != m_nDataRate ) {
			if ( inError ) {
				*inError = L"Data Rates don't match";
			}
			return false;
		}
		if ( inOther.m_nSignalStrength != m_nSignalStrength ) {
			if ( inError ) {
				*inError = L"Signal Strengths don't match";
			}
			return false;
		}
		if ( inOther.m_nNoiseStrength != m_nNoiseStrength ) {
			if ( inError ) {
				*inError = L"Noise Strengths don't match";
			}
			return false;
		}
		if ( inOther.m_nMediaSpecType != m_nMediaSpecType ) {
			if ( inError ) {
				*inError = L"Media Spec Types don't match";
			}
			return false;
		}
		if ( inOther.m_nProtocol != m_nProtocol ) {
			if ( inError ) {
				*inError = L"Protocols don't match";
			}
			return false;
		}
		if ( inOther.m_nDirection != m_nDirection ) {
			if ( inError ) {
				*inError = L"Directions don't match";
			}
			return false;
		}
		if ( inOther.m_nChannelFreq != m_nChannelFreq ) {
			if ( inError ) {
				*inError = L"Channel Frequencies don't match";
			}
			return false;
		}
		if ( inOther.m_nChannelBand != m_nChannelBand ) {
			if ( inError ) {
				*inError = L"Channel Bands don't match";
			}
			return false;
		}
		if ( inOther.m_nFlagsN != m_nFlagsN ) {
			if ( inError ) {
				*inError = L"FlagsN don't match";
			}
			return false;
		}
		for ( int i = 0; i < s_dBmCount; i++ ) {
			if ( inOther.m_nSignaldBm[i] != m_nSignaldBm[i] ) {
				if ( inError ) {
					*inError = L"Signal dBms don't match";
				}
				return false;
			}
			if ( inOther.m_nNoisedBm[i] != m_nNoisedBm[i] ) {
				if ( inError ) {
					*inError = L"Noise dBms don't match";
				}
				return false;
			}
		}
	...
	bool	bSliced = IsSliced() || inOther.IsSliced();
	size_t	nLength = GetPacketDataLength();
	size_t	nOtherLength = inOther.GetPacketDataLength();
	size_t	nCount = nLength;
	if ( inFlags & kFlag_MatchSlice ) {
		if ( bSliced ) {
			if ( nOtherLength < nLength ) {
				nCount = nOtherLength;
			}
		}
	}
	else if ( nOtherLength != nLength ) {
		if ( inError ) {
			*inError = L"Payload Lengths don't match";
		}
		return false;
	}
	...
	const UInt8*	pPacketData( &m_PacketData[0] );
	const UInt8*	pOtherPacketData( &inOther.m_PacketData[0] );
	if ( (nCount > 12) ) {
		if ( inFlags & (kFlag_NoEthernetDestination | kFlag_NoEthernetSource) ) {
			pPacketData += 12;
			pOtherPacketData += 12;
			nCount -= 12;
		}
		else if ( inFlags & kFlag_NoEthernetSource ) {
			if ( memcmp( pOtherPacketData, pPacketData, 6 ) != 0 ) {
				if ( inError ) {
					*inError = L"Ethernet Sources don't match";
				}
				return false;
			}
			pPacketData += 12;
	...
	else if ( (nCount > 6) && (inFlags & kFlag_NoEthernetDestination) ) {
		pPacketData += 6;
		pOtherPacketData += 6;
		nCount -= 6;
	}
	if ( memcmp( pOtherPacketData, pPacketData, nCount ) != 0 ) {
		if ( inError ) {
			*inError = L"Ethernet Destinations don't match";
		}
		return false;
	}

	if ( !(inFlags & kFlag_NoCRC) && !bSliced ) {
		// nCount is already decremented by 4.
		UInt32	nCRC( *(reinterpret_cast<const UInt32*>( &m_PacketData[nCount] )) );
		UInt32	nOtherCRC( *(reinterpret_cast<const UInt32*>( &inOther.m_PacketData[nCount] )) );
		if ( (nCRC != 0) || (nOtherCRC != 0) ) {
			if ( nOtherCRC != nCRC ) {
				if ( inError ) {
					*inError = L"Payloads don't match";
				}
				return false;
			}
		}
	...
	CPacketAttributeList&	outAttribs ) const
{
	...
	outAttribs.Add( TaggedFile::Attrib_TimeStampLo,
		(reinterpret_cast<const tLongTime*>( &m_nTimeStamp ))->sTime.TimeLo );
	outAttribs.Add( TaggedFile::Attrib_TimeStampHi,
		(reinterpret_cast<const tLongTime*>( &m_nTimeStamp ))->sTime.TimeHi );
	// From tagfile.h
	...
	outAttribs.Add( TaggedFile::Attrib_PacketDataLength, static_cast<UInt32>( m_PacketData.size() ) );
}
	...
	UInt64		inOffset )
{
	...
	if ( nDataLength > 0 ) {
		...
		_ASSERT( nRead == m_PacketData.size() );
	}
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		...
		switch ( inAttribs[i].GetId() ) {
			...
			case Attrib_TimeStampLo:
				(reinterpret_cast<tLongTime*>( &m_nTimeStamp ))->sTime.TimeLo = nValue;
				break;
			case Attrib_TimeStampHi:
				(reinterpret_cast<tLongTime*>( &m_nTimeStamp ))->sTime.TimeHi = nValue;
				break;
	...
	int				inMode )
{
	if ( m_spFile.get() == nullptr ) {
		m_spFile = std::make_unique<CWinFile>();
	}

	if ( m_spFile.get() == nullptr ) return false;

	m_spFile->Open( inFileName, inMode );
...
{
	if ( m_pBaseFile == nullptr ) return INVALID_HANDLE_VALUE;
	CWinFile*	pWinFile = dynamic_cast<CWinFile*>( &m_pBaseFile->GetFile() );
	...
	const wstring&	inFileName )
{
	...
	if ( (nPos != wstring::npos) && (nPos < inFileName.length()) ) {
		...
		transform( strExt.begin(), strExt.end(), strExt.begin(), towlower );
...
{
	if ( inMode.IsReader() ) {
		m_pBaseFile = OpenReader( inFileName, inMode );
	}
	else if ( inMode.IsWriter() ) {
		m_pBaseFile = OpenWriter( inFileName, inMode );
	}
	...
	const tFileMode&	inMode )
{
	...
	case kFileFormatPCap:
		...
		if ( pReader && !pReader->Open( inFileName, inMode.fScan ) ) {
			delete pReader;
			pReader = new CPCapNgCaptureReader();
			if ( pReader && !pReader->Open( inFileName, inMode.fScan ) ) {
				delete pReader;
				pReader = nullptr;
			}
	...
	const tFileMode&	inMode )
{
	...
	int	nFileType = GetFileFormat( inFileName );
	switch ( nFileType ) {
