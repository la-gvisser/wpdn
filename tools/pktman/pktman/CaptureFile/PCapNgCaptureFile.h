...
public:
	static bool		IsVersion1( const PCapNgVersion& inVersion ) {
		return ((inVersion.major == kPCapNgVersion1.major) &&
			    (inVersion.minor == kPCapNgVersion1.minor));
	}
...
public:
	...
	static bool	IsKind( const std::wstring& inKind ) {
		return (inKind.compare( L"pcapng" ) == 0);
	}
	bool		IsVersion1() const {
		return ((m_Version.major == kPCapNgVersion1.major) &&
			    (m_Version.minor == kPCapNgVersion1.minor));
	}
...
public:
	...
	virtual bool	ReadFilePackets( CFilePacketList& outPackets, UInt32 inCount,
	...
	void	SetTimeStampMultiplier( UInt64 inMultiplier ) {
		m_nTimeStampMultiplier = inMultiplier;
	}
