	...
	COSFile&	inFile )
{
	...
	size_t	uLength = bvData.size();
	if ( uLength > kMaxUInt32 ) return false;

	UInt32	nLength = static_cast<UInt32>( uLength );
	...
	UInt32				inFirst /*= NEXT_PACKET*/)
{
	...
	if ( m_ayOffsets.size() > kMaxUInt32 ) return false;
	...
	UInt32	inIndex )
{
	...
	while ( nIndex <= inIndex ) {
		...
		if ( !m_ayOffsets.SetOffset( nIndex, nOffset ) ) {
			if ( m_ayOffsets.IsEOF( nOffset ) || (nIndex == inIndex) ) {
				m_ayOffsets.SetComplete();
			}
			break;
		}
...
CPeekCaptureWriter::Close()
{
	if ( IsOpen() ) {
		...
		if ( (m_SessionBlock.GetOffset() != -1) && (wfWrite.GetSize() > m_PacketsBlock.GetOffset()) ) {
	...
	const CFilePacket&	inPacket )
{
	...
	if ( inPacket.GetPacketDataLength() > kMaxUInt32 ) return false;
