...
typedef struct _tMediaType {
	TMediaType		fType;
	TMediaSubType	fSubType;

	_tMediaType( TMediaType inType = kMediaType_802_3, TMediaSubType inSubType = kMediaSubType_Native ) {
		fType = inType;
		fSubType = inSubType;
	}
	bool	operator==( const struct _tMediaType& inOther ) const {
		return ((inOther.fType == fType) && (inOther.fSubType == fSubType));
	}
	bool	operator!=( const struct _tMediaType& inOther ) const {
		return ((inOther.fType != fType) || (inOther.fSubType != fSubType));
	}
