...
public:
	;		CStandardFile( FILE* inFile = nullptrptr  )
		:	COSFile()
		,	m_hFile( inFile )
	{}
	...
	virtual bool	IsOpen() const { return (m_hFile != nullptrptr); }
	...
	bool	SetOffset( size_t inIndex, UInt64 inOffset ) {
		...
		if ( m_ayOffsets.size() < (inIndex + 1) ) {
			...
			size_t	nNewSize = m_ayOffsets.size() +
				((nMoreOffsets > 64) ? nMoreOffsets : 64);
...
public:
	...
	bool			Compare( const CFilePacket& inOther, UInt32 inFlags = kFlag_None, std::wstring* pError = nullptr );
	...
	void	SetSignaldBm( size_t inIndex, UInt32 indBm ) {
...
public:
	...
	virtual UInt32	GetPacketCount() const override { return 0; }
	...
	virtual bool	WriteFilePacket( const CFilePacket& /*inPacket*/ ) override {
		return false;
	}
...
public:
	static int	GetFileFormat( const std::wstring& inFileName );
...
public:
	...
	;		CCaptureFile( CCaptureFile& inOther )
			:	m_pBaseFile()
	{
		std::swap( m_pBaseFile, inOther.m_pBaseFile );
	}
	...
	virtual void		CopyMetaData( const CCaptureFile& inOther ) {
		if ( m_pBaseFile != nullptr ) {
			m_pBaseFile->CopyMetaData( inOther );
		}
	}
	virtual void		Close() {
		if ( m_pBaseFile != nullptr ) {
	...
	int					GetFileAccess() const {
		return (m_pBaseFile != nullptr)
			? m_pBaseFile->GetFileAccess()
			: CF_ACCESS_UNKNOWN;
	}
	int					GetFileFormat() const {
		return (m_pBaseFile != nullptr)
			? m_pBaseFile->GetFileFormat()
			: kFileFormatUnknown;
	}
...
// 	}
	UInt64				GetFileSize() const {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->GetFileSize()
			:	0;
	}
	int					GetFileType() const {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->GetFileType()
			:	CF_TYPE_UNKNOWN;
	}
	virtual tMediaType	GetMediaType() const {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->GetMediaType()
			:	kMediaType_Ethernet;
	}
	virtual UInt32		GetPacketCount() const {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->GetPacketCount()
			:	0;
	}
...
// 	}
	...
	bool				IsReader() const {
		return (m_pBaseFile != nullptr)
			?	(m_pBaseFile->GetFileType() == CF_TYPE_READER)
			:	false;
	}
	bool				IsWriter() const {
		return (m_pBaseFile != nullptr)
			?	(m_pBaseFile->GetFileType() == CF_TYPE_WRITER)
			:	false;
	}
	...
	virtual bool	ReadFilePacket( CFilePacket& outPacket, UInt32 inIndex = NEXT_PACKET  ) {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->ReadFilePacket( outPacket, inIndex )
			: false;
	}
	virtual bool	ReadFilePackets( CFilePacketList& outPackets, UInt32 inCount, UInt32 inFirst = NEXT_PACKET ) {
		return (m_pBaseFile != nullptr)
			?	m_pBaseFile->ReadFilePackets( outPackets, inCount, inFirst )
			: false;
	}

	virtual bool	WriteFilePacket( const CFilePacket& inPacket ) {
		return (m_pBaseFile != nullptr)
			? m_pBaseFile->WriteFilePacket( inPacket )
			: false;
	}
	virtual bool	WriteFilePackets( const CFilePacketList& inPackets, UInt32& outCount ) {
		return (m_pBaseFile != nullptr)
			? m_pBaseFile->WriteFilePackets( inPackets, outCount )
			: false;
	}
};
