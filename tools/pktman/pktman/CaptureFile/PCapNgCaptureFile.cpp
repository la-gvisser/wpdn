	...
	UInt64&					outFirstPacketOffset )
{
	...
	do {
		...
		AnyBlockHeader		any;
		unique_ptr<UInt8>	spAllocData = nullptr;
		...
		if ( bSuccess ) {
			if ( any.header.type == PCAPNG_INTERFACE_DESCRIPTION_BLOCK ) {
				...
				if ( nRemaining > 0 ) {
					...
					UInt8	data[DESC_DATA_SIZE];
					UInt8*	pReadData = data;
					if ( nDataLength > DESC_DATA_SIZE ) {
						spAllocData.reset( new UInt8[nDataLength] );
						pReadData = spAllocData.get();
					}
					UInt32	cbRead = cfRead.Read( nDataLength, pReadData );
					nRemaining -= (cbRead < nDataLength) ? cbRead : nRemaining;

					UInt8*			pBegin = pReadData ;
					UInt8*			pEnd = pReadData  + cbRead;
	...
	UInt32			inIndex )
{
	...
	UInt32						nError;		// From Folder Adapter outError;

	if ( inIndex != static_cast<UInt32>( NEXT_PACKET ) ) {
		if ( !SeekToPacket( inIndex, nError ) ) {
			return false;
		}
	...
	if ( !ReadPacketHeader( pktHeader, nOptionsLength, nError ) ) {
	...
	if ( nDataRead != nBufferLength ) {
		nError = CF_ERROR_INCOMPLETE_PAYLOAD;
		return false;
	}
	...
	UInt32&	outError )
{
	...
	while ( nIndex <= inIndex ) {
		...
		nOffset = GetFile().GetPosition() + ALIGN32( pktHeader.GetDataLength() ) +
			nOptionsLength + sizeof( BlockTrailer );
	...
	const CCaptureFile*	/*inTemplate*/ )
{
	...
	blockHeader.length = sizeof( BlockHeader ) + sizeof( SectionBlockHeader ) + sizeof( BlockTrailer );
	...
	blockHeader.length = sizeof( BlockHeader ) + sizeof( InterfaceDescriptionBlockHeader ) + sizeof( BlockTrailer );
	(void) fwWriter.Write( sizeof( blockHeader ), &blockHeader );
