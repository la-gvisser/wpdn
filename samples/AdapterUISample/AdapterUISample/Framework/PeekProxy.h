...
public:
	...
	bool	RemoveAdapter( CAdapter& inAdapter ) {
		bool	bResult( false ); 
		if ( IsValid() ) {
			HeResult	hr = m_spAdapterManager->RemoveAdapter( inAdapter.GetPtr() );
			bResult = HE_SUCCEEDED( hr );
		}
		return bResult;
	}
...
protected:
	...
	void		SetPtr( Helium::IHeServiceProvider* inServiceProvider ) {
		if ( !IsValid() ) {
			inServiceProvider->QueryService( IAppConfig::GetIID(), &m_spAppConfig.p );
		}
	}
...
public:
	...
	CPeekString	GetDataDirList() const {
...
class CLogService
{
	friend class CPeekAdapterProxy;
...
public:
	;		CLogService( ILogMessage* inMessage = nullptr )
	...
	bool	IsNotValid() const { return (m_spMessageService == nullptr); }
	bool	IsValid() const { return (m_spMessageService != nullptr ); }
};
...
public:
	...
	CAdapterManager	GetAdapterManager() { return m_AdapterManager; }
	CAppConfig		GetAppConfig() { return m_AppConfig; }
};
...
class CPeekAdapterProxy
	: public CPeekProxy
{
	friend class CPeekAdapter;
...
public:
	;					CPeekAdapterProxy() {}
	virtual				~CPeekAdapterProxy() {}

	CLogService&		GetLogService() { return m_LogService; }
	CHePtr<IHardwareOptionsCollection>	GetHardwareOptionsCollection() { return m_spHardwareOptionsCollection; }
};
