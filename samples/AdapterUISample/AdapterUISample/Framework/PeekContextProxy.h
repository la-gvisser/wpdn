...
public:
	...
	void	SetHandlerId( const CGlobalId& inId ) {
		if ( IsValid() ) {
			HeLib::CHeQIPtr<IPluginUISource>	spUISource( m_spPluginAdapterInfo );
			Peek::ThrowIf( spUISource == NULL );
			Helium::HeID	id;
			memcpy( &id, &inId, sizeof( Helium::HeID ) );
			Peek::ThrowIfFailed( spUISource->SetHandlerID( &id ) );
		}
	}
	...
	void	SetPluginId( const CGlobalId& inPlugin ) {
		if ( IsValid() ) {
			Helium::HeID	id;
			memcpy( &id, &inPlugin, sizeof( Helium::HeID ) );
			Peek::ThrowIfFailed( m_spPluginAdapterInfo->SetPluginID( id ) );
		}
	}
...
public:
	...
	void	Add( CGlobalId inId ) {
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapterConfig->Add( inId ) );
		}
	}
	...
	void	Remove( CGlobalId inId ) {
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapterConfig->Remove( inId ) );
		}
	}
	void	RemoveAll() {
		if ( IsValid() ) {
...
public:
	;		CAdapter( IAdapter* inAdapter = nullptr )
		: m_spAdapter( nullptr )
	{
		SetPtr( inAdapter );
	}
	;		CAdapter( CRemotePlugin* inRemotePlugin, const CGlobalId& inId, const CPeekString& inName )
		: m_spAdapter( nullptr )
	{
		Create( inRemotePlugin, inId, inName );
	}
	;		~CAdapter() {}
	...
	void	Create( CRemotePlugin* inRemotePlugin, const CGlobalId& inId, const CPeekString& inName );

	void	GetData( int inType, UInt32* inFlags, CPeekStream& outData ) {
		if ( IsValid() ) {
			HeLib::CHeQIPtr<IPluginUISource>	spUISource( m_spAdapter );
			if ( spUISource != nullptr )
			{
				Peek::ThrowIfFailed( spUISource->GetData( inType, inFlags, outData.GetIStreamPtr() ) );
			}
		}
	}
	...
	CGlobalId		GetHandlerId() {
		...
		if ( IsValid() ) {
			...
			{
				HeID	heId;
				Peek::ThrowIfFailed( spUISource->GetHandlerID( &heId ) );
				id = heId;
			}
		}
		return id;
	...
	CPluginAdapterInfo	GetPluginAdapterInfo() {
		IPluginAdapterInfo*	pPluginAdapterInfo( NULL );
		if ( IsValid() ) {
			IAdapterInfo* pAdapterInfo( NULL );
			Peek::ThrowIfFailed( m_spAdapter->GetAdapterInfo( &pAdapterInfo ) );
			Peek::ThrowIfFailed( pAdapterInfo->QueryInterface( IPluginAdapterInfo_IID, (void**)&pPluginAdapterInfo ) );
		}
		return CPluginAdapterInfo( pPluginAdapterInfo );
	}
...
public:
	...
	CPacketBuffer&	operator=( CPersistFile& inPersistFile );
...
public:
	...
	CPeekCapture	GetPeekCapture() const {
 		CHeQIPtr<IPeekCapture> spPeekCapture( m_spHandler );
 		Peek::ThrowIf( spPeekCapture == nullptr );
...
public:
	...
	bool	Remove( CPacketHandler inItem ) {
		auto	i = find( begin(), end(), inItem );
		if ( i == end() ) return false;
		std::vector< CPacketHandler >::erase( i );
		return true;
	}
};
