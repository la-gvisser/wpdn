...
class CMessageGetPrefsPage
	:	public CPeekMessage
{
public:
	static const int			s_nMessageType = 'GPRP';
	static const CPeekString	s_strMessageName;
...
public:
	;		CMessageGetPrefsPage() : CPeekMessage( s_strMessageName ) {}
	;		CMessageGetPrefsPage( const CPeekStream& inMessage )
		: CPeekMessage( inMessage )
	{
		LoadMessage();
	}

	const CPeekStream&	GetPrefsPage() const { return m_PrefsPage; }

	virtual bool		LoadMessage();

	void				SetPreferencePage( UInt32 inCount, const UInt8* inData ) {
		m_PrefsPage.Write( inCount, inData );
	}
	bool				StartResponse() { return CPeekMessage::StartResponse( s_strMessageName ); }
	virtual bool		StoreResponse( CPeekStream& outResponse );
