...
public:
	...
	const T&		GetAt( SInt32 inIndex ) const
						{
							ASSERT(IsValidIndex(inIndex));
							UInt8*	pData = static_cast<UInt8*>(m_pData);
							return *(T*) (pData + (inIndex * sizeof(T)));
	...
	// but the hiding isn't really sufficient since you can cast to a
...
//						: TArrayEx<T>( sizeof(T), inComparator, inKeepSorted ) {}
	...
	const T*		Peek( ) const			{ return GetItem(this->GetCount() - 1); }
	T				Pop( )
	{
		T t;
		GetItem(this->GetCount() - 1, t);
		SetCount(this->GetCount() - 1);
		return t;
	}

	bool			IsEmpty() const	{	return (this->GetCount() == 0);	}
	void			Reset()			{	this->SetCount(0);				}
};
	...
	T&	outItem )
{
	...
	{
		bGotItem = true;
		this->RemoveItems( 1, CArrayEx::kArrayIndex_First );
	}
...
public:
	...
	{	// walk through and delete them all
		for (UInt32 nIdx = 0; nIdx < this->GetCount(); nIdx++)
			delete (*this)[nIdx];
	}

	virtual void SetCount(UInt32 nCount)
	{	// make new entries and add them in
		UInt32 nCurCount = this->GetCount();
		// delete the end ones
	...
	{
		ASSERT(0);
		return TArrayEx<T*>::kArrayIndex_Invalid;
	}
