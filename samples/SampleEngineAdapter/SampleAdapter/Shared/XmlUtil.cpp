...
CreateXmlDocument(
	...
	bool					bAddPI )
{
	if ( ppXmlDoc == NULL ) return HE_E_NULL_POINTER;
...
{
	// Sanity checks.
	if ( pszFile == NULL ) return HE_E_NULL_POINTER;
	if ( ppXmlDoc == NULL ) return HE_E_NULL_POINTER;
	...
	Xml::IXMLDOMDocument*	pXmlDoc )
{
	// Sanity checks.
	if ( pszFile == NULL ) return HE_E_NULL_POINTER;
	if ( pXmlDoc == NULL ) return HE_E_NULL_POINTER;
...
{
	// Sanity checks.
	if ( pDoc == NULL ) return HE_E_NULL_POINTER;
	...
	try
	{
		...
		if ( pszIndent != NULL ) strIndent = pszIndent;
		if ( strIndent.empty() )
		{
			strIndent = L"\t";
		}
...
{
	// Sanity checks.
	if ( pNode == NULL ) return HE_E_NULL_POINTER;
	...
	try
	{
		...
		if ( hr == HE_S_OK )
		{
			...
			while ( (hr == HE_S_OK) && (spChildNode != NULL) )
			{
				HeThrowIfFailed( spChildNode->get_nodeType( &typeChildNode ) );
				...
				if ( (typeChildNode == Xml::NODE_ELEMENT) && !bLastWasTextNode )
				{
					...
					HeThrowIfFailed( spDoc->createNode( CHeVariant( Xml::NODE_TEXT ), NULL, NULL, &spNewNode.p ) );
			...
			if ( !bFirstChildTextNode )
			{
				...
				HeThrowIfFailed( spNewNode->put_nodeValue( CHeVariant( strValue ) ) );
				HeThrowIfFailed( pNode->appendChild( spNewNode, NULL ) );
			}
		}
	...
	const CHeVariant&		varVal )
{
	...
	if ( varVal.vt == HE_VT_BSTR )
	{
		...
		HeThrowIfFailed( pElem->appendChild( spTextNode, NULL ) );
	}
	else
	{
		// Convert to string.
...
SetElement(
	...
	const CHeBSTR&			bstrVal )
{
	if ( pDoc == NULL ) HeThrow( HE_E_NULL_POINTER );
	if ( pElem == NULL ) HeThrow( HE_E_NULL_POINTER );
	...
	Xml::IXMLDOMElement**	ppElem )
{
	...
	HeThrowIfFailed( pNode->appendChild( spElem, NULL ) );
...
AddChildElement(
	...
	const wchar_t*			pszName,
	IHeUnknown*				pObj,
	Xml::IXMLDOMElement**	ppElem,					// default = NULL
	bool					bWriteClsidAttribute,	// default = true
	bool					bClearDirty )			// default = true
{
	// Sanity checks.
	...
	Xml::IXMLDOMElement**	ppNewElem )
{
	...
	// 
	if ( (pSrcDoc == NULL) || (pSrcNode == NULL) ||
			(pDstDoc == NULL) || (pDstNode == NULL) ) return HE_E_NULL_POINTER;
	...
	try
	{
...

		{
			...
			if ( HE_SUCCEEDED(hr) && (spAttribMap != NULL) )
			{
				SInt32	nAttribCount;
				...
				for ( SInt32 i = 0; i < nAttribCount; i++ )
				{
					...
					if ( HE_FAILED(hr) || (spAttrib == NULL) ) continue;
		...
		CHeBSTR	bstrChildValue;
		{
			...
			for ( SInt32 i = 0; i < nChildCount; i++ )
			{
				...
				{
					HeThrowIfFailed( DuplicateXmlNode( pSrcDoc, spChildNode, pDstDoc, spNewElem, NULL ) );
				}
		...
		if ( bstrChildValue.Length() > 0 )
		{
			...
			HeThrowIfFailed( spNewElem->appendChild( spNewText, NULL ) );
		}

		HeThrowIfFailed( pDstNode->appendChild( spNewElem, NULL ) );
	}
	HE_CATCH(hr)

	if ( HE_SUCCEEDED(hr) && (ppNewElem != NULL) )
	{
