...
public:
	...
	static UInt32 Increment(SInt32* p) { return ++(*p); }
	static UInt32 Decrement(SInt32* p) { return --(*p); }
};
...
public:
	...
	static UInt32 Increment(SInt32* p) { return HeAtomicIncrement32( p ); }
	static UInt32 Decrement(SInt32* p) { return HeAtomicDecrement32( p ); }
};
...
typedef HeResult (HE_CREATORARGFUNC)(void* pv, const Helium::HeIID& iid,
										void** ppv, uintptr_t dw);
...
struct _HE_INTERFACE_MAP_ENTRY
{
	Helium::HeIID		iid;
	uintptr_t			data;
	HE_CREATORARGFUNC*	pFunc; // 0:end, 1:offset, n:ptr
} HE_PACK_STRUCT;
#include "hepoppack.h"
#ifdef HE_WIN32
...
#define _HE_SIMPLEMAPENTRY	((HeLib::HE_CREATORARGFUNC*)1)

#define HE_INTERFACE_MAP_BEGIN(x) \
public: \
	typedef x _InterfaceMapClass; \
	Helium::IHeUnknown* GetUnknown() throw() \
	{ HE_ASSERT(_GetEntries()[0].pFunc == _HE_SIMPLEMAPENTRY); \
	return (Helium::IHeUnknown*)((uintptr_t)this+_GetEntries()->data); } \
	HeResult _InternalQueryInterface(const Helium::HeIID& iid, void** ppv) \
	{ return InternalQueryInterface( this, _GetEntries(), iid, ppv ); } \
	const static HeLib::_HE_INTERFACE_MAP_ENTRY* _GetEntries() { \
	static const HeLib::_HE_INTERFACE_MAP_ENTRY _entries[] = {

#define HE_INTERFACE_ENTRY_IID(iid, x) \
	{iid, \
	reinterpret_cast<uintptr_t>(static_cast<x*>((_InterfaceMapClass*)8))-8, \
	_HE_SIMPLEMAPENTRY},
...
#define HE_INTERFACE_MAP_END() \
	{HE_NULL_ID, 0, 0} }; return _entries; }

#define HE_WEAK_INTERFACE_MAP_BEGIN(x) \
public: \
	Helium::IHeUnknown* GetUnknownWeak() throw() \
	{ HE_ASSERT(_GetEntriesWeak()[0].pFunc == _HE_SIMPLEMAPENTRY); \
	return (Helium::IHeUnknown*)((uintptr_t)this+_GetEntriesWeak()->data); } \
	...
	const static HeLib::_HE_INTERFACE_MAP_ENTRY* _GetEntriesWeak() { \
	static const HeLib::_HE_INTERFACE_MAP_ENTRY _entriesWeak[] = {

#define HE_WEAK_INTERFACE_MAP_END() \
	{HE_NULL_ID, 0, 0}}; return _entriesWeak;} \
	...
	void**							ppv )
{
	...
	*ppv = NULL;
	...
	if ( iid.Equals( Helium::IHeUnknown::GetIID() ) )
	{
		// Use the first interface.
		Helium::IHeUnknown*	pUnk = reinterpret_cast<Helium::IHeUnknown*>(
								(uintptr_t)pThis+pEntries->data);
		pUnk->AddRef();
		*ppv = pUnk;
		return HE_S_OK;
	}
	...
	{
		if ( iid.Equals( pEntries->iid ) )
		{
			if ( pEntries->pFunc == _HE_SIMPLEMAPENTRY )
			{
				Helium::IHeUnknown*	pUnk = reinterpret_cast<Helium::IHeUnknown*>(
										(uintptr_t)pThis+pEntries->data);
				pUnk->AddRef();
			...
			{
				return (*pEntries->pFunc)( pThis, iid, ppv, pEntries->data );
			}
		}
		pEntries++;
	}
...
public:
	...
	virtual SInt32 Lock()
	{
		return CHeGlobalsThreadModel::Increment( &m_nLockCount );
	}

	virtual SInt32 Unlock()
	{
		return CHeGlobalsThreadModel::Decrement( &m_nLockCount );
	}
...
public:
	...
	{
		UInt32	cRef = this->InternalRelease();
		if ( cRef == 0 )
		{
			delete this;
		}
...
public:
	...
	{
		UInt32	cRef = this->InternalWeakRelease();
...
public:
	...
	{
#ifdef _DEBUG
		SInt32	cRef = _ThreadModel::Decrement( &m_cRef );
		if ( cRef < -(INT_MAX / 2) )
		{
