...
public:
	...
	HeResult CopyTo( Helium::HEBSTR* pbstrDest )
	{
		...
		*pbstrDest = HeLib::HeAllocStringByteLen( (char*) bstrVal, HeStringByteLen( bstrVal ) );
	...
	HeResult	WriteToStream( Helium::IHeStream* pStream );
...
inline HeResult CHeVariant::WriteToStream( Helium::IHeStream* pStream )
{
	using namespace Helium;
	...
	if ( cbWrite != 0 )
	{
		return pStream->Write( &bVal, cbWrite, NULL );
	}
...
inline UInt32 CHeVariant::GetSize() const
{
	...
	switch ( vt )
	{
		...
		case HE_VT_DISPATCH:
		{
			...
			if ( spStream != NULL )
			{
				...
				nSize += static_cast<UInt32>(cbSizeMax + sizeof(HeCID));
			}
			else
			{
				nSize += sizeof(HeCID);
	...
	if ( nSize == sizeof(HEVARTYPE) )
	{
		...
		{
			const std::size_t	cb = sizeof(UInt32) + HeStringByteLen( bstr ) + sizeof(wchar_t);
			nSize += static_cast<UInt32>(cb);
		}
	}
	return nSize;
}
