...
public:
	...
	static bool	IsGreaterThan( const GUID& inA, const GUID& inB ) {
		return (memcmp( &inA, &inB, sizeof( GUID ) ) > 0);
	}
	static bool	IsLessThan( const GUID& inA, const GUID& inB ) {
		return (memcmp( &inA, &inB, sizeof( GUID ) ) < 0);
	}
...
public:
	...
	bool	operator>( const CGlobalId& inOther ) const { return IsGreaterThan( m_Id, inOther.m_Id ); }
	bool	operator<( const CGlobalId& inOther ) const { return IsLessThan( m_Id, inOther.m_Id ); }
