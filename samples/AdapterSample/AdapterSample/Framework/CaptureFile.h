...
public:
	...
	bool	SetOffset( size_t inIndex, UInt64 inOffset ) {
		...
		if ( m_ayOffsets.size() < (inIndex + 1) ) {
			...
			UInt32	nMoreOffsets = (m_nFileSize > 0)
				? static_cast<UInt32>( (m_nFileSize - inOffset) / 256 )
				: static_cast<UInt32>( m_ayOffsets.size() );
			size_t	nNewSize = m_ayOffsets.size() + ((nMoreOffsets > 64) ? nMoreOffsets : 64);
...
protected:
	...
	UInt32		m_nLength;			// Packet Length on the wire.
...
public:
	...
	UInt32			GetPacketDataLength() const { return static_cast<UInt32>( m_PacketData.size() ); }
	...
	UInt32			GetSliceLength() const { return (GetLength() == GetPacketDataLength()) ? 0 : GetPacketDataLength(); }
	...
	void	SetPacketData( UInt32 inLength, UInt32 inDataLength, UInt8* inData ) {
		m_nLength = inLength;
		if ( inDataLength > 0 ) {
