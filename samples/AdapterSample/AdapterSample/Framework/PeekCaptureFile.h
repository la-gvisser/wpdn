...
public:
	...
	virtual bool	Read( CWinFile& inFile );
	virtual bool	ReadData( CWinFile& inFile );
...
public:
	...
	UInt32				GetPacketCount() const {
		return (m_SessionBlock.GetPacketCount())
			...
			: static_cast<UInt32>( m_ayOffsets.GetHighest() + 1 );
	}
...
public:
	...
	virtual void	Init( const CCaptureFile* inTemplate = NULL );
	static bool		IsKind( const std::wstring& inKind ) {
		return CPeekCapture::IsKind( inKind );
	}
