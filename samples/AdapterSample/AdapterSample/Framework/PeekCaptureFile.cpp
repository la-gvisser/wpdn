	...
	CWinFile&	inFile )
{
	...
	m_bvData.resize( m_nLength );
	UInt32	dwBytesRead = static_cast<UInt32>( inFile.Read( m_nLength & 0xFFFFFFFF, &m_bvData[0] ) );
	return (dwBytesRead >= m_nLength );
}
	...
	CWinFile&	inFile )
{
	...
	UInt32	nWrite = inFile.Write( static_cast<UInt32>( bvData.size() ), &bvData[0] );
	return (nWrite == bvData.size());
}
	...
	CByteVector&	outData )
{
	...
	const UInt32	nBlockSize = static_cast<UInt32>( ((strXml.size() + 0x1000) / 0x1000) * 0x1000);
	outData.resize( nBlockSize, ' ' );
	...
	UInt32			inIndex )
{
	...
	UInt32	nDataRead =GetFile().Read(  static_cast<UInt32>( bvPacketData.size() ), &bvPacketData[0] );
	_ASSERT( nDataRead == bvPacketData.size() );
	...
	UInt32				inFirst /*= NEXT_PACKET*/)
{
	...
	UInt32	nPacketCount = static_cast<UInt32>( m_ayOffsets.size() );
	if ( inFirst > nPacketCount ) return false;

	UInt32	nEndIndex = inFirst + inCount;
	...
	while ( nIndex <= inIndex ) {
		...
		for ( int a = 0; a < nAttribMax; a++ ) {
			if ( pAttrib->fieldType == Attrib_PacketDataLength ) {
				...
				pAttrib++;
				nPacketLength =
					static_cast<UInt32>( (reinterpret_cast<byte*>( pAttrib ) - 
						reinterpret_cast<byte*>( ayPacketData )) +
						nPacketDataLength );
				break;
			}
			pAttrib++;
		}
		_ASSERT( nPacketLength > 0 );
		if ( nPacketLength == 0 ) {
			return false;
		}
	...
	const CFilePacket&	inPacket )
{
	...
	GetFile().Write( inPacket.GetPacketDataLength(), inPacket.GetPacketData() );
