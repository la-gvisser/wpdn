	...
	UInt32			inIndex )
{
	...
	UInt32	nDataRead = GetFile().Read(
		static_cast<UInt32>( bvPacketData.size() ), &bvPacketData[0] );
	_ASSERT( nDataRead == bvPacketData.size() );
	if ( nDataRead != bvPacketData.size() ) return false;

	outPacket.SetPacketData( pktHeader.m_nPacketLength,
		static_cast<UInt32>( bvPacketData.size() ), &bvPacketData[0] );
	...
	const CFilePacket&	inPacket )
{
	...
	pktHeader.caplen = inPacket.GetLength() - 4;
	pktHeader.len = inPacket.GetPacketDataLength();
