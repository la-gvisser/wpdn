...
public:
	...
	HWND		AddTabWindow( CPeekString inInsertPoint,  bool bInsertAfter,
					CPeekString inCommand, CPeekString inWindowClass, CPeekString inWindowName ) {
		HWND	hWnd = NULL;
		if ( IsValid() ) {
	...
	HWND		AddTabWindow( CPeekString inCommand, CPeekString inWindowClass,
					CPeekString inWindowName ) {
		HWND	hWnd = NULL;
		if ( IsValid() ) {
			CHeBSTR	bstrCommand( inCommand );
			CHeBSTR	bstrClass( inWindowClass );
	...
	IUnknown*	AddTabControl( CPeekString inInsertPoint, bool bInsertAfter,
					CPeekString inCommand, CPeekString inControlClass,
					CPeekString inWindowName ) {
	...
	void		DoRefresh( CPeekString inTab ) {
		if ( IsValid() ) {
...
protected:
	...
	void			SetPtr( Helium::IHeServiceProvider* inServiceProvider ) {
		if ( m_spAppConfig == NULL ) {
			inServiceProvider->QueryService(
				ILogMessage::GetIID(), &m_spAppConfig.p );
		}
	}

public:
	;		CAppConfig( IAppConfig* inAppConfig = NULL )
		: m_spAppConfig( inAppConfig ) {}
	;		~CAppConfig() { ReleasePtr(); }
	...
	bool	IsNotValid() const { return (m_spAppConfig == NULL); }
	bool	IsValid() const { return (m_spAppConfig != NULL ); }
};
...
public:
	...
	static bool		ValidateConsole() {
		if ( IsConsole() ) return true; ASSERT( 0 ); return false;
	}
	static bool		ValidateEngine() {
		if ( IsEngine() ) return true; ASSERT( 0 ); return false;
	}
#ifdef _DEBUG
	static void		ValidateHost( bool inHostType ) {
		ASSERT( s_bEngineHost == inHostType );
	}
#endif
