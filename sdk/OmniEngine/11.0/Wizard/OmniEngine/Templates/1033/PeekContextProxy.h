...
public:
	...
	bool	IsNotValid() const { return (m_spMediaInfo == nullptr); }
	bool	IsValid() const { return (m_spMediaInfo != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spChannelManager == nullptr); }
	bool	IsValid() const { return (m_spChannelManager != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spUpdateSummaryStats == nullptr); }
	bool	IsValid() const { return (m_spUpdateSummaryStats != nullptr); }
...
public:
	...
	CChannelManager	GetChannelManager() {
		CHePtr<IChannelManager>	spChannelManager;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapterInfo->GetChannelManager( &spChannelManager.p ) );
		}
		return CChannelManager( spChannelManager );
	}
	...
	CMediaInfo	GetMediaInfo() {
		CHePtr<IMediaInfo> pMediaInfo;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapterInfo->GetMediaInfo( &pMediaInfo.p ) );
		}
		return CMediaInfo(pMediaInfo);
	}
	tMediaType	GetMediaType() const {
		tMediaType	mt;
		if ( IsValid() ) {
	...
	bool	IsNotValid() const { return (m_spAdapterInfo == nullptr); }
	bool	IsPersisitent() const { return GetPersistent(); }
	bool	IsValid() const { return (m_spAdapterInfo != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spPluginAdapterInfo == nullptr); }
	bool	IsValid() const { return (m_spPluginAdapterInfo != nullptr); }
	...
	void	SetIdentifier( const CGlobalId& inId ) {
		if ( IsValid() ) {
...
public:
	...
	bool	IsNotValid() const { return (m_spAdapterInfoList == nullptr); }
	bool	IsValid() const { return (m_spAdapterInfoList != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spAdapterConfig == nullptr); }
	bool	IsValid() const { return (m_spAdapterConfig != nullptr); }
...
public:
	...
	CAdapterInfo		GetAdapterInfo() {
		CHePtr<IAdapterInfo> pAdapterInfo;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapter->GetAdapterInfo( &pAdapterInfo.p ) );
		}
...
public:
	...
	bool	IsNotValid() const { return (m_spInitialize== nullptr); }
	bool	IsValid() const { return (m_spInitialize != nullptr); }
};
...
public:
	...
	CAdapter	GetAdapter( PeekAdapterType inType, const CGlobalId& inId,
	...
	CAdapterInfoList	GetAdapterInfoList() {
		CHePtr<IAdapterInfoList> pAdapterList;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spAdapterManager->GetAdapterInfoList( &pAdapterList.p ) );
		}
		return CAdapterInfoList( pAdapterList );
	}

	bool	IsNotValid() const { return (m_spAdapterManager == nullptr); }
	bool	IsValid() const { return (m_spAdapterManager != nullptr); }
...
public:
	...
	CPeekXml	GetCaptureTemplate() const {
		CPeekXml	xmlDoc;
		if ( IsValid() ) {
			CHeQIPtr<IPeekCaptureInitialize>	spInitialize( m_spPeekCapture );
			if ( spInitialize ) {
				Helium::IHePropertyBag* pTemplate( nullptr );
				Peek::ThrowIfFailed( spInitialize->GetCaptureTemplate( &pTemplate ) );
				CHeQIPtr<Helium::IHePersistXml>	spPersistXml( pTemplate );
				if ( spPersistXml ) {
					xmlDoc.Create( L"capturetemplate", false, true );
					spPersistXml->Save( static_cast<CPeekXmlDocument>( xmlDoc ), xmlDoc.GetElement(), FALSE );
				}
			}
		}
		return xmlDoc;
...
{
	friend class COmniAdapter;
	friend class QueueThreadObject;
...
public:
	...
	bool	StartPacketProvider() {
		CHePtr<IPacketHandler> safeHandle( m_spHandler );
		if ( safeHandle == nullptr ) return false;

		HeLib::CHeQIPtr<IPacketProviderEvents>	spEvents( safeHandle );
		if ( spEvents != nullptr ) {
			spEvents->OnStartPacketProvider();
		}

		return true;
	}

	bool	StopPacketProvider() {
		CHePtr<IPacketHandler> safeHandle( m_spHandler );
		if ( safeHandle == nullptr ) return false;

		HeLib::CHeQIPtr<IPacketProviderEvents>	spEvents( safeHandle );
		if ( spEvents != nullptr ) {
			spEvents->OnStopPacketProvider();
		}
...
public:
	;		CPeekContextProxy()
		:	m_pUnkSite( nullptr )
		,	m_nMediaType( kMediaType_802_3 )
		,	m_nMediaSubType( kMediaSubType_Native )
	{}
