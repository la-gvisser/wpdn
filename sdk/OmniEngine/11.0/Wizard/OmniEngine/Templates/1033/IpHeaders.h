...
public:
	...
	size_t	Add( const CIpAddress& inAddress ) {
		push_back( inAddress );
		return (size() - 1);
	}
	size_t	AddUnique( const CIpAddress& inAddress ) {
	...
	bool	Find( const CIpAddress& inAddress, size_t& outIndex ) const {
		size_t	nCount = size();
		for ( size_t i = 0; i < nCount; i++ ) {
			if ( inAddress == at( i ) ) {
				outIndex = i;
...
public:
	...
	void			SetIpAddress( const CIpAddress& inIpAddress ) {
		m_IpAddress = inIpAddress;
	}
