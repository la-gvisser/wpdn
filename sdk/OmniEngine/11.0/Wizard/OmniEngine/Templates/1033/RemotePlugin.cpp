	...
	const std::string& inMessage )
{
	...
	if ( !fb.is_open() ) {
		...
		std::string		strFileName = strTempDir + "[!output SAFE_PROJECT_NAME] " + ptNow.FormatTimeA() + ".txt";
...
// -----------------------------------------------------------------------------
//		IEnginePluginCaptureEvents::OnCaptureCreated
// -----------------------------------------------------------------------------

HE_IMETHODIMP
CRemotePlugin::OnCaptureCreated(
	IHeUnknown*	inCapture )
{
  #ifdef DEBUG_LOGINTERFACE
	AddLogMsg( "On Capture Created" );
  #endif
	...
	HeResult	hr = HE_E_FAIL;
	try {
		CHeQIPtr<IPeekCapture>	spCapture( inCapture );
		CPeekCapture			capture( spCapture );
		if ( m_spPlugin->OnCaptureCreated( capture ) == PEEK_PLUGIN_SUCCESS ) {
			hr = HE_S_OK;
		}
	}
	HE_CATCH( hr );
...
// -----------------------------------------------------------------------------
//		IEnginePluginCaptureEvents::OnCaptureDeleted
// -----------------------------------------------------------------------------

HE_IMETHODIMP
CRemotePlugin::OnCaptureDeleted(
	IHeUnknown*	inCapture )
{
  #ifdef DEBUG_LOGINTERFACE
	AddLogMsg( "On Capture Deleted" );
  #endif
	...
	try {
		CHeQIPtr<IPeekCapture>	spCapture( inCapture );
		CPeekCapture	capture( spCapture );
		if ( m_spPlugin->OnCaptureDeleted( capture ) == PEEK_PLUGIN_SUCCESS ) {
			hr = HE_S_OK;
		}
	}
	HE_CATCH( hr );
