...
CPeekCapture::GetPacketHandler() const {
	CHeQIPtr<IPacketHandler> spPacketHandler( m_spPeekCapture );
	Peek::ThrowIf( spPacketHandler == nullptr );
	...
	bool&		outHasGraphs )
{
	...
	try {
		CHeQIPtr<IPeekCapture>	spPeekCapture( inUnkSite );
		if ( spPeekCapture != nullptr ) {
		...
		CHeQIPtr<IFileView>	spFileContext( inUnkSite );
		if ( spFileContext != nullptr ) {
