...
private:
	void		CreateStream() {
		HeLib::CHePtr<IMemoryStream>	spMemoryStream;
...
protected:
	HeLib::CHePtr<Helium::IHeStream>	m_spStream;
...
public:
	...
	operator Helium::IHeStream*() const { return GetIStreamPtr(); }

	void		Attach( size_t inCount, const void* inData ) {
		HeLib::CHeQIPtr<IMemoryStream2>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
		Peek::ThrowIfFailed( spMemoryStream->Attach( inData, inCount ) );
	}
	...
	void		Clear() {
		HeLib::CHeQIPtr<IMemoryStream2>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
		Peek::ThrowIfFailed( spMemoryStream->Clear() );
	}
	...
	void*		Detach( size_t* outCount ) {
		HeLib::CHeQIPtr<IMemoryStream2>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
		void*	pData( nullptr );
		Peek::ThrowIfFailed( spMemoryStream->Detach( &pData, outCount ) );
		return pData;
	}
	...
	size_t		GetCapacity() {
		...
		Peek::ThrowIf( spMemoryStream == nullptr );
		size_t	nCount( 0 );
		Peek::ThrowIfFailed( spMemoryStream->GetCapacity( &nCount ) );
		return nCount;
	}
	// Returns pointer to internal data.
	UInt8*		GetData() const {
		HeLib::CHeQIPtr<IMemoryStream>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
	...
	void		Reserve( size_t inCount ) {
		HeLib::CHeQIPtr<IMemoryStream2>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
		Peek::ThrowIfFailed( spMemoryStream->Reserve( inCount ) );
	}
	...
	void		ShrinkToFit() {
		HeLib::CHeQIPtr<IMemoryStream2>	spMemoryStream( m_spStream );
		Peek::ThrowIf( spMemoryStream == nullptr );
		Peek::ThrowIfFailed( spMemoryStream->ShrinkToFit() );
	}
