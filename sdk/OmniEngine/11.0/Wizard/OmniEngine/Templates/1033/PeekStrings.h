...
public:
	...
	CPeekString&	MakeLower() {
		std::transform( cbegin(), cend(), begin(), towlower );
		return *this;
	}
	CPeekString&	MakeUpper() {
		std::transform( cbegin(), cend(), begin(), towupper );
		return *this;
	}
...
public:
	...
	CPeekStringA&	MakeUpper() {
		std::transform( cbegin(), cend(), begin(), [](const unsigned char i){ return static_cast<unsigned char>( toupper(i) ); });
		return *this;
	}
...
public:
	...
	int			CompareNoCase( const CPeekStringA& inOther ) const {
		return m_strA.CompareNoCase( inOther );
	}
	int			CompareNoCase( const CPeekString& inOther ) const {
		return m_strW.CompareNoCase( inOther );
	}
