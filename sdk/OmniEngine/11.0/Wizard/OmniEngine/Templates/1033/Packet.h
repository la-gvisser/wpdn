...
public:
	...
	void		Set( UInt16 inProtoSpec, const CByteVectRef& inHeader, const CByteVectRef& inData ) {
...
protected:
	CHePtr<IPacketLayers>	m_spLayers;
...
public:
	;			CPacketLayers( void* inPtr = nullptr );
	...
	bool		IsNotValid() const { return (m_spLayers == nullptr); }
	bool		IsValid() const { return (m_spLayers != nullptr); }
...
public:
	;		CPacketData( const UInt8* inData = nullptr ) : m_pData( inData ) {}
	;		~CPacketData() { Dealloc( m_pData ); }

#if 0
	static UInt8*	Alloc( size_t inLength ) { return new UInt8[inLength]; }
	...
	static void		Dealloc( const UInt8* inData ) { free( (void*)inData ); }
#endif

	;		operator const UInt8*() const { return m_pData; }

	bool	IsEmpty() const { return (m_pData == nullptr); }
	bool	IsNotValid() const { return (m_pData == nullptr); }
	bool	IsValid() const { return (m_pData != nullptr); }
};

typedef std::shared_ptr<CPacketData>	CPacketDataPtr;
...
protected:
	...
	static const UInt8*	AllocFrom( IPacket* inIPacket, tPacketInfo& outInfo );
	static UInt8*		AllocFrom( const PeekPacket* inPacket, const UInt8* inData, tPacketInfo& outInfo );
...
public:
	...
	CPacket&		operator=( const CPacket& inOther ) {
		if ( this == &inOther ) return *this;	// self assignment.
		Copy( inOther );
		return *this;
	}
	bool			operator==( CPacket& inOther ) {
	...
	// GetAllPacketData(Ref) includes the FCS and any Extra Bytes.
	UInt16			GetAllPacketData( const UInt8*& pData ) const;
	UInt16			GetAllPacketData( CPeekStream& outData ) const {
		const UInt8*	pData = nullptr;
		UInt16			nDataLen = GetAllPacketData( pData );
		if ( (nDataLen > 0) && (pData != nullptr) ) {
			outData.Write( nDataLen, pData );
			return nDataLen;
		}
		return 0;
	}
	CByteVectRef	GetAllPacketDataRef() const {
		const UInt8*	pData = nullptr;
		UInt16			nDataLen = GetAllPacketData( pData );
		CByteVectRef	bvData( nDataLen, pData );
		return bvData;
	}
	TMediaSpec		GetApplication() const {
		TMediaSpec	ms;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spPacket->GetApplication( &ms ) );
		}
		return ms;
	}
	#pragma message( "TODO: Verify this." )
	CString		GetApplicationString() const {
		TMediaSpec	ms = GetApplication();
	...
	UInt32			GetFrameCheckSequence() const {
		UInt32	nFCS = 0;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spPacket->GetFrameCheckSequence( &nFCS ) );
		}
		return nFCS;
	}
	...
	// GetPacketData(Ref) does not include the FCS or Extra Bytes.
	...
	UInt16			GetPacketData( CPeekStream& outData ) const {
		const UInt8*	pData = nullptr;
		UInt16			nDataLen = GetPacketData( pData );
		if ( nDataLen > 0 ) outData.Write( nDataLen, pData );
		return nDataLen;
	}
	...
	// GetPacketDataLength includes the FCS and any Extra Bytes.
	...
	PeekPacket		GetPeekPacket() const {
		const PeekPacket*	pHeader = nullptr;
		if ( IsValid() ) {
	...
	void			Initialize( PeekPacket* inPacket, const UInt8* inData, tMediaType inMediaType ) {
		try {
			_ASSERTE( IsValid() );
			if ( IsValid() ) {
				Peek::ThrowIfFailed( m_spPacket->Initialize( nullptr, 0, inPacket,
					inData, inMediaType.fType, inMediaType.fSubType ) );
			}
		}
		catch( ... ) {
			throw;
	...
	bool			IsDataOwner() const { return (m_spData != nullptr); }
	...
	bool			IsError() const {
		int	bResult = TRUE;	// If not valid the it's an error packet.
		if ( IsValid() ) {
	...
	bool			IsNotValid() const { return (m_spPacket == nullptr); }
	bool			IsValid() const { return (m_spPacket != nullptr); }
	bool			IsWAN() const {
		int	bResult = FALSE;
		if ( IsValid() ) {
