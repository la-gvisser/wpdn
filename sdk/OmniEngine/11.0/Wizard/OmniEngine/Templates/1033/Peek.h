...
inline bool Throw( const SInt32 hr )
{
	throw hr;
}
...
{
	_ASSERTE( !b );
	if ( b ) throw( HE_E_FAIL );
	return b;
}

inline SInt32 ThrowIfFailed( const SInt32 hr )
{
...
inline bool ThrowIfNull( void* ptr )
{
	_ASSERTE( ptr != nullptr );
	if ( ptr == nullptr ) throw( HE_E_NO_INTERFACE );
	return (ptr == nullptr);
}
...
enum SummaryStatFlags
{
	kSummaryStatFlag_Ungraphable = 0x00000001,	// This item should not be graphed.
	kSummaryStatFlag_OverTime    = 0x00000002,	// This item is an average over time.
	kSummaryStatFlag_Sampled     = 0x00000004,	// This item is a sample per second.
	kSummaryStats_SortSubItems   = 0x00000008,	// If this item has subitems, sort them alphabetically.
	kSummaryStatFlag_Counter     = 0x00000010	// This item is a counter that should be reduced using differences.
};
...
struct SummaryStatValuePair {
	...
	SummaryStatValuePair()
		:	Packets( 0 )
		,	Bytes( 0 )
	{}
} WP_PACK_STRUCT;
typedef struct SummaryStatValuePair	SummaryStatValuePair;
...
typedef union SummaryStatValue	SummaryStatValue;

struct SummaryStat {
	SummaryStatType			Type;
	UInt32					Flags;	// SummaryStatFlags
	SummaryStatValue		Value;

	SummaryStat()
		:	Type( kSummaryStatType_Null )
		,	Flags( 0 )
	{}
} WP_PACK_STRUCT;
typedef struct SummaryStat	SummaryStat;
...
typedef struct SummaryStatEntry	SummaryStatEntry;
