...
// =============================================================================
//		CMessageImportExport
// =============================================================================

class CMessageImportExport
	:	public CPeekMessage
{
public:
	static const int			s_nMessageType = 'MIMP';
	static const CPeekString	s_strMessageName;
	static const UInt8			s_nIdImport;
	static const UInt8			s_nIdExport;

protected:
//	static const CPeekString	s_strTagName;
	//	static const CPeekString	s_strTagId;
	static const CPeekString	s_strTagOptions;
	static const CPeekString	s_strTagImportExportMessage;
	static const CPeekString	s_strTagIdImportExport;

protected:
	CPeekString		m_strName;
	CPeekString		m_strImportMessage;
	COptions		m_Options;
	UInt8			m_nIdImportExport;

public:
	;		CMessageImportExport();
	;		CMessageImportExport( UInt8 inIdImportExport );
	;		CMessageImportExport( const CPeekStream& inMessage )
		: CPeekMessage( inMessage )
	{
		LoadMessage();
	}
...
//	const COptions&	GetOptions() const { return m_Options; }
	COptions&		GetOptions() { return m_Options; }
//	UInt8			GetImportExportId() { return m_nIdImportExport; }
	CPeekString		GetImportExportMessage() { return m_strImportMessage; }

	bool			IsImport() { return (m_nIdImportExport == s_nIdImport); }
	bool			IsExport() { return (m_nIdImportExport == s_nIdExport); }
	
	virtual bool	LoadMessage();
	virtual bool	LoadResponse( const CPeekStream& inResponse );

	bool			ModelAllItems( bool bModelOptions );

	void			SetAsImport() { m_nIdImportExport = s_nIdImport; }
	void			SetAsExport() { m_nIdImportExport = s_nIdExport; }

	void			SetOptions( const COptions& inOptions ) { m_Options = inOptions; }
	void			SetImportExportMessage( CPeekString& inFilePath ) { m_strImportMessage = inFilePath; }
	void			SetIdImportExport( UInt8 inIdImportExport ) { m_nIdImportExport = inIdImportExport; }
	bool			StartResponse() { return CPeekMessage::StartResponse( s_strMessageName ); }
	virtual bool	StoreMessage( CPeekStream& outMessage );
	virtual bool	StoreResponse( CPeekStream& outResponse );
