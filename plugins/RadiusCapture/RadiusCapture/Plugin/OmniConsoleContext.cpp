	...
	CPeekDataModeler& ioPrefs )
{
	...
	if ( ioPrefs.IsLoading() && m_Id.IsNull() ) {
	  #ifdef RESET_OPTIONS_ON_LOAD
		m_Options.Reset();
	  #else // RESET_OPTIONS_ON_LOAD
		m_Options.Model( ioPrefs );
	  #endif // RESET_OPTIONS_ON_LOAD
	}
	else {
		m_Options.Model( ioPrefs );
	}
...
COmniConsoleContext::OnOptions()
{
	...
	COmniPlugin*	pPlugin( m_RemotePlugin.GetPlugin() );

	COptions		theOptions( m_Options );
	COptionsDialog	OptionsDlg( &theOptions, bIsCapturing, false, pPlugin );

	if ( OptionsDlg.DoModal() == IDOK ) {
		if ( theOptions != m_Options ) {
