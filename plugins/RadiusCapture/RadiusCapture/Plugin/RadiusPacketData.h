...
/*
	...
	CRadiusPacketData( PluginFilterParam* ioparams ) {
		PeekPacket* fPacket = (PeekPacket*)ioparams->fPacket;

		m_nDataLength = (fPacket->fSliceLength > 0)
			? min( fPacket->fSliceLength, fPacket->fPacketLength )
			: fPacket->fPacketLength;

		ASSERT( m_nDataLength > 0 );
		NewRadiusPacketData( fPacket, ioparams->fPacketData, m_nDataLength );
		ioparams->fBytesAccepted = m_nDataLength;
	}
...
public:
	...
	void AddNewPacketData( PluginFilterParam* ioparams ) {
			Add( ioparams );
			ASSERT( GetCount() == 1 );
			UInt64 uTimeStamp = GetTimeStamp();
			if ( uTimeStamp > m_uLatestTimestamp ) {
				m_uLatestTimestamp = uTimeStamp;
			}
		}
