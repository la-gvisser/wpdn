...
{
	UInt64	nPos = ftell( m_hFile );
	fseek( m_hFile, 0, SEEK_END );
	UInt64	nSize = ftell( m_hFile );
	fseek( m_hFile, nPos, SEEK_SET );
	return nSize;
}
...
CStandardFile::Open(
	...
	UInt32			inMode )
{
	// TODOD - implement this properly.
	const static UInt32	kFileMode_Create{ FILEMODE_CREATE };
	const static UInt32	kFileMode_Read{ FILEMODE_READ };
	const static UInt32	kFileMode_Write{ FILEMODE_WRITE };
...
{
	if ( (inLength > 0) && IsOpen() ) {
		return fread( outData, inLength, 1, m_hFile );
	}
	return 0;
}
...
{
	if ( (inLength > 0) && IsOpen() ) {
		return fwrite( inData, inLength, 1, m_hFile );
	}
	return 0;
}
#endif /* _WIN32 */
...
CCaptureReader::Open(
	...
	bool			inScanPackets )
{
	if ( CBaseFile::Open( inFileName, COSFile::FILEMODE_READ ) ) {
		Parse( inScanPackets );
	}
