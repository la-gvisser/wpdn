...
public:
	...
	bool	Copy( const CCaptureBuffer& inOther ) {
		...
		_ASSERTE( inOther.m_DynamicBuffer.IsValid() );
		...
		_ASSERTE( GetCount() == inOther.GetCount() );
		return true;
	}
	...
	bool		WritePacket( const CPacket& inPacket ) { return m_DynamicBuffer.PutPacket( inPacket ); }
