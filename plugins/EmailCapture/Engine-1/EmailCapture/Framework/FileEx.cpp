...
CFileEx::Close()
{
	...
	if ( !m_strBaseName.IsEmpty() ) {
		...
		if ( ::MoveFile( strInProgressName, m_strBaseName ) == FALSE ) {
...
CFileEx::GetFileCreate() const
{
		...
		if ( !::GetFileTime( m_hFile, &ftCreate, &ftAccess, &ftModify ) ) {
			return CPeekTime::Now();
		}

		return CPeekTime( ftCreate );
...
CFileEx::GetLength() const
{
	...
	if ( liSize.LowPart == static_cast<DWORD>( -1 ) ) {
		m_dwLastError = ::GetLastError();
		if ( m_dwLastError != NO_ERROR ) throw -1;
	}
...
{
	CPeekString	strPath( inPath );
	_ASSERTE( !strPath.IsEmpty() );
	if ( strPath.IsEmpty() ) return false;

	if ( IsFilePath( strPath ) ) return true;
