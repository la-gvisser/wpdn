...
public:
	...
	//void		Invert( CIpAddressPortPair& outOther ) const {
	//	outOther.m_Source = m_Destination;
	//	outOther.m_Destination = m_Source;
	//}

	void		Set( const CIpHeaderRef& inIpHeader, const tTcpHeader* inTcpHeader = NULL );
...
public:
	;			CEthernetHeader( const tEthernetHeader* inHeader = NULL )
	{
		if ( inHeader ) {
			m_Header = *inHeader;
		}
...
public:
	;			CEthernetHeaderRef( const tEthernetHeader* inHeader = NULL ) : m_pHeader( inHeader ) {}
	;			~CEthernetHeaderRef() { m_pHeader = NULL; }
...
public:
	...
	UInt16			GetPayloadLength() const { return (NETWORKTOHOST16( m_Header.TotalLength ) - GetLength()); }
...
public:
	;			CIpHeaderRef( int inVersion ) : m_nVersion( inVersion ), m_bValid( false ) {}
...
public:
	;			CIpV4HeaderRef() : CIpHeaderRef( kIpVersion_4 ), m_pHeader( NULL ) {}
	...
	UInt16			GetLength() const {	// Header Length.
		AssertValid();
		return ((m_pHeader->VersionLength & 0x0F) * 4);
	}
	const UInt8*	GetPayload() const;
	...
	// IPeekHash
	virtual size_t	Hash() const { return (m_pHeader != NULL) ? m_pHeader->Hash() : 0; }
};
...
public:
	;			CIpV6HeaderRef() : CIpHeaderRef( kIpVersion_6 ), m_pHeader( NULL ) {}
...
public:
	;			CTcpHeader( const tTcpHeader* inHeader = NULL )
	{
		Set( inHeader );
	}
	;			~CTcpHeader() {}
...
public:
	;			CTcpHeaderRef( const tTcpHeader* inHeader = NULL ) : m_pHeader( inHeader ) {}
	;			~CTcpHeaderRef() {}
	...
	// IPeekHash
	size_t		Hash() const { return (m_pHeader != NULL) ? m_pHeader->Hash() : 0; }
};
...
public:
	;			CUdpHeader( const tUdpHeader* inHeader = NULL )
	{
		Set( inHeader );
	}
	;			~CUdpHeader() {}
...
public:
	;			CUdpHeaderRef( const tUdpHeader* inHeader = NULL ) : m_pHeader( inHeader ) {}
	;			~CUdpHeaderRef() {}
...
public:
	;			CDnsHeader( const tDnsHeader* inHeader = NULL )
	{
		Set( inHeader );
	}
	;			~CDnsHeader() {}
...
public:
	;			CDnsHeaderRef( const tDnsHeader* inHeader = NULL ) : m_pHeader( inHeader ) {}
	;			~CDnsHeaderRef() {}
