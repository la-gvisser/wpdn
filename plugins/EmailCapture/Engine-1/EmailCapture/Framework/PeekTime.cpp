	...
	tTimeUnits	inType )
{
	...
	UInt64	nRounded = ((inDuration + (nConversion / 2)) / nConversion) * nConversion;
	return nRounded;
}
	...
	const CPeekTimeLocal& inTime )
{
	...
	FILETIME	ftTime;
	::LocalFileTimeToFileTime( &ftLocal, &ftTime );
	m_ltTime = ftTime;
}
	...
	tTimeUnits			inFormat /*= kMilliseconds*/ )
{
	...
	if ( inFormat == kMilliseconds ) {
		ssTime << std::setw( 3 ) << (inTime.fNanosecond / 1000000);
	}
	else {
		ssTime << std::setw( 9 ) << inTime.fNanosecond;
	}
	if ( inZone ) {
		UInt64	nZoneMinutes( _abs64( inZone ) / CTimeUnits::GetConversion( kMinutes ) );

		UInt64	nHours( nZoneMinutes / 60 );
		UInt64	nMinutes( nZoneMinutes % 60 );
		ssTime << ((inZone > 0) ? L"-" : L"+")	// Reverse the sign; UTC = local + bias
			<< std::setw( 2 ) << nHours
			<< L":"
			<< std::setw( 2 ) << nMinutes;
	...
	tTimeUnits			inFormat )
{
	...
	if ( inZone ) {
		UInt64	nHours( _abs64( inZone ) / 100 );
		UInt64	nMinutes( _abs64( inZone ) % 100 );
		ssTime << ((inZone < 0) ? "-" : "+")
			<< std::setw( 2 ) << nHours
			<< ":"
			<< std::setw( 2 ) << nMinutes;
...
void
CPeekTime::GetLocalTime(
	SYSTEMTIME&	outTime )
{
	FILETIME	ftTime = m_ltTime;		// convert to FILETIME.

	// Convert the UTC file time to Local file time.
	FILETIME	ftLocal;
	::FileTimeToLocalFileTime( &ftTime, &ftLocal );

	::FileTimeToSystemTime( &ftLocal, &outTime );
}
	...
	FILETIME&	outTime )
{
	...
	FILETIME	ftLocal;
	::FileTimeToLocalFileTime( &ftTime, &ftLocal );
	tLongTime	ltLocal( ftLocal );

	tLongTime	ltDate( (ltLocal.i / kFileTimeUnitsInADay) * kFileTimeUnitsInADay );
	tLongTime	ltTime( ltLocal.i % kFileTimeUnitsInADay );
	outDate = ltDate;
	outTime = ltTime;
}
