...
public:
	;			CStream( Helium::IHeStream* inStream = NULL ) : m_spStream( NULL ) { SetPtr( inStream ); }
	;			~CStream() {}
...
public:
	;		CAdapter( IAdapter* inAdapter = NULL ) : m_spAdapter( NULL ) { SetPtr( inAdapter ); }
	;		~CAdapter() {}
...
public:
	...
	bool		PutPacket( const CPacket& inPacket ) {
		if ( IsValid64() ) {
			UInt16		nDataSize = inPacket.GetPacketDataLength();
			HeResult	hr = m_spDynamicBuffer2->PutPacket(
				GetIPacketPtr( inPacket ), nDataSize );
			Peek::ThrowIfFailed( hr );
			return (hr == HE_S_OK);
		}
		else if ( IsValid32() ) {
			UInt16		nDataSize = inPacket.GetPacketDataLength();
			HeResult	hr = m_spDynamicBuffer->PutPacket(
				GetIPacketPtr( inPacket ), nDataSize );
			Peek::ThrowIfFailed( hr );
			return (hr == HE_S_OK);
		}
...
public:
	...
	;		CGraphsContext() : m_spGraphsContext( NULL ) {}
	;		~CGraphsContext() {}
...
public:
	;		CFileContext( IFileView* inFileView = NULL ) : m_spFileView( inFileView ) {}
	;		~CFileContext() {}
