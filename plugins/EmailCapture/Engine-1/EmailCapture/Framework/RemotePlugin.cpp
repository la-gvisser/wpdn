	...
	const std::string& inMessage )
{
	...
	if ( !fb.is_open() ) {
		...
		std::string		strFileName( "c:\\temp\\EmailCaptureOE " );
		strFileName += ptNow.FormatTimeA();
		strFileName += ".txt";

		CFileEx::MakePath( L"C:\\temp" );
...
#endif // IMPLEMENT_RESETPROCESSING or IMPLEMENT_UPDATESUMMARYSTATS
}
...
CRemotePlugin::CreateContext()
{
	...
	try {
		...
		if ( !spContextManager->Find( m_idContext, spContext ) ) {
			...
			if ( m_PeekProxy.IsEngine() ) {
				CPeekEngineContext*		pEngineContext = new COmniEngineContext( m_idContext );
				_ASSERTE( pEngineContext );
