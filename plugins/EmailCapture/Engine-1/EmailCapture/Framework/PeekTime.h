...
static const UInt64	kMillisecondsInASecond = 1000ull;
static const UInt64	kMicrosecondsInASecond = 1000000ull;
static const UInt64	kNanosecondsInASecond  = 1000000000ull;
...
public:
	static CPeekString	Format( const tTimeFields& inTime, SInt64 inZone, tTimeUnits inFormat );
	...
	static CPeekStringA	FormatA( const tTimeFields& inTime, SInt64 inZone, tTimeUnits inFormat );
...
public:
	...
	explicit	CPeekTime( const SYSTEMTIME& inTime ) {
		FILETIME	ftTime;
		::SystemTimeToFileTime( &inTime, &ftTime );
		m_ltTime = ftTime;
	}
	...
	CPeekString		Format( tTimeUnits inFormat = kMilliseconds ) const {
		return Format( m_ltTime, 0, inFormat );
	}
	...
	CPeekStringA	FormatA( tTimeUnits inFormat = kMilliseconds ) const {
		return FormatA( m_ltTime, 0, inFormat );
	}
	...
	void	Round( tTimeUnits inType ) {
		m_ltTime.i = CTimeUnits::Round( m_ltTime.i, inType );
	}

	void	Set( tTimeUnits inType, UInt64 inCount ) {
		m_ltTime.i = CTimeUnits::Duration( inType, inCount );
	}
	void	SetLocalTime( SYSTEMTIME& inTime );
	void	SetLocalDateTime( FILETIME inDate, FILETIME inTime );
...
public:
	...
	;		CPeekTimeLocal( const CPeekTime& inTime )
		: CPeekTime( ToLocal( static_cast<tLongTime>( inTime ) ) )
	{}

	CPeekString		Format( tTimeUnits inFormat = kMilliseconds ) const {
		return CPeekTime::Format( m_ltTime, CPeekTime::GetLocalTimeOffset(), inFormat );
	}
	CPeekString		FormatDate() const {
		return CPeekTime::FormatDate( static_cast<FILETIME>( *this ) );
	}
	CPeekString		FormatTime( tTimeUnits inFormat = kMilliseconds ) const {
		return CPeekTime::FormatTime( m_ltTime, inFormat );
	}
	CPeekStringA	FormatA( tTimeUnits inFormat = kMilliseconds ) const {
		return CPeekTime::FormatA( m_ltTime, 0, inFormat );
	}
	CPeekStringA	FormatDateA() const {
		return CPeekTime::FormatDateA( static_cast<FILETIME>( *this ) );
	}
	CPeekStringA	FormatTimeA( tTimeUnits inFormat = kMilliseconds ) const {
		return CPeekTime::FormatTimeA( m_ltTime, inFormat );
	}
