...
public:
	...
	bool PulseEvent() { _ASSERTE(m_hEvent != NULL); return ( ::PulseEvent(m_hEvent) == TRUE ); }
	bool ResetEvent() { _ASSERTE(m_hEvent != NULL); return ( ::ResetEvent(m_hEvent) == TRUE ); }
	bool SetEvent()	  { _ASSERTE(m_hEvent != NULL); return ( ::SetEvent(m_hEvent) == TRUE ); }
};
#endif
...
public:
	CPeekMutex( bool bInitiallyOwn = false, PCTSTR lpszName = NULL,
