...
//	set to NULL. Performance testing has shown this algorithm to be
...
//	Example:
//		const CPeekString*	ayStrings[] = { &strA, &strB, &strC, NULL };
//		CPeekString			str = FastCat( ayStrings );
//
//	CFastCat encapsulates vector<CPeekString*> and FastCat.
//
...
public:
	...
	bool		Find( size_t& outPos, const char inTarget, size_t inOffset = 0 ) const {
		std::string::size_type	nIndex = find( inTarget, inOffset );
		if ( nIndex == std::string::npos ) return false;
		outPos = nIndex;
		return true;
	}
	int			Find( const char* inTarget, size_t inOffset = 0 ) const {
...
void
IntegerToString( SInt32 inValue, CPeekString& outValue );

void
IntegerToString( SInt64 inValue, CPeekString& outValue );

void
IntegerToString( UInt32 inValue, CPeekString& outValue );

void
IntegerToString( UInt64 inValue, CPeekString& outValue );

// Visual Studio 2008 in 64-bit build will cast size_t to a UInt64,
// but in 32-bit build will NOT cast a size_t to a UInt32.
#ifndef WIN64
void
IntegerToString( size_t inValue, CPeekString& outValue );
#endif

void
IntegerToString( SInt32 inValue, CPeekStringA& outValue );

void
IntegerToString( SInt64 inValue, CPeekStringA& outValue );

void
IntegerToString( UInt32 inValue, CPeekStringA& outValue );

void
IntegerToString( UInt64 inValue, CPeekStringA& outValue );

#ifndef WIN64
void
IntegerToStringA( size_t inValue, CPeekStringA& outValue );
#endif
...
void
StringToInteger( const CPeekString& inValue, SInt32& outValue );

void
StringToInteger( const CPeekString& inValue, SInt64& outValue );

void
StringToInteger( const CPeekString& inValue, UInt32& outValue );

void
StringToInteger( const CPeekString& inValue, UInt64& outValue );

#ifndef WIN64
void
StringToInteger( const CPeekString& inValue, size_t& outValue );
#endif

void
StringToInteger( const CPeekStringA& inValue, SInt32& outValue );

void
StringToInteger( const CPeekStringA& inValue, SInt64& outValue );

void
StringToInteger( const CPeekStringA& inValue, UInt32& outValue );

void
StringToInteger( const CPeekStringA& inValue, UInt64& outValue );

#ifndef WIN64
void
StringToIntegerA( const CPeekStringA& inValue, size_t& outValue );
#endif
...
public:
	...
	const CPeekStringA*	RefA() const { return &m_strA; }
	const CPeekString*	RefW() const { return &m_strW; }
...
class CFastCat
	:	public std::vector<const CPeekString*>
{
public:
	;		CFastCat( size_t nReserve = 0 )
	{
		reserve( nReserve );
	}

	void	Add( const CPeekString* inString ) {
		push_back( inString );
	}

	CPeekString	FastCat() const {
		return ::FastCat( *this );
	}
...
class CFastCatA
	:	public std::vector<const CPeekStringA*>
{
public:
	;		CFastCatA( size_t nReserve = 0 )
	{
		reserve( nReserve );
	}

	void	Add( const CPeekStringA* inString ) {
		push_back( inString );
	}

	CPeekStringA	FastCat() const {
		return ::FastCat( *this );
	}
