	...
	CWinFile&	inFile )
{
	...
	DWORD			dwBytesRead = inFile.Read(  sizeof( hdrBlock ), &hdrBlock );
	if ( dwBytesRead >= sizeof( hdrBlock ) ) {
	...
	CWinFile&	inFile )
{
	...
	DWORD	dwBytesRead = inFile.Read( nLength, &m_bvData[0] );
	return (dwBytesRead >= nLength );
}
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		switch ( ayAttribs[i].GetId() ) {
			case CBlockNameList::Version_FileVersion:
				StringToInteger( ayAttribs[i].GetValue(), m_nVersion );
				break;
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		...
		switch ( ayAttribs[i].GetId() ) {
			case CBlockNameList::Session_RawTime:
				StringToInteger( ayAttribs[i].GetValue(), nValue64 );
				m_RawTime = nValue64;
				break;
			...
			case CBlockNameList::Session_TimeZoneBias:
				StringToInteger( ayAttribs[i].GetValue(), m_TimeZoneBias );
				break;
			case CBlockNameList::Session_MediaType:
				StringToInteger( ayAttribs[i].GetValue(), nValue32 );
				m_MediaType.fType = static_cast<TMediaType>( nValue32 );
				break;
			case CBlockNameList::Session_MediaSubType:
				StringToInteger( ayAttribs[i].GetValue(), nValue32 );
				m_MediaType.fSubType = static_cast<TMediaSubType>( nValue32 );
				break;
			case CBlockNameList::Session_LinkSpeed:
				StringToInteger( ayAttribs[i].GetValue(), m_LinkSpeed );
				break;
			case CBlockNameList::Session_PacketCount:
				StringToInteger( ayAttribs[i].GetValue(), m_PacketCount );
				break;
			...
			case CBlockNameList::Session_FileIndex:
				StringToInteger( ayAttribs[i].GetValue(), m_FileIndex );
				break;
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		...
		switch ( ayAttribs[i].GetId() ) {
			case CBlockNameList::Session_Channel_Number:
				StringToInteger( ayAttribs[i].GetValue(), outChannel.fNumber );
				break;
			case CBlockNameList::Session_Channel_Frequency:
				StringToInteger( ayAttribs[i].GetValue(), outChannel.fFrequency );
				break;
			case CBlockNameList::Session_Channel_Band:
				StringToInteger( ayAttribs[i].GetValue(), outChannel.fBand );
				break;
			default:
				_ASSERTE( ayAttribs[i].GetId() != ayAttribs[i].GetId() );
				break;
	...
	const string&	inRates )
{
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		if ( ayAttribs[i].GetId() == nRateId ) {
			UInt32	nRate;
			StringToInteger( ayAttribs[i].GetValue(), nRate );
			m_ayRates.push_back( nRate );
		}
	}
}
	...
	for ( size_t i = 0; i < nCount; i++ ) {
		switch ( ayAttribs[i].GetId() ) {
			...
			case CBlockNameList::FileId_Index:
				StringToInteger( ayAttribs[i].GetValue(), m_nIndex );
				break;
			default:
				_ASSERTE( ayAttribs[i].GetId() != ayAttribs[i].GetId() );
				break;
	...
	bool	inScanPackets /*= false*/ )
{
	...
	while ( blk.Read( GetFile() ) ) {
		...
		nFileOffset = GetFile().GetPosition();
	}
	...
	if ( nPacketCount == 0 ) {
		m_ayOffsets.SetFirstOffset( GetFile().GetSize(), 1, nOffset );
	}
	else {
		m_ayOffsets.SetFirstOffset( GetFile().GetSize(), nPacketCount, nOffset );
	}
	...
	UInt32			inIndex )
{
	...
	const UInt64	nOffset = m_ayOffsets[inIndex];
	...
	UInt32	nRead = GetFile().Read( nAttribSize, ayAttribData );
	...
	if ( ayAttribs.empty() ) return false;

	UInt64	nDataOffset = nOffset + (ayAttribs.size() * sizeof( PacketAttribute ));
	GetFile().SeekTo( nDataOffset );
	...
	UInt32				inFirst /*= NEXT_PACKET*/)
{
	...
	try {
		...
		for ( UInt32 nIndex = inFirst; nIndex < nEndIndex; ++nIndex ) {
			...
			if ( !ReadFilePacket( pkt, nIndex ) ) break;
	...
	UInt32	inIndex )
{
	...
	while ( nIndex <= inIndex ) {
		...
		UInt32			nRead = GetFile().Read( nCount, ayPacketData );
		if ( nRead < 6 ) return false;

		size_t				nPacketLength = 0;
		PacketAttribute*	pAttrib = reinterpret_cast<PacketAttribute*>( ayPacketData );
		for ( int a = 0; a < nAttribMax; a++ ) {
...
{
	if ( IsOpen() ) {
		if ( (m_SessionBlock.GetOffset() != -1) && (GetFile().GetSize() > m_PacketsBlock.GetOffset()) ) {
			GetFile().SeekTo( m_SessionBlock.GetOffset() );
			m_SessionBlock.Write( GetFile() );
		}
	...
	const CCaptureFile*	inTemplate )
{
	...
	m_VersionBlock.SetOffset( GetFile().GetPosition() );
	m_VersionBlock.Write( GetFile() );

	m_SessionBlock.SetOffset( GetFile().GetPosition() );
	m_SessionBlock.Write( GetFile() );

	m_FileIdBlock;
	m_GPSBlock;
	
	m_PacketsBlock.SetOffset( GetFile().GetPosition() );
	m_PacketsBlock.Write( GetFile() );
}
	...
	const CFilePacket&	inPacket )
{
	...
	for ( size_t i = 0; i < nAttribCount; i++ ) {
		...
		GetFile().Write( sizeof( PacketAttribute ), &pa );
	}

	GetFile().Write( static_cast<UInt32>( nPacketDataLength ), inPacket.GetPacketData() );
