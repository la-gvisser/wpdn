...
public:
	...
	class CStreamFileGroup
		:	public CAtlArray<CStreamFileItem>
	{
	public:
		;		CStreamFileGroup() {}
		void	DeleteOpen() {
			for ( size_t i = 0; i < GetCount(); i++ ) {
				CStreamFileItem&	item( GetAt( i ) );
		...
		bool Exists() const {
			for ( size_t i = 0; i < GetCount(); i++ ) {
				if ( ::PathFileExists( GetAt( i ).GetName() ) ) {
					return true;
				}
			}
			return false;
...
protected:
	...
	public:
		...
		;		~CMap() { Reset(); }
		void	Reset() {
			std::map<CIpAddressPortPair, S*>::iterator	item( begin() );
			std::map<CIpAddressPortPair, S*>::iterator	last( end() );
			while ( item != last ) {
				delete item->second;
				item->second = NULL;
				++item;
	...
	const CIpAddressPortPair&	inIpPortPair )
{
	...
	std::map<CIpAddressPortPair, S*>::iterator	pItem = m_Map.find( inIpPortPair );
	if ( pItem != m_Map.end() ) {
		return pItem->second;
	}
...
{
	typedef std::map<CIpAddressPortPair, S*>::iterator	CMapIter;

	std::vector<CIpAddressPortPair>	ayMorgue;

	CPeekTime		ptCurTime( true );
	for ( CMapIter pos = m_Map.begin(); pos != m_Map.end(); pos++ ) {
		...
		if ( bRemove ) {
			...
			outRemovedStreams.Add( CPeekString( strRemoved ) );
