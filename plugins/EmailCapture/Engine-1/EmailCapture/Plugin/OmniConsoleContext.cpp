...
#undef SubclassWindow							// The MSVC compiler picks up this macro
			...
			CContextTab*	pTab = new CContextTab( this, *(GetOptions()) );
			_ASSERTE( pTab != NULL );
			if ( pTab != NULL ) {
				pTab->Create( IDD_CONTEXT_TAB, &m_TabHost );
				m_TabHost.SetChild( pTab );
				CRect	rc;
				m_TabHost.GetClientRect( &rc );
				pTab->MoveWindow( 0, 0, rc.Width(), rc.Height() );
			}
		}
	}
...
{
#ifdef PEEK_UI
	COptionsDialog theOptionsDlg( *(GetOptions()) );
	if ( theOptionsDlg.DoModal() == IDOK ) {
		COptionsPtr	pOptions = GetOptions();
		pOptions->Copy( theOptionsDlg.GetOptions() );
		return true;
	}
#endif
