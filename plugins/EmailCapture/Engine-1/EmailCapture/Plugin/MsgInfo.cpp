	...
	CArrayString&	outAddresses )
{
	...
	outAddresses.Add( strAddressList );
}
...
{
	size_t			nFromCount = m_MailFromList.GetCount();
	size_t			nToCount = m_RcptToList.GetCount();
	...
	if ( !strFromList.IsEmpty() ) {
		cfHeaders.Add( &strFromList );
		cfHeaders.Add( Tags::kxNewLine.RefA() );
	}

	// To:
	CPeekStringA	strToList( m_RcptToList.FormatEmlA( CMsgAddress::kType_To ) );
	if ( !strToList.IsEmpty() ) {
		cfHeaders.Add( &strToList );
		cfHeaders.Add( Tags::kxNewLine.RefA() );
	}
	...
	const CTargetAddress&	inTarget )
{
	...
	const size_t	nToCount = m_RcptToList.GetCount();
	const size_t	nFromCount = m_MailFromList.GetCount();
	...
	if ( inTarget.GetType() == CTargetAddress::kTypeAddress ) {
		...
		const CMsgAddress&	RcptTo = m_RcptToList.GetAt( nRecipient );
		...
		for ( size_t i = 0; i < nToCount; i++ ) {
			const CMsgAddress&	RcptTo = m_RcptToList.GetAt( i );
