...
{
	const size_t	nCount( m_MsgPacketList.GetCount() );
	size_t			nMaxLength( 0 );
	...
	CStreamFileGroup&	ioFiles )
{
	...
	for ( int x = 0; x < 100; x++ ) {
		...
		UINT			nFlags( CFileEx::modeNew | CFileEx::modeWrite );
		const size_t	nCount = ioFiles.GetCount();
		for ( size_t i = 0; i < nCount; i++ ) {
			CStreamFileItem&	item( ioFiles.GetAt( i ) );
		...
		if ( !ioFiles.Exists() ) {
			BOOL	bResult( TRUE );
			size_t	nCount = ioFiles.GetCount();
			for ( size_t i = 0; i < nCount; i++ ) {
				CStreamFileItem&	item( ioFiles.GetAt( i ) );
...
{
	if ( m_PenInfoPtr == NULL ) {
		m_PenInfoPtr.Attach( new CPenInfo( this ) );
		ASSERT( m_PenInfoPtr != NULL );
		if ( m_PenInfoPtr == NULL ) throw( 1 );
	}
	return m_PenInfoPtr;
}
	...
	CMsgPacketPtr	inMsgPacket )
{
	...
	size_t	nIndex( m_MsgPacketList.Add( inMsgPacket ) );
	return m_MsgPacketList[nIndex].m_p;
}
...
{
	bool	bResult = true;
	size_t	nPacketCount( m_MsgPacketList.GetCount() );
	...
	try {
		...
		while ( CaptureBuffer.GetCount() < nPacketCount ) {
			for ( size_t i = 0; i < nPacketCount; i++ ) {
				...
				if ( !CaptureBuffer.WritePacket( pMsgPacket->GetPacket() ) ) {
					// Only one retry.
					if ( fRetried ) {
