	...
	const CPeekString&	inStr ) const
{
	for ( size_t i = 0; i < GetCount(); i++ ) {
		const CTargetAddress&	Address( GetAt( i ) );
...
CTargetAddressList::FormatLongA() const
{
	...
	_ASSERTE( fcAddress.GetCount() <= nLineCount );
...
CTargetAddressList::FormatCount() const
{
	...
	IntegerToString( nCount, strCount );

	CPeekString		strAddressCount;
	IntegerToString( nAddressCount, strAddressCount );
...
CTargetAddressList::GetNext(
	...
	size_t&			ioPos ) const
{
	if ( ioPos > GetCount() ) return false;

	for ( size_t i = (ioPos + 1); i < GetCount(); i++ ) {
		const CTargetAddress&	Address( GetAt( i ) );
		if ( Address.IsEnabled() ) {
			if ( i < kMaxUInt32 ) {
				outItem = Address;
	...
	CPeekDataElement&	ioAddressList )
{
	...
	for ( UInt32 i = 0; i < nCount; i++ ) {
		...
		if ( elemAddress.IsValid() ) {
			...
			CTargetAddress&	cTarget = GetAt( i );
