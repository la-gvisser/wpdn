...
typedef std::vector< Helium::HeID >				HeIDVector;
typedef std::map< Helium::HeID, std::wstring >	NamedIDMap;
...
public:
	...
	/// Compile the filters with filter mode.
	HE_IMETHOD Compile(/*in*/ PeekFilterMode inMode) = 0;
	/// Get the media type.
	...
	/// Compile the filters with filter mode, return IDs of missing filters, caller supplies the HeIDVector.
	HE_IMETHOD Compile(/*in*/ PeekFilterMode inMode, /*out, optional*/ HeIDVector* pMissingIDs) = 0;
};
