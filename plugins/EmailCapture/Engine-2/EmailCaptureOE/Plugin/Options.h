...
public:
	...
	public:
		...
		const CCase&		GetAt( size_t inIndex ) const { return m_CaseList.at( inIndex ); }
		size_t				GetCount() const { return m_CaseList.size(); }
		...
		bool	IsEmpty() const { return m_CaseList.empty(); }
		...
		void	RemoveAt( int inIndex ) { m_CaseList.erase( m_CaseList.begin() + inIndex ); }
		void	Reset() { m_CaseList.clear(); }
