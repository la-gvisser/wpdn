...
CReportMode::FormatLongA() const
{
	...
	aStrs.push_back( static_cast<const char *>( FormatMode() ) );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsPenEmail()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsPenXml()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsPenText()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsPenPerAddress()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsFullEmail()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsFullPacket()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsSaveAge()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	CPeekOutStringA	ssAge;
	ssAge << GetSaveAgeValue();
	aStrs.push_back( ssAge );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( (IsSaveSize()) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	CPeekOutStringA	ssSize;
	ssSize << GetSaveSizeValue();
	aStrs.push_back( ssSize );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );
...
#if (TOVERIFY)
			const CPeekStringA*	ayHeaderStrs[] = {
				&inPreAmble,
				&Tags::kLogPenSaveFileA,
				Tags::kxNewLine.RefA(),
				NULL
			};
			inFile.Write( FastCat( ayHeaderStrs ) );
#else
	...
	CPeekDataElement&	ioCase )
{
	...
	if ( saLabels.empty() ) {
		saLabels.push_back( ConfigTags::kPen );
		saLabels.push_back( ConfigTags::kFull );
	}
	size_t	nIndex = m_Mode;
	ioCase.Attribute( ConfigTags::kCaptureMode, nIndex, saLabels );
	if ( ioCase.IsLoading() && (nIndex < saLabels.size()) ) {
...
{
	UInt32	nCount( /* std::_wtol( inCount ) */ 0 );
	UInt32	nSeconds( 0 );
