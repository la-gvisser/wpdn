	...
	case CMsgAddress::kType_From:
		{
			...
			if ( !strMailFrom.IsEmpty() ) {
				push_back( strMailFrom );
				CMsgAddress	&maItem( at( size() ) );
	...
	case CMsgAddress::kType_BCC:
		{
			...
			if ( inMsgHdr.GetRecipientTo( strsRecipientList ) ) {
				const size_t	nCount( strsRecipientList.size() );
				for ( size_t i( 0 ); i < nCount; i++ ) {
					push_back( strsRecipientList[i] );
					CMsgAddress	&maItem( at( size() ) );
					maItem.SetExplicit();
				}
			}
	...
	CMsgAddress::MsgAddressType	inType )
{
	...
	if ( nAddressCount > 0 ) {
		...
		for ( size_t i( 0 ); i < nAddressCount; i++ ) {
			CMsgAddress&	maItem( at( ayIndexes[i] ) );
	...
	size_t&					outIndex ) const
{
	const size_t	nCount( size() );
	for ( size_t i( 0 ); i < nCount; i++ ) {
		if ( inItem.IsMatch( at( i ).GetAddress() ) ) {
			outIndex = i;
			return true;
		}
	}
	...
	const CTargetAddress&	inItem ) const
{
	const size_t	nCount( size() );
	for ( size_t i( 0 ); i < nCount; i++ ) {
		if ( inItem.IsMatch( at( i ).GetAddress() ) ) return true;
	}
	return false;
}
