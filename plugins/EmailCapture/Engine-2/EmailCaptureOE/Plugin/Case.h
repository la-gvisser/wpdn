...
public:
	...
	const CPeekStringX	GetFileNamePrefixAppend() const {
		CPeekString strResult( m_strFileNamePrefix.GetW() );
		if ( IsAppendEmail() ) {
			strResult += L"<email>_";
		}
...
public:
	;			CCaseList() {}
	;			CCaseList( const CCaseList& in ) { assign( in.begin(), in.end() ); }
	...
	int		GetCount32() const { return (size() < kIndex_Invalid) ? (size() & 0x0FFFFFFFF) : 0; }
