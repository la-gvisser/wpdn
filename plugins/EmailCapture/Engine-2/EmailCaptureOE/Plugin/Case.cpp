...
CCaseSettings::FormatA() const
{
	...
	aStrs.push_back( Tags::kAcceptPackets );
	aStrs.push_back( (m_bAcceptPackets) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kWritePackets );
	aStrs.push_back( (m_bWritePackets) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kSummaryStats );
	aStrs.push_back( (m_bSummaryStats) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kLogMessages );
	aStrs.push_back( (m_bLogMessages) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kOutputDirectory );
	aStrs.push_back( static_cast<PCSTR>( m_strOutputDirectory ) );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kLogMessages );
	aStrs.push_back( (m_bLongName) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kOutputDirectory );
	aStrs.push_back( Tags::kProtocols );
	if ( m_Protocols[kProtocol_SMTP] ) aStrs.push_back( Tags::kSMTP );
	if ( m_Protocols[kProtocol_POP3] ) aStrs.push_back( Tags::kPOP3 );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	CPeekStringA	strStart( FormatStartTimeA() );
	aStrs.push_back( Tags::kStartTime );
	aStrs.push_back( strStart );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );
	
	CPeekStringA	strStop( FormatStopTimeA() );
	aStrs.push_back( Tags::kStopTime );
	aStrs.push_back( strStop );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );
...
CCaseSettings::FormatLongA() const
{
	...
	if ( m_Protocols[kProtocol_SMTP] ) aStrs.push_back( Tags::kxSmtp.GetA() );
	if ( m_Protocols[kProtocol_POP3] ) aStrs.push_back( Tags::kxPop3.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( m_Mode.FormatLongA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( FormatStartTimeA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );
	
	aStrs.push_back( FormatStopTimeA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );
...
CCase::FormatA() const
{
	...
	aStrs.push_back( m_strName );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kTargetIs );
	aStrs.push_back( (m_bEnabled) ? Tags::kxEnabled.GetA() : Tags::kxDisabled.GetA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( Tags::kAddressList );
	aStrs.push_back( m_AddressList.FormatLongA() );
	aStrs.push_back( Tags::kxCommaSpace.GetA() );

	aStrs.push_back( m_Settings.FormatA() );
...
CCase::FormatLongA() const
{
	...
	aStrs.push_back( m_Settings.FormatLongA() );
	...
	const CCase& inCase ) const
{
	...
	if ( GetName().GetW().CompareNoCase( inCase.GetName() ) != 0 ) return false;
	...
	CFileEx&			inFile ) const
{
	try {
		...
		if ( inCase.m_strName.CompareNoCase( static_cast<CPeekStringA>( m_strName.GetW() ) ) != 0 ) {
#if (TOVERIFY)
			CPeekOutStringA	ss;
			ss <<
				inPreAmble <<
	...
	CFileEx&			inFile ) const
{
	...
	CBoolResultTrue	bResult;
	try {
		std::vector<ciCase>		ayCasesToRemove;
		const size_t			nCaseCount( inCaseList.size() );
		std::vector<CBool>		ayUpdatedCases( nCaseCount );
		...
		typedef std::vector<ciCase>::reverse_iterator	ri;
