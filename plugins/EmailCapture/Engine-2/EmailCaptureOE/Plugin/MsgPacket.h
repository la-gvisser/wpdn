...
typedef std::unique_ptr<CMsgPacket>		CMsgPacketPtr;
...
class CMsgPacketList
	:	public std::vector< std::unique_ptr<CMsgPacket> >
{
public:
	;		CMsgPacketList() {}
	...
	UInt64	GetBufferSize( size_t inInitIndex = 0 ) const {
		UInt64	nSize = 4096;
		for ( size_t i = inInitIndex; i < size(); i++ ) {
			const CMsgPacket*	pMsgPacket = at( i ).get();
