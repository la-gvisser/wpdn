...
public:
	...
	CHeVariant& operator=( const wchar_t* pszSrc )
	{
		...
		if ( (bstrVal == nullptr) && (pszSrc != nullptr) )
		{
			vt    = HE_VT_ERROR;
			hrVal = HE_E_OUT_OF_MEMORY;
			HeThrow( HE_E_OUT_OF_MEMORY );
...
public:
	...
	{
		if ( pbstrDest == nullptr ) return HE_E_NULL_POINTER;
		if ( vt != Helium::HE_VT_BSTR ) return HE_E_FAIL;

		HE_ASSERT( (pbstrDest != nullptr) && (vt == Helium::HE_VT_BSTR) );

		*pbstrDest = HeLib::HeAllocStringByteLen(
			reinterpret_cast<char*>(bstrVal), HeStringByteLen( bstrVal ) );

		return (*pbstrDest != nullptr) ? HE_S_OK : HE_E_OUT_OF_MEMORY;
	}
	...
	HeResult Attach( Helium::HEVARIANT* pSrc)
	{
		...
		if ( pSrc == nullptr ) return HE_E_NULL_POINTER;
		...
		if ( HE_SUCCEEDED(hr) )
		{
			...
			std::memcpy( static_cast<Helium::HEVARIANT*>(this), pSrc, sizeof(HEVARIANT) );
			pSrc->vt = HE_VT_EMPTY;
			hr = HE_S_OK;
		}
		return hr;
	...
	HeResult Detach( Helium::HEVARIANT* pDest )
	{
		...
		if ( pDest == nullptr ) return HE_E_NULL_POINTER;
	...
	HeResult ChangeType( Helium::HEVARTYPE vtNew, const Helium::HEVARIANT* pSrc = nullptr )
	{
	...
	HeResult GetInterface( Q** pp)
	{
		...
		// Sanity checks.
		if ( this->punkVal == nullptr ) return HE_E_INVALID_ARG;
...
inline HeResult CHeVariant::WriteToStream( Helium::IHeStream* pStream ) const
{
	...
	HeResult	hr = pStream->Write( &vt, sizeof(HEVARTYPE), nullptr );
	if ( HE_FAILED(hr) ) return hr;
	...
	if ( cbWrite != 0 )
	{
		return pStream->Write( &bVal, static_cast<UInt32>(cbWrite), nullptr );
	}
...
inline HeResult CHeVariant::ReadFromStream( Helium::IHeStream* pStream )
{
	...
	if ( cbRead != 0 )
	{
		hr = pStream->Read( &bVal, cbRead, nullptr );
	...
	if ( HE_FAILED(hr) )
	{
		...
		pStream->Seek( nOffset, IHeStream::STREAM_SEEK_CUR, nullptr );
		return hr;
	}
	vt = HE_VT_BSTR;
...
inline UInt32 CHeVariant::GetSize() const
{
	...
	if ( nSize == sizeof(HEVARTYPE) )
	{
		...
		// Add the size of the length, the string itself and the nullptr character
