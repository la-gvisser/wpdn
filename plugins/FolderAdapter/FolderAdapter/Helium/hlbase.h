...
HeInternalQueryInterface(
	...
	void**							ppv )
{
	HE_ASSERT( pThis != nullptr );
	HE_ASSERT( pEntries != nullptr );
	HE_ASSERT( ppv != nullptr );
	if ( (pThis == nullptr) || (pEntries == nullptr) || (ppv == nullptr) )
	{
...
public:
	...
	explicit CHeObject(void* = nullptr) throw()
	{
		_pModule->Lock();
	}
	...
	{
		if ( ppv == nullptr ) return HE_E_NULL_POINTER;
...
public:
	...
	explicit CHeObjectSI(void* = nullptr) throw()
	{
		_pModule->Lock();
	}
...
public:
	...
	explicit CHeObjStack(void* = nullptr)
	{
...
{
public:
	explicit CHeObjLockT( const T* p ) : m_p( p )
	{
		if ( m_p != nullptr )
		{
			m_p->Lock();
...
{
public:
	explicit CHeObjUnlockT( const T* p ) : m_p( p )
	{
		if ( m_p != nullptr )
		{
			m_p->Unlock();
...
// For stack-based use only. Not thread-safe.
template <class T>
class CHeObjTryLockT
{
public:
	explicit CHeObjTryLockT( const T* p ) :
		m_p( p ), m_bLocked( false )
	{
	}

	~CHeObjTryLockT() throw()
	{
		this->Unlock();
	}
...
template <class T>
class CHeDelayedObjLockT
{
public:
	CHeDelayedObjLockT( const T* p ) :
	  m_p( p ), m_bLocked( false )
	{
	}

	~CHeDelayedObjLockT() throw()
	{
		this->Unlock();
	}
...
public:
	...
	typedef CHeObjTryLockT< CHeObjRoot<_ThreadModel> > ObjectTryLock;
...
public:
	...
	bool TryLock() const { return m_cs.TryLock(); }
...
public:
	...
	typedef CHeObjTryLockT< CHeObjRootSI<_ThreadModel> > ObjectTryLock;
...
public:
	...
	CHeFactory() : m_pfnCreateInstance( nullptr ) {}
...
public:
	HE_IMETHOD CreateInstance(Helium::IHeUnknown* pOuter, const Helium::HeIID& iid, void** ppv)
	{
		...
		*ppv = nullptr;
		...
		if ( m_pfnCreateInstance != nullptr )
		{
...
public:
	...
	{
		return T::_CreatorClass::CreateInstance( nullptr, Q::GetIID(), reinterpret_cast<void**>(ppQ) );
	}
};
