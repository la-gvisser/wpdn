...
/// \brief GetLastError (Windows) or errno as HeResult
inline HeResult HeGetLastError()
{
#if defined(HE_WIN32) && defined(WINVER)	// Must have included <windows.h>
    return Helium::HeResultFromWin32( ::GetLastError() );
#else
    return Helium::HeResultFromErrorCode( errno );
#endif
}

/// \brief Throw last error
inline void HE_NORETURN HeThrowLastError()
{
	HeLib::HeThrow( HeGetLastError() );
}
