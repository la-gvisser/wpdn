...
//	omnicommands.h
// =============================================================================
//	Copyright (c) 2003-2013	WildPackets, Inc. All rights reserved.
...
/// \struct PluginMessageDataHdr
/// \brief Beginning of the plugin message data. It is used to determine the data
/// and binary lengths in the message data.
#include "hepushpack.h"
struct PluginMessageDataHdr
{
	UInt64	DataModelerLength;
	UInt64	BinaryDataLength;
} HE_PACK_STRUCT;
#include "hepoppack.h"
...
namespace OmniProtocolCommands
{
	...
	const UInt32 OMNI_GET_ADAPTER_INFO_LIST						= 113;
...
//	const UInt32 OMNI_EXPERT_PERFORM_QUERY						= 153;	// Never used.
	...
	const UInt32 OMNI_QUERY_STATS								= 182;
	...
	const UInt32 OMNI_SAVE_SECURITY								= 223;
	const UInt32 OMNI_SEND_NOTIFICATIONS						= 224;
	const UInt32 OMNI_LOG_MESSAGES								= 225;
	const UInt32 OMNI_SEND_EMAIL								= 226;
	const UInt32 OMNI_PRE_SAVE_SECURITY							= 227;
	const UInt32 OMNI_GET_PLUGIN_DATA_BLOCKS					= 228;
	const UInt32 OMNI_SET_GRAPH_TEMPLATES						= 229;
	const UInt32 OMNI_GET_ADAPTER_LIST							= 230;
	const UInt32 OMNI_GET_PROTOCOL_TRANSLATIONS					= 231;
	const UInt32 OMNI_SET_PROTOCOL_TRANSLATIONS					= 232;
	const UInt32 OMNI_GET_PROTOCOL_LIST							= 233;
	const UInt32 OMNI_LICENSING_GET_LOCKING_CODE				= 234;
	const UInt32 OMNI_LICENSING_GET_INFO						= 235;
	const UInt32 OMNI_LICENSING_SET_INFO						= 236;
	const UInt32 OMNI_GET_APPLICATION_LIST						= 237;
	const UInt32 OMNI_CONVERT_FILTER							= 238;
	const UInt32 OMNI_GET_COUNTRY_LIST							= 239;
	const UInt32 OMNI_MODIFY_FILE								= 240;
	const UInt32 OMNI_CREATE_FLOW_VISUALIZER					= 241;
	const UInt32 OMNI_DELETE_FLOW_VISUALIZER					= 242;
	const UInt32 OMNI_GET_FLOW_VISUALIZER						= 243;
	const UInt32 OMNI_RESOLVE_ADDRESSES							= 244;
	const UInt32 OMNI_RESOLVE_NAMES								= 245;
	const UInt32 OMNI_MODIFY_NOTIFICATIONS						= 246;
	const UInt32 OMNI_DELETE_ALL_CAPTURES						= 247;
	const UInt32 OMNI_DELETE_ALL_FILE_VIEWS						= 248;
	const UInt32 OMNI_DELETE_ALL_FILTERS						= 249;
	const UInt32 OMNI_GET_ENGINES								= 250;
	const UInt32 OMNI_SET_ENGINES								= 251;
	const UInt32 OMNI_DELETE_ENGINES							= 252;
	const UInt32 OMNI_GET_HARDWARE_CONFIG						= 253;
	const UInt32 OMNI_LICENSING_GET_SETTINGS					= 254;
	const UInt32 OMNI_LICENSING_SET_SETTINGS					= 255;
	const UInt32 OMNI_GET_DASHBOARDS							= 256;
	const UInt32 OMNI_SET_DASHBOARDS							= 257;
	const UInt32 OMNI_DELETE_DASHBOARDS							= 258;
	const UInt32 OMNI_GET_DASHBOARD_WIDGETS						= 259;
	const UInt32 OMNI_SET_DASHBOARD_WIDGETS						= 260;
	const UInt32 OMNI_DELETE_DASHBOARD_WIDGETS					= 261;
	const UInt32 OMNI_GET_DISTRIBUTED_FORENSIC_SEARCH_LIST		= 262;
	const UInt32 OMNI_CREATE_DISTRIBUTED_FORENSIC_SEARCH		= 263;
	const UInt32 OMNI_GET_DISTRIBUTED_FORENSIC_SEARCH			= 264;
	const UInt32 OMNI_DELETE_DISTRIBUTED_FORENSIC_SEARCH		= 265;
	const UInt32 OMNI_DELETE_ALL_DISTRIBUTED_FORENSIC_SEARCHES	= 266;
	const UInt32 OMNI_STOP_DISTRIBUTED_FORENSIC_SEARCH			= 267;
	const UInt32 OMNI_STOP_DISTRIBUTED_FORENSIC_SEARCH_SEARCH	= 268;
	const UInt32 OMNI_SET_PACKET_STATUS							= 269;
	const UInt32 OMNI_GET_DECRYPTION_KEYS						= 270;
	const UInt32 OMNI_SET_DECRYPTION_KEYS						= 271;
	const UInt32 OMNI_DELETE_DECRYPTION_KEYS					= 272;
	const UInt32 OMNI_SET_DECRYPTION_KEY_NAME					= 273;
	const UInt32 OMNI_GET_MSA_PROJECT_LIST						= 274;
	const UInt32 OMNI_CREATE_MSA_PROJECT						= 275;
	const UInt32 OMNI_GET_MSA_PROJECT							= 276;
	const UInt32 OMNI_DELETE_MSA_PROJECT						= 277;
	const UInt32 OMNI_STOP_MSA_PROJECT							= 278;
	const UInt32 OMNI_SET_MSA_PROJECT_OPTIONS					= 279;
	const UInt32 OMNI_SAVE_CAPTURE_FILE_TASK					= 280;
	const UInt32 OMNI_SELECT_WEB								= 281;
	const UInt32 OMNI_SELECT_WEB_TASK							= 282;
	const UInt32 OMNI_GET_RECONSTRUCTIONS_LIST					= 283;
	const UInt32 OMNI_GET_RECONSTRUCTIONS						= 284;
	const UInt32 OMNI_REFRESH_ADAPTER_LIST						= 285;
	const UInt32 OMNI_GET_FILE_VIEW_LIST_HISTORY				= 286;
	const UInt32 OMNI_DELETE_FILE_VIEW_LIST_HISTORY				= 287;
	const UInt32 OMNI_GET_FILE_VIEW_LIST_HISTORY_ITEM			= 288;
	const UInt32 OMNI_SET_FILE_VIEW_LIST_HISTORY_ITEM			= 289;
	const UInt32 OMNI_DELETE_FILE_VIEW_LIST_HISTORY_ITEM		= 290;
...
//	const UInt32 OMNI_ADMCMD_GET_COMMAND_LIST					= 522;		// Obsolete, use OMNI_GET_COMMAND_LIST
	...
	const UInt32 OMNI_CREATE_FILE								= 524;
	const UInt32 OMNI_DMS_RESTART								= 525;
	const UInt32 OMNI_GET_SNMP_SETTINGS							= 526;
	const UInt32 OMNI_TEST_ACTIVE_DIRECTORY						= 527;
	const UInt32 OMNI_GET_DMS_CONFIG							= 528;
	const UInt32 OMNI_SET_DMS_CONFIG							= 529;
	const UInt32 OMNI_SET_SNMP_SETTINGS							= 530;

	const UInt32 OMNI_SET_DEF_ACCOUNT							= 550;		// Used only with OWS?

	const UInt32 OMNI_LOG_IN									= 600;
	const UInt32 OMNI_LOG_OUT									= 601;
	const UInt32 OMNI_GET_AUTH_TOKEN_LIST						= 602;
	const UInt32 OMNI_CREATE_AUTH_TOKEN							= 603;
	const UInt32 OMNI_MODIFY_AUTH_TOKEN							= 604;
	const UInt32 OMNI_GET_AUTH_TOKEN							= 605;
	const UInt32 OMNI_DELETE_AUTH_TOKEN							= 606;
	...
	const UInt32 OMNI_SET_LOG_REDIRECT							= 705;
	...
	const UInt32 OMNI_GET_SUPPORT_INFO							= 721;
	const UInt32 OMNI_DATABASE_MAINTENANCE						= 722;
	...
	const UInt32 OMNI_GET_LIVEFLOW_CONFIGURATION				= 1100;
	const UInt32 OMNI_SET_LIVEFLOW_CONFIGURATION				= 1101;
	const UInt32 OMNI_GET_LIVEFLOW_CONTEXT						= 1102;
	const UInt32 OMNI_GET_LIVEFLOW_STATUS						= 1103;
	const UInt32 OMNI_GET_THREATEYENV_CONFIGURATION				= 1104;
	const UInt32 OMNI_SET_THREATEYENV_CONFIGURATION				= 1105;
