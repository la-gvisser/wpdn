...
public:
	...
	/// notification. Usually a capture. May be HE_NULL_ID.
...
/// \internal
/// \interface INotifyCheck
/// \ingroup Notification
/// \brief Check whether notification is enabled.
#define INotifyCheck_IID \
{ 0xFE898860, 0xE990, 0x452E, {0x80, 0x3F, 0x2E, 0x94, 0xF7, 0xF5, 0x5A, 0x91} }

class HE_NO_VTABLE INotifyCheck : public Helium::IHeUnknown
{
public:
	HE_DEFINE_STATIC_IID_ACCESSOR(INotifyCheck_IID)

	/// Check whether a notification is enabled.
	/// \param context ID associated with the component that is sending this
	/// notification. Usually a capture. May be HE_NULL_ID. Not used.
	/// \param source ID associated with the object sending this notification.
	/// \param nSourceKey Key that can be used by the source. Not used.
	/// \param severity Severity of the notification.
	/// \param actionClass Class ID of the action type to be checked. May be HE_NULL_ID.
	/// \param pbEnabled Returns whether the notification will be sent.
	HE_IMETHOD NotifyCheck(/*in*/ const Helium::HeID& context,
		/*in*/ const Helium::HeID& source, /*in*/ UInt32 nSourceKey,
		/*in*/ PeekSeverity severity, /*in*/ const Helium::HeCID& actionClass,
...
/// \enum EmailScheme
/// \brief Protocol scheme for email actions.
enum EmailScheme
{
	emailSchemeSmtp,
	emailSchemeSmtpSSL,
...
/// \brief Configure parameters for an email message.
#define IEmailMessage_IID \
{ 0x14235DC4, 0x4A47, 0x4461, {0xBC, 0x30, 0x97, 0x2A, 0xCF, 0x85, 0x72, 0x2B} }

class HE_NO_VTABLE IEmailMessage : public Helium::IHeUnknown
{
public:
	HE_DEFINE_STATIC_IID_ACCESSOR(IEmailMessage_IID)

	/// Set the email server.
	/// \param pszServer The email server IP address or DNS name.
	HE_IMETHOD SetServer(/*in*/ const char* pszServer) = 0;
	/// Set the email server.
	/// \param nPort The email server port.
	HE_IMETHOD SetPort(/*in*/ UInt16 nPort) = 0;
	/// Set the email scheme (SMTP, SMTP-SSL, SMTP-TLS).
	/// \param scheme The email scheme.
	/// \see EmailScheme
	HE_IMETHOD SetScheme(/*in*/ EmailScheme scheme) = 0;
	/// Set whether server authentication is enabled.
	/// \param scheme TRUE if authentication is enabled.
	HE_IMETHOD SetUseAuthentication(/*in*/ BOOL bUseAuthentication) = 0;
	/// Set the username for the sender.
	/// \param pszUsername The username for the sender.
	HE_IMETHOD SetUsername(/*in*/ const char* pszUsername) = 0;
	/// Set the username for the sender.
	/// \param pszPassword The password for the sender.
	HE_IMETHOD SetPassword(/*in*/ const char* pszPassword) = 0;
	/// Set the email sender.
	/// \param pszSender The sender of the email (From:).
	HE_IMETHOD SetSender(/*in*/ const char* pszSender) = 0;
	/// Set the email recipients (To:).
	/// \param pszRecipients The sender of the email (From:).
	HE_IMETHOD SetRecipients(/*in*/ const char* pszRecipients) = 0;
	/// Set the email subject (Subject:).
	/// \param pszSubject The subject of the email.
	HE_IMETHOD SetSubject(/*in*/ const char* pszSubject) = 0;
	/// Set the email body.
	/// \param pszBody The contents of the email.
	HE_IMETHOD SetBody(/*in*/ const char* pszBody) = 0;
};

/// \internal
/// \interface ISendEmailMessage
/// \brief Send an email message.
#define ISendEmailMessage_IID \
{ 0x8A991F71, 0xD2E1, 0x476D, {0xB5, 0xF8, 0x2E, 0x86, 0x71, 0x44, 0xC3, 0x7B} }

class HE_NO_VTABLE ISendEmailMessage : public Helium::IHeUnknown
{
public:
	HE_DEFINE_STATIC_IID_ACCESSOR(ISendEmailMessage_IID)

	/// Send the email message.
	/// \param pbstrError Error text in case things go wrong.
	HE_IMETHOD Send(/*out*/ Helium::HEBSTR* pbstrError) = 0;
};
...
public:
	...
	/// Get the email recipients (To:).
	/// \param pbstrRecipients Pointer to the recipients of email notifications.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD GetRecipients(/*out*/ Helium::HEBSTR* pbstrRecipients) = 0;
	/// Set the email recipients (To:).  Multiple recipients may be set by
	/// separating email addresses with spaces, commas, or semicolons.
	/// \param bstrRecipients The recipients of email notifications.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD SetRecipients(/*in*/ Helium::HEBSTR bstrRecipients) = 0;
	...
	/// Get the email scheme.
	/// \param pScheme Pointer to the protocol scheme used to connect to the
	/// email server.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD GetScheme(/*out*/ EmailScheme* pScheme) = 0;
	/// Set the email scheme.
	/// \param scheme The protocol scheme used to connect to the email server.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD SetScheme(/*in*/ EmailScheme scheme) = 0;
	/// Get whether the sender is using authentication.
	/// \param pbUseAuthentication Pointer to whether the sender is using authentication.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD GetUseAuthentication(/*out*/ BOOL* pbUseAuthentication) = 0;
	/// Set whether the sender is using authentication.
	/// \param bUseAuthentication Whether the sender is using authentication.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD SetUseAuthentication(/*in*/ BOOL bUseAuthentication) = 0;
	/// Get the username for the sender.
	/// \param pbstrUsername Pointer to the username for the sender.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD GetUsername(/*out*/ Helium::HEBSTR* pbstrUsername) = 0;
	/// Set the username for the sender.
	/// \param bstrUsername The username for the sender.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD SetUsername(/*in*/ Helium::HEBSTR bstrUsername) = 0;
	/// Get the password for the sender.
	/// \param pbstrPassword Pointer to the password for the sender.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
	HE_IMETHOD GetPassword(/*out*/ Helium::HEBSTR* pbstrPassword) = 0;
	/// Set the password for the sender.
	/// \param bstrPassword The password for the sender.
	/// \retval HE_S_OK Success
	/// \retval HE_E_FAIL Error
	/// \remarks None
...
#define EmailMessage_CID \
{ 0x29CAD4CC, 0x2B38, 0x4A78, {0xBA, 0xAD, 0x3C, 0xCA, 0xF2, 0xF2, 0xCD, 0xFE} }
