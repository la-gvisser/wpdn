	...
	bool&		outHasGraphs )
{
	...
	try {
		...
		if ( HE_SUCCEEDED( hr ) ) {
			m_PeekCapture.SetPtr( spPeekCapture.get() );
		...
		if ( HE_SUCCEEDED( hr ) ) {
			...
			if ( HE_SUCCEEDED( hr ) ) {
				m_GraphsContext.SetPtr( spGraphsContext.get() );
				outHasGraphs = true;
			}
		}
		...
		if ( HE_SUCCEEDED( hr ) ) {
			m_FileContext.SetPtr( spFileContext.get() );
		}
		else {
			spServiceProvider.Release();
			hr = inUnkSite->QueryInterface( &spServiceProvider.p );
			if ( HE_SUCCEEDED( hr ) ) {
				m_FileContext.SetPtrOther( spServiceProvider.get() );
