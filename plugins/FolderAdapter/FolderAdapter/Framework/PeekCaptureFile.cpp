	...
	bool	inScanPackets /*= false*/ )
{
	...
	while ( blk.Read( cfRead ) ) {
		...
		nFileOffset = cfRead.GetPosition();
	}
	...
	if ( inScanPackets && (nPacketCount > 0) ) {
		m_ayOffsets.SetFirstOffset( cfRead.GetSize(), nPacketCount, nOffset );
		SeekToPacket( nPacketCount, nError );
	}
	else {
		m_ayOffsets.SetFirstOffset( cfRead.GetSize(), 1, nOffset );
	}
	SeekToPacket( 0, nError );
	...
	UInt32			inIndex )
{
	...
	UInt32	nRead = cfRead.Read( nAttribSize, ayAttribData );
	...
	UInt32	nDataRead = cfRead.Read( static_cast<UInt32>( bvPacketData.size() ), &bvPacketData[0] );
	...
	if ( m_SessionBlock.GetPacketCount() &&
		...
		m_ayOffsets.SetOffset( (inIndex + 1), cfRead.GetPosition() );
	}
	...
	UInt32				inFirst /*= NEXT_PACKET*/)
{
	...
	if ( !SeekToPacket( inFirst, outError ) ) return false;

	size_t	nPacketCount = m_ayOffsets.size();
	if ( inFirst > nPacketCount ) {
		outError = CF_ERROR_EOF;
		return false;
	}
	...
	UInt32&	outError )
{
	...
	if ( m_ayOffsets.HasIndex( inIndex ) ) {
		return (cfRead.SeekTo( m_ayOffsets[inIndex] ) == m_ayOffsets[inIndex]);
	}
	...
	while ( nIndex <= inIndex ) {
		...
		if ( cfRead.SeekTo( nOffset ) != nOffset ) {
			outError = CF_ERROR_UNEXPECTED_EOF;
			return false;
		}
		...
		UInt32			nRead = cfRead.Read( nCount, ayPacketData );
		if ( nRead < 6 ) {
			outError = CF_ERROR_INCOMPLETE_HEADER;
			return false;
		}
	...
	cfRead.SeekTo( m_ayOffsets[inIndex] );
	return true;
}
