...
inline UInt16 Swap16( UInt16 x ) { return bswap_16( x ); }
inline UInt32 Swap32( UInt32 x ) { return bswap_32( x ); }
inline UInt64 Swap64( UInt64 x ) { return bswap_64( x ); }

inline UInt16 SwapIf16( bool b, UInt16 x ) { return (b) ? bswap_16(x) : (x); }
inline UInt32 SwapIf32( bool b, UInt32 x ) { return (b) ? bswap_32(x) : (x); }
inline UInt64 SwapIf64( bool b, UInt64 x ) { return (b) ? bswap_64(x) : (x); }
...
typedef struct _tFileMode {
	...
	_tFileMode( int inType, bool inScan = false,
	...
	_tFileMode( int inType, int inAccess,
			bool inScan = false, tMediaType inMediaType = kMediaType_Ethernet ) {
		fType = inType;
		fAccess = inAccess;
		fScan = inScan;
		fMediaType = inMediaType;
...
public:
	...
	;		operator FILETIME() const {
		tTimeLong	ltTime( m_ltTime.i / kPeekTime_FileTime );
		return ltTime.ft;
	}
#endif
...
protected:
	void	AddName( std::wstring& inName ) {
...
public:
	...
	// Stores current file position. Restores file position
	// unless Commit is called.
	class COSFileRollback
	{
	protected:
		COSFile&	m_OSFile;
		bool		m_bRollback;
	...
	public:
		;	COSFileRollback( COSFile& inFile )
				:	m_OSFile( inFile )
				,	m_bRollback( true )
				,	m_nPosition( inFile.GetPosition() )
		{}
		virtual ~COSFileRollback()
		{
			if ( m_bRollback ) {
				m_OSFile.SeekTo( m_nPosition );
			}
		}
...
public:
	;		COSFile() {}
	virtual ~COSFile() { Close(); }

	virtual void	Close() {}

//	const std::wstring&	GetFileName() const { return m_strFileName; }	// Return just the file name.
//	std::string			GetFileNameA() const { return WtoA( m_strFileName ); }
	const std::wstring&	GetPathName() const { return m_strPathName; }	// Returns fully qualified file and pathname.
//	std::string			GetPathNameA() const { return WtoA( m_strPathName ); }
	virtual UInt64		GetSize() const = 0;		// Returns size of the file.
	virtual UInt64		GetPosition() const = 0;	// Returns current file position.

	virtual bool	IsOpen() const = 0;

	virtual bool	Open( const wchar_t* inFileName, UInt32 inMode ) = 0;
	virtual bool	Open( const std::wstring& inFileName, UInt32 inMode ) = 0;
	virtual bool	Open( const char* inFileName, UInt32 inMode ) = 0;
	virtual bool	Open( const std::string& inFileName, UInt32 inMode ) = 0;

	virtual UInt64	MoveTo( SInt64 inOffset ) = 0;		// Move to a relative position.

	virtual UInt32	Read( UInt32 inLength, void* outData ) = 0;

	virtual UInt64	SeekTo( UInt64 inPosition ) = 0;	// Move to an absolute position.
...
{
protected:
	CPeekStream			m_streamData;

protected:
	const UInt8*	GetData() const { return m_streamData.GetData(); }
	void			SetName( const std::wstring& inName );
	bool			ZLibRead( void* inFile );
...
public:
	...
	virtual UInt64	GetPosition() const { return m_streamData.Position(); }
	virtual UInt64	GetSize() const { return m_streamData.GetLength(); }

	virtual bool	IsOpen() const { return !m_streamData.IsEmpty(); }

	virtual bool	Open( const wchar_t* inFileName, UInt32 inMode ) {
		return Open( std::wstring( inFileName ), inMode );
	}
	virtual bool	Open( const std::wstring& inFileName, UInt32 inMode );
	virtual bool	Open( const char* inFileName, UInt32 inMode ) {
		return Open( std::string( inFileName ), inMode );
	}
	virtual bool	Open( const std::string& inFileName, UInt32 inMode ) {
		return Open( AtoW( inFileName ), inMode );
	}

	virtual UInt64	MoveTo( SInt64 inOffset ) {
		return m_streamData.Seek( inOffset, STREAM_SEEK_CUR );
	}

	virtual UInt32	Read( UInt32 inLength, void* outData ) {
		return m_streamData.Read32( inLength, outData );
	}

	virtual UInt64	SeekTo( UInt64 inPosition ) {
		return m_streamData.Seek( inPosition, STREAM_SEEK_SET );
	}

	virtual UInt32	Write( UInt32 inLength, const void* inData ) {
		return m_streamData.Write32( inLength, inData );
	}
};
...
public:
	...
	virtual UInt64	GetSize() const;		// Returns size of the file.
	virtual UInt64	GetPosition() const;	// Returns current file position.

	virtual bool	IsOpen() const { return (m_hFile != INVALID_HANDLE_VALUE); }
	...
	virtual UInt64	MoveTo( SInt64 inOffset );		// Move to a relative position.

	virtual UInt32	Read( UInt32 inLength, void* outData );

	virtual UInt64	SeekTo( UInt64 inPosition );	// Move to an absolute position.
...
public:
	;		CStandardFile( FILE* inFile = nullptr  )
		:	COSFile()
		,	m_hFile( inFile )
	{}
	virtual ~CStandardFile() { Close(); }
	...
	virtual bool	IsOpen() const { return (m_hFile != nullptr); }
...
public:
	...
	bool	Load( COSFile& inFile, UInt64 inOffset );
...
// 	CWinFile		m_File;
// #else
// 	CStandardFile	m_File;
// #endif

	std::unique_ptr<COSFile>	m_spFile;

protected:
	int				m_nFileType;
	UInt32			m_nFileFormat;
	int				m_nFileAccess;

public:
	;		CBaseFile( int inType, UInt32 inFormat, int inAccess )
		:	m_nFileType( inType )
		,	m_nFileFormat( inFormat )
		,	m_nFileAccess( inAccess )
	{}
	;		CBaseFile( std::unique_ptr<COSFile> inOSFile,
				int inType, UInt32 inFormat, int inAccess)
		:	m_spFile( std::move( inOSFile ) )
		,	m_nFileType( inType  )
		,	m_nFileFormat( inFormat )
		,	m_nFileAccess( inAccess )
	{}
...
//#ifdef _WIN32
	virtual COSFile&	GetFile() { return *(m_spFile.get()); }
// #else
// 	virtual CStandardFile&	GetFile() { return m_File; }
// #endif
	int					GetFileAccess() const { return m_nFileAccess; }
	UInt32				GetFileFormat() const { return m_nFileFormat; }
//	const std::wstring&	GetFileName() const { return m_File.GetFileName(); }
//	std::string			GetFileNameA() const { return m_File.GetFileNameA(); }
	UInt64				GetFileSize() const { return (IsOpen()) ? m_spFile->GetSize() : 0; }
	int					GetFileType() const { return m_nFileType; }
	virtual tMediaType	GetMediaType() const = 0;
	virtual UInt32		GetPacketCount() const = 0;
//	const std::wstring&	GetPathName() const { return m_File.GetPathName(); }
//	std::string			GetPathNameA() const { return m_File.GetPathNameA(); }

	bool			IsOpen() const { return (m_spFile && m_spFile->IsOpen()); }

	virtual std::unique_ptr<COSFile>	MoveFile() { return std::move( m_spFile ); }
	...
	virtual bool	ReadFilePacket( CFilePacket& outPacket, UInt32& outError, UInt32 inIndex = NEXT_PACKET ) = 0;
	virtual bool	ReadFilePackets( CFilePacketList& outPackets, UInt32& outError, UInt32 inCount, UInt32 inFirst = NEXT_PACKET ) = 0;
...
class CBaseFilePtr
	:	public std::unique_ptr<CBaseFile>
{
public:
	;		CBaseFilePtr( CBaseFile* inBaseFile = nullptr )
		:	std::unique_ptr<CBaseFile>( inBaseFile )
	{}
...
{
protected:
	virtual bool	Parse( bool inScanPackets ) = 0;

public:
	;		CCaptureReader(
		UInt32 inFormat = kFileFormatUnknown,
		int inAccess = CF_ACCESS_UNKNOWN )
		:	CBaseFile( CF_TYPE_READER, inFormat, inAccess )
	{}
	;		CCaptureReader( UInt32 inFormat, std::unique_ptr<COSFile> inOSFile )
		:	CBaseFile( std::move( inOSFile ), CF_TYPE_READER, inFormat, CF_ACCESS_UNKNOWN )
	{}
	...
	virtual bool	ReadFilePacket( CFilePacket& outPacket, UInt32& outError, UInt32 inIndex ) = 0;
	virtual bool	ReadFilePackets( CFilePacketList& outPackets, UInt32& outError, UInt32 inCount, UInt32 inFirst = NEXT_PACKET );
...
public:
	;		CCaptureWriter(
		int inFormat = kFileFormatUnknown,
		int inAccess = CF_ACCESS_UNKNOWN )
	...
	virtual void	Init( const CCaptureFile* inTemplate = nullptr ) = 0;
...
public:
	static UInt32	GetFileFormat( const std::wstring& inFileName );
...
public:
	...
	;	operator tFileMode() const {
		tFileMode	fm( GetFileFormat(), false, GetFileAccess(), GetMediaType() );
		return fm;
	}
...
//	}
	...
	bool				IsEOF() {
		COSFile&	cfRead( GetFile() );	// GetFile checks IsValid.
		return (cfRead.GetPosition() >= cfRead.GetSize());
	}
	...
	virtual bool	ReadFilePacket( CFilePacket& outPacket, UInt32& outError, UInt32 inIndex = NEXT_PACKET  ) {
		outError = CF_ERROR_EOF;
		return (m_spBaseFile.IsValid())
			?	m_spBaseFile->ReadFilePacket( outPacket, outError, inIndex )
			: false;
	}
	virtual bool	ReadFilePackets( CFilePacketList& outPackets, UInt32& outError, UInt32 inCount, UInt32 inFirst = NEXT_PACKET ) {
		outError = CF_ERROR_EOF;
		return (m_spBaseFile.IsValid())
			?	m_spBaseFile->ReadFilePackets( outPackets, outError, inCount, inFirst )
			: false;
	}
