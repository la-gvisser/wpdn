	...
	Helium::HEVARTYPE	inType /*= HE_VT_BSTR*/ )
{
	...
	while ( HE_SUCCEEDED( hr ) && (spChildNode.get() != nullptr) ) {
...
CPeekXml::HasValue() const
{
	...
	hr = m_spElement->get_firstChild( &spChildNode.p );
	if ( HE_FAILED( hr ) || (spChildNode.get() == nullptr) ) {
		return false;
	}
...
{
	_ASSERTE( m_spCreatedDoc.get() == nullptr );
	_ASSERTE( m_spElement.get() == nullptr );
	...
	const HeLib::CHeVariant&	inAttribute )
{
	...
	if ( m_spElement.get() == nullptr ) return;
	_ASSERTE( inName );
