...
CPeekXmlLoad::Attribute(
	const wchar_t*	inName,
	std::wstring&	outAttribute )
{
	HeLib::CHeVariant	varValue;
	if ( !GetAttribute( inName, varValue, Helium::HE_VT_BSTR ) ) return false;
	outAttribute = std::wstring( varValue.bstrVal, HeLib::HeStringLen( varValue.bstrVal ) );
	return true;
}
	...
	std::wstring&	outValue )
{
	HeLib::CHeVariant	varValue;
	if ( !GetValue( varValue, Helium::HE_VT_BSTR ) ) return false;
	outValue = std::wstring( varValue.bstrVal, static_cast<int>(HeLib::HeStringLen( varValue.bstrVal )) );
	return true;
}
...
CPeekXmlStore::Attribute(
	const wchar_t*	inName,
	std::wstring&	inAttribute )
{
	SetAttribute( inName, CHeBSTR( inAttribute.length(), &inAttribute[0] ) );
	return true;
}
