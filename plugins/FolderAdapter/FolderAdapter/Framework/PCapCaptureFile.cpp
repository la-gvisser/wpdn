	...
	bool	inScanPackets /*= false*/ )
{
	...
	cfRead.SeekTo( 0 );
	for ( int i = 0; i < 11; ++i ) {
		if ( cfRead.GetSize() >= sizeof( TcpDumpFileHeader ) ) {
			break;
		}
	...
	UInt32	dwBytesRead = cfRead.Read( sizeof( TcpDumpFileHeader ), &pcapHeader );
	...
	UInt32&				outError )
{
	...
	UInt32					nRead = cfRead.Read( nCount, &pktHeader );
	if ( nRead < nCount ) {
		PlatformSleep( 1 );
	...
	UInt32&	outError )
{
	...
	while ( nIndex <= inIndex ) {
		...
		TPCapPacketHeader	pktHeader;
		if ( !ReadPacketHeader( pktHeader, outError ) ) return false;
