...
public:
	...
	bool	IsNotValid() const { return (m_spPeekCaptures.get() == nullptr); }
	bool	IsValid() const { return (m_spPeekCaptures.get() != nullptr); }
...
public:
	;		CAppConfig( Helium::IHeUnknown* inUnkSite )
	{
		...
		if ( HE_SUCCEEDED( hr ) ) {
			SetPtr( spServiceProvider.get() );
		}
	}
	;		~CAppConfig() {}

	CPeekString	GetConfigurationDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetConfigDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CPeekString	GetLibraryDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetLibDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CPeekString	GetResourceDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetResourceDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CPeekString	GetDocumentDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetDocDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CPeekString	GetLogDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetLogDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CPeekString	GetDataDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetDataDir(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}
	CArrayString	GetDataDirectoryList() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetDataDirList(
			&bstrDirectory.m_str ) );
		CArrayString	ayStrings;
		ayStrings.Parse( L';', bstrDirectory.m_str );
		return ayStrings;
	}
	CPeekString	GetDatabaseDirectory() {
		CHeBSTR	bstrDirectory;
		Peek::ThrowIfFailed( m_spAppConfig->GetDatabasePath(
			&bstrDirectory.m_str ) );
		return CPeekString( bstrDirectory );
	}

	bool	IsNotValid() const { return (m_spAppConfig.get() == nullptr); }
	bool	IsValid() const { return (m_spAppConfig.get() != nullptr ); }
};
...
public:
	...
	bool	IsNotValid() const { return (m_spPeekSetup.get() == nullptr); }
	bool	IsValid() const { return (m_spPeekSetup.get() != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spConsoleUI.get() == nullptr); }
	bool	IsValid() const { return (m_spConsoleUI.get() != nullptr); }
};
...
public:
	...
	bool		IsNotValid() const { return (m_spMediaUtils.get() == nullptr); }
	bool		IsValid() const { return (m_spMediaUtils.get() != nullptr); }

	CPeekString	SpecToString( const TMediaSpec& inSpec ) {
		CHeBSTR	bstrText;
		if ( m_spMediaUtils.get() != nullptr ) {
...
protected:
	CPeekEngineProxy*	m_pProxy;

	void	SetPtr( CPeekEngineProxy* inProxy ) {
		m_pProxy = inProxy;
	}

public:
	;		CLogService() : m_pProxy( nullptr ) {}
	;		~CLogService() {}
	...
	bool	IsNotValid() const { return (m_pProxy == nullptr); }
	bool	IsValid() const { return (m_pProxy != nullptr ); }
};
...
public:
	...
	bool	IsNotValid() const { return (m_spNameTable.get() == nullptr); }
	bool	IsValid() const { return (m_spNameTable.get() != nullptr); }
...
public:
	...
	bool	IsNotValid() const { return (m_spLookupNameEntry.get() == nullptr); }
	bool	IsValid() const { return (m_spLookupNameEntry.get() != nullptr); }
...
public:
	...
	Helium::IHeUnknown* GetSite() { return m_spUnkSite.get(); }
#ifdef IMPLEMENT_NOTIFICATIONS
