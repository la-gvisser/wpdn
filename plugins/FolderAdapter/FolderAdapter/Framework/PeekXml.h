...
public:
	...
	operator CPeekPersistFile() const {
		if ( (m_spCreatedDoc.get() == nullptr) && (m_spElement.get() != nullptr) ) {
			CPeekXmlDocument	spOwner;
			m_spElement->get_ownerDocument( &spOwner.p );
			if ( spOwner.get() != nullptr ) {
	...
	bool	IsNotValid() const { return (m_spElement.get() == nullptr); }
	bool	IsOpen() const { return (m_spElement.get() != nullptr); }
	bool	IsValid() const { return (m_spElement.get() != nullptr); }
