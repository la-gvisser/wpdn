...
public:
	...
	bool	IsOpen() const { return (m_spXml.get() != nullptr) ? m_spXml->IsOpen() : false; }
	bool	IsLoading() const { return (m_spXml.get() != nullptr) ? m_spXml->IsLoading() : false; }
	bool	IsNotValid() const { return !IsOpen(); }
	bool	IsStoring() const { return (m_spXml.get() != nullptr) ? m_spXml->IsStoring() : false; }
...
public:
	...
	UInt32		GetChildCount( const wchar_t* inName = nullptr ) const {
		return ( m_spXml.get() != nullptr ) ? m_spXml->GetChildCount( inName ) : 0;
	}
