...
CPeekStringA	MakeRegExMask( const CPeekStringA& inMask );
bool			IsMatchingFileName( const CPeekString& inFileName, const CPeekString& inMask );
bool			IsMatchingFileName( const CPeekStringA& inFileName, const CPeekStringA& inMask );
bool			IsMatchingFileName( const CPeekString& inFileName, std::regex inRegEx );
bool			IsMatchingFileName( const CPeekStringA& inFileName, std::regex inRegEx );
...
#ifdef _DEBUG
			T _str = m_strPath.substr( end );
			typename T::const_pointer _psz = _str.c_str(); (void)_psz;
#endif
...
public:
	enum {
		kAttribute_None = 0x00000,
		kAttribute_ReadOnly = 0x00001,
		kAttribute_Hidden = 0x00002,
		kAttribute_System = 0x00004,
		kAttribute_Directory = 0x00010,
		kAttribute_Archive = 0x00020
	};

protected:
	static const CPeekString	s_strTagName;
	static const CPeekString	s_strTagSize;
	static const CPeekString	s_strTagAttributes;
	static const CPeekString	s_strTagModification;

protected:
	CPeekString	m_strName;
	UInt64		m_nSize;
	UInt32		m_nAttributes;
	CPeekTime	m_Time;			// Last Modification Time

public:
	;		CFileDetail() : m_nSize( 0 ), m_nAttributes( 0 ) {}
	;		CFileDetail( const CPeekString& inName, UInt64 inSize,
				UInt32 inAttributes, CPeekTime inTime )
		:	m_strName( inName )
		,	m_nSize( inSize )
		,	m_nAttributes( inAttributes )
		,	m_Time( inTime )
	{}

	bool	operator==( const CFileDetail& inOther ) {
		return (inOther.m_strName == m_strName);
	}

	CPeekString			FormatAttributes() const {
		CPeekString	str;
		if ( IsArchive() ) str.Append( L"A" );
		if ( IsReadOnly() ) str.Append( L"R" );
		if ( IsHidden() ) str.Append( L"H" );
		if ( IsSystem() ) str.Append( L"S" );
		return str;
	}
	CPeekString			FormatSize() const {
		CPeekOutString	ostr;
		ostr << m_nSize;
		return ostr;
	}

	UInt32				GetAttributes() const { return m_nAttributes; }
	const CPeekString&	GetName() const { return m_strName; }
	UInt64				GetSize() const { return m_nSize; }
	CPeekTime			GetTime() const { return m_Time; }

	bool	IsCurrentDirectory() const {
		if ( !IsDirectory() ) return false;
		return (m_strName.compare( L"." ) == 0);
	}
	bool	IsDotDirectory() const {
		if ( !IsDirectory() ) return false;
		if ( m_strName.compare( L"." ) == 0 ) return true;
		return (m_strName.compare( L".." ) == 0);
	}
	bool	IsArchive() const { return ((m_nAttributes & kAttribute_Archive) != 0); }
	bool	IsDirectory() const { return ((m_nAttributes & kAttribute_Directory) != 0); }
	bool	IsEmpty() const { return m_strName.IsEmpty(); }
	bool	IsFile() const { return ((m_nAttributes & kAttribute_Directory) == 0); }
	bool	IsHidden() const { return ((m_nAttributes & kAttribute_Hidden) != 0); }
	bool	IsParentDirectory() const {
		if ( !IsDirectory() ) return false;
		return (m_strName.compare( L".." ) == 0);
	}
	bool	IsReadOnly() const { return ((m_nAttributes & kAttribute_ReadOnly) != 0); }
	bool	IsSystem() const { return ((m_nAttributes & kAttribute_System) != 0); }

	void	Set( const CPeekString& inName, UInt64 inSize, UInt32 inAttributes, CPeekTime inTime ) {
#ifdef _DEBUG
		const wchar_t*	szName( inName.c_str() ); (void) szName;
#endif
		m_strName = inName;
		m_nSize = inSize;
		m_nAttributes = inAttributes;
...
typedef CPeekArray< CFileDetail >	CFileDetailList;

bool	GetFileDetail( const wchar_t* inFileName, CFileDetail& outDetail );
bool	GetFileDetail( const char* inFileName, struct stat64* pStat, CFileDetail& outDetail );
#ifdef _WIN32
bool	GetFileDetail( const WIN32_FIND_DATAW& inFindData, CFileDetail& outDetail );
#endif
bool	GetFileDetailList( const std::wstring& inPath, const std::wstring& inMask, CFileDetailList& outFileList );
...
public:
	...
	static bool			DeleteFile( const CPeekString& inFileName );
	...
	static bool			MoveFile( const CPeekString& inSource, const CPeekString& inDestination );
