...
//	PeekPacket.h
// =============================================================================
//	Copyright (c) 1998-2015 Savvius, Inc. All rights reserved.
...
enum PeekPacketFlags {
	...
	kPacketFlag_Radiotap			= 0x400,	///< Radiotap header follows packet data.
	// Note: flags > 16 bits not saved in pkt files.
	kPacketFlag_FilterSliceMask		= 0x3FFF0000,	///< Slice length to be applied.
	kPacketFlag_FilterReject		= 0x40000000	///< Packet did not pass filters.
};
...
enum PeekPacketStatus {
	...
	kPacketStatus_Encrypted			= 0x04,			///< Encrypted packet (e.g., WEP/WPA/WPA2).
...
struct PeekPacket13
{
	...
	UInt16		fSliceLength;			// Sliced length of packet.
	UInt8*		fMediaSpecInfoBlock;	// MediaSpecificPrivateHeader + (wireless or full duplex or wan or ... )
	UInt32		fAppID;					// PACE2 Application ID (top bit: 0=application, 1=protocol)
	UInt32		fFlowID;				// Flow ID
} WP_PACK_STRUCT;
typedef struct PeekPacket13 PeekPacket13;

// Define the current versions of things.
typedef struct PeekPacket13 PeekPacket;
