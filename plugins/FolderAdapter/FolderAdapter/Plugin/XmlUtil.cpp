	...
	size_t&				indentOrigLen )
{
	...
	try {
		...
		if ( hr == HE_S_OK ) {
			...
			while ( (hr == HE_S_OK) && (spChildNode.get() != nullptr) ) {
				HeThrowIfFailed( spChildNode->get_nodeType( &typeChildNode ) );
				...
				if ( (typeChildNode == Xml::NODE_ELEMENT) && !bLastWasTextNode ) {
					...
					HeThrowIfFailed( pNode->insertBefore( spNewNode, CHeVariant( spChildNode.get() ), &spN.p ) );
				}
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_BSTR, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_I1, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_UI1, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_I2, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_UI2, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_I4, varVal ) ) return false;
	lVal = varVal.lVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_UI4, varVal ) ) return false;
	ulVal = varVal.ulVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_I8, varVal ) ) return false;
	llVal = varVal.llVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_UI8, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_INT, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_UINT, varVal ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetAttribute( pElem, pszName, HE_VT_R8, varVal ) ) return false;
	...
	CHeVariant&			varVal )
{
	...
	if ( !GetChildElement( pNode, pszName, &spElem.p ) ) return false;
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_BSTR, varVal ) ) return false;
	bstrVal.Attach( varVal.bstrVal );
	varVal.vt = HE_VT_EMPTY;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_I1, varVal ) ) return false;
	cVal = varVal.cVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_UI1, varVal ) ) return false;
	bVal = varVal.bVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_I2, varVal ) ) return false;
	iVal = varVal.iVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_UI2, varVal ) ) return false;
	uiVal = varVal.uiVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_I4, varVal ) ) return false;
	lVal = varVal.lVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_UI4, varVal ) ) return false;
	ulVal = varVal.ulVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_I8, varVal ) ) return false;
	llVal = varVal.llVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_UI8, varVal ) ) return false;
	ullVal = varVal.ullVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_INT, varVal ) ) return false;
	intVal = varVal.intVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_UINT, varVal ) ) return false;
	uintVal = varVal.uintVal;
	return true;
}
...
{
	CHeVariant	varVal;
	if ( !GetChildElement( pNode, pszName, HE_VT_R8, varVal ) ) return false;
	dblVal = varVal.dblVal;
	return true;
}
	...
	Xml::IXMLDOMElement**	ppNewElem )
{
	...
	try {
...

		{
			...
			if ( HE_SUCCEEDED(hr) && (spAttribMap.get() != nullptr) ) {
				SInt32	nAttribCount;
				...
				for ( SInt32 i = 0; i < nAttribCount; i++ )
				{
					...
					if ( HE_FAILED(hr) || (spAttrib.get() == nullptr) ) continue;
