	...
	bool	inProcInstr /*= false*/ )
{
	...
	if ( inProcInstr ) {
		...
		Peek::ThrowIfFailed( m_spCreatedDoc->createProcessingInstruction( L"xml",
			L"version=\"1.0\" encoding=\"utf-8\"", &spPI.p ) );
		Peek::ThrowIfFailed( m_spCreatedDoc->appendChild( spPI, NULL ) );
	}
	...
	Peek::ThrowIfFailed( m_spCreatedDoc->appendChild( m_spElement, NULL ) );
}
	...
	PCWSTR	inName )
{
	...
	// Append the child element.
	Peek::ThrowIfFailed( m_spElement->appendChild( spElement, NULL ) );
	...
	Helium::HEVARTYPE	inType /*= HE_VT_BSTR*/ )
{
	...
	while ( HE_SUCCEEDED( hr ) && (spChildNode != NULL) ) {
...
CPeekXml::HasValue() const
{
	...
	hr = m_spElement->get_firstChild( &spChildNode.p );
	if ( HE_FAILED( hr ) || (spChildNode == NULL ) ) {
		return false;
	}
...
{
	_ASSERTE( m_spCreatedDoc == NULL );
	_ASSERTE( m_spElement == NULL );
	...
	const UInt8*	inData )
{
	...
	psXml.Rewind();		// Might need for Omni 6.8, will need for Omni 7.
	...
	const Helium::HEBSTR	inValue )
{
	...
	Peek::ThrowIfFailed( m_spElement->appendChild( spTextNode, NULL ) );
}
