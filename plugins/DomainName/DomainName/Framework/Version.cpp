	...
	const CPeekString&	inVersion )
{
	...
	if ( (4 != swscanf_s( inVersion, L"%u.%u.%u.%u", &nMajorVersion, &nMinorVersion, &nMajorRevision, &nMinorRevision )) &&
		 (3 != swscanf_s( inVersion, L"%u.%u.%u", &nMajorVersion, &nMinorVersion, &nMajorRevision )) &&
		 (2 != swscanf_s( inVersion, L"%u.%u", &nMajorVersion, &nMinorVersion )) &&
	...
	CVersion&	outProduct )
{
	...
	DWORD	nIgnored;
	DWORD	nVersionSize = ::GetFileVersionInfoSize( strFilePath, &nIgnored );

	if ( nVersionSize > 0 ) {
		// Alloc the version info.
		void*	pVersionInfo = malloc( nVersionSize );
		_ASSERTE( pVersionInfo != NULL );
