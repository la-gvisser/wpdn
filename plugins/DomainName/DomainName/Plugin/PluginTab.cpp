...
CPluginTab::OnInitDialog()
{
	...
	if ( m_pView != NULL ) {
		CWnd*	pOptions = GetDlgItem( IDC_CONTEXT_OPTIONS_PL );
		if ( pOptions ) {
...
CPluginTab::OnSize(
	...
	int		cy )
{
	const LONG kAdjust( 15 );

	CDialog::OnSize( nType, cx, cy );

	if ( !m_pContextList ) return;

	CRect	rcContextList;
	m_pContextList->GetWindowRect( &rcContextList );
	ScreenToClient( &rcContextList );
	rcContextList.bottom = cy - rcContextList.top + kAdjust;
	m_pContextList->MoveWindow( &rcContextList );

	CWnd*	pContextOptions = GetDlgItem( IDC_CONTEXT_OPTIONS_PL );
	if ( !pContextOptions ) return;

	CRect	rcOptions;
	pContextOptions->GetWindowRect( &rcOptions );
	ScreenToClient( &rcOptions );
	rcOptions.right = cx - rcOptions.top + kAdjust;
	rcOptions.bottom = cy - rcOptions.top + kAdjust;
	pContextOptions->MoveWindow( &rcOptions );
...
CPluginTab::OnLbnSelchangeContextList()
{
	if ( m_pContextList && m_pView ) {
		...
		m_pView->Refresh( NamedOpt.fOptions );
	}
}
