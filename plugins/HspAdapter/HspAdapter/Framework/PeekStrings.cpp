...
{
	va_list	pArgs;
	va_start( pArgs, inFormat );

	FormatList( inFormat, pArgs );
...
#pragma warning(disable:4996)	// Warning: This function or variable may be unsafe. Use '_s' instead.
    size_t size = _vsnwprintf( nullptr, 0, inFormat, pArgs ) + 1; // Extra space for '\0'
    std::unique_ptr<wchar_t[]> buf( new wchar_t[ size ] ); 
    _vsnwprintf_s( buf.get(), size, size, inFormat, pArgs );
#pragma warning(pop)
#else
    size_t size = std::vswprintf( nullptr, 0, inFormat, pArgs ) + 1; // Extra space for '\0'
    std::unique_ptr<wchar_t[]> buf( new wchar_t[ size ] ); 
    std::vswprintf( buf.get(), size, inFormat, pArgs );
#endif
	this->assign( buf.get(), buf.get() + size - 1 ); // Don't want the trailing '\0'.
}
...
#pragma warning(push)
#pragma warning(disable:4996)	// Warning: This function or variable may be unsafe. Use '_s' instead.
    size_t size = _vsnprintf( nullptr, 0, inFormat, pArgs ) + 1; // Extra space for '\0'
    std::unique_ptr<char[]> buf( new char[ size ] ); 
    _vsnprintf_s( buf.get(), size, size, inFormat, pArgs );
#pragma warning(pop)
#else
    size_t size = std::vsnprintf( nullptr, 0, inFormat, pArgs ) + 1; // Extra space for '\0'
    std::unique_ptr<char[]> buf( new char[ size ] ); 
    std::vsnprintf( buf.get(), size, inFormat, pArgs );
#endif
	this->assign( buf.get(), buf.get() + size - 1 ); // Don't want the trailing '\0'.
}
