...
public:
	...
	bool		IsNotValid() const { return (m_spItem == nullptr); }
	bool		IsValid() const { return (m_spItem != nullptr); }
};
...
public:
	...
	CPeekString	GetItemLabel( const GUID& inId ) const {
		CHeBSTR	bstrLabel;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spSnapshot->GetItemLabel( reinterpret_cast<const HeID&>( inId ),
				&bstrLabel.m_str ) );
		}
		return CPeekString( bstrLabel );
	}
	CGlobalId	GetItemParent( const GUID& inId ) const {
 	...
 	Peek::SummaryStat	GetItemValue( const GUID& inId ) const {
 		...
 		if ( IsValid() ) {
 			Peek::ThrowIfFailed( m_spSnapshot->GetItemValue( reinterpret_cast<const HeID&>( inId ),
	...
	bool		IsNotValid() const { return (m_spSnapshot == nullptr); }
	bool		IsValid() const { return (m_spSnapshot != nullptr); }
