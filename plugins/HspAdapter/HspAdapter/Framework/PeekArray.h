...
public:
	...
	size_t		Add() { this->push_back( T() ); return (this->size() - 1); }
	size_t		Add( const T& inItem ) {
		this->push_back( inItem );
		return (this->size() - 1);
	}
	size_t		AddUnique( const T& inItem ) {
		typename std::vector<T>::iterator	itr =
				std::find( this->begin(), this->end(), inItem );
		if ( itr != this->end() ) {
	...
	size_t		Find( const T& inItem ) { return this->find( inItem ); }

	const T&	GetAt( size_t inIndex ) const { return this->at( inIndex ); }
	T&			GetAt( size_t inIndex ) { return this->at( inIndex ); }
	size_t		GetCount() const { return this->size(); }

	void		InsertAt( size_t inIndex, const T& inItem ) {
		insert( (this->begin() + inIndex), inItem );
	}
	...
	void		RemoveAt( size_t inIndex ) { this->erase( this->begin() + inIndex ); }
	void		Reserve( size_t inCount ) { this->reserve( inCount ); }

	void		SetCount( size_t inCount ) { this->resize( inCount ); }
};
...
private:
	...
	CPeekArrayDep( const CPeekArrayDep& );
	CPeekArrayDep& operator=( const CPeekArrayDep& );
};


template< typename E >
inline size_t CPeekArrayDep< E >::GetCount() const
{
	return( m_nSize );
}
...
template< typename E >
inline bool CPeekArrayDep< E >::IsEmpty() const
{
	return( m_nSize == 0 );
}

template< typename E >
inline void CPeekArrayDep< E >::RemoveAll()
{
	SetCount( 0, -1 );
}
...
template< typename E >
inline const E* CPeekArrayDep< E >::GetData() const
{
	return( m_pData );
}

template< typename E >
inline E* CPeekArrayDep< E >::GetData()
{
	return( m_pData );
}
...
template< typename E >
void CPeekArrayDep< E >::FreeExtra()
{
	ATLASSERT_VALID(this);
...
template< typename E >
void CPeekArrayDep< E >::CallDestructors( E* pElements, size_t nElements )
{
	(void)pElements;
