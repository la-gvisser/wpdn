...
inline void
Sleep(
	unsigned long nMilliseconds ) throw()
{
#ifdef _WIN32
	::Sleep( nMilliseconds );
#else
...
public:
	...
	Mutex()
	{
		const int	res = pthread_mutex_init( &m_Mutex, nullptr );
		if ( res != 0 ) {
...
public:
	Condition()
	{
		const int	res = pthread_cond_init( &m_Cond, nullptr );
		if ( res != 0 ) {
...
class Event;
typedef std::vector<Event*>	EventPtrList;

class Event
{
public:
	typedef struct WaitResult_t {
		UInt32	m_nResult;
		UInt32	m_nIndex;

		WaitResult_t( UInt32 inResult, UInt32 inIndex = 0 )
			: m_nResult( inResult )
			, m_nIndex( inIndex )
		{};
	} WaitResult;

	static WaitResult WaitMultiple( EventPtrList inEvents, bool inWaitAll, unsigned long inTimeout ) {
#if defined(_WIN32)
		if ( inTimeout == 0 ) {
			inTimeout = INFINITE;
		}

		std::vector<HANDLE>	ayHandles;
		for each ( auto itr in inEvents ) {
			if ( itr->IsValid() ) {
				ayHandles.push_back( itr->m_pImpl->m_Handle );
			}
		}

		_ASSERTE( ayHandles.size() < MAXIMUM_WAIT_OBJECTS );

		DWORD result = ::WaitForMultipleObjects( static_cast<UInt32>( ayHandles.size() ),
			&ayHandles[0], inWaitAll, inTimeout );
		if ( (result & 0xFFFFFF80) == WAIT_OBJECT_0 ) {
			return WaitResult( kWaitResult_Success, (result & 0x000007E) );
		}
		if ( (result & 0xFFFFFF80) == WAIT_ABANDONED ) {
			return WaitResult( kWaitResult_Abandoned, (result & 0x000007E) );
		}
		if ( result == WAIT_TIMEOUT ) {
			return WaitResult( kWaitResult_Timeout );
		}
		return WaitResult( static_cast<UInt32>( kWaitResult_Error ) );
#else
		// HSP_TODO: wait on multiple...
		auto itrFirst = inEvents.begin();
		auto itrSecond = ++itrFirst;
		int result = (*itrSecond)->Wait( inTimeout );
...
private:
	...
	{
	public:
		Impl( const wchar_t* inName )
		{
			m_Handle = ::CreateEventW( nullptr, FALSE, FALSE, inName );
			if ( m_Handle == nullptr ) {
				throw std::runtime_error( "Event handle initialization failed" );
			}
		}

		Impl( bool, const wchar_t* inName )
		{
			m_Handle = ::OpenEventW( FALSE, FALSE, inName );
#ifdef _DEBUG
		...
		~Impl() { ::CloseHandle( m_Handle ); }

		bool Reset() { return (::ResetEvent(m_Handle) != FALSE); }

		bool Set() { return (::SetEvent(m_Handle) != FALSE); }

		int Wait() {
			DWORD result = ::WaitForSingleObject( m_Handle, INFINITE );
			switch ( result ) {
				case WAIT_OBJECT_0: {
					return kWaitResult_Success;
				}

				case WAIT_TIMEOUT: {
					return kWaitResult_Timeout;
				}
	...
	public:
		...
		Impl( bool, const wchar_t* /* inName */ )
		{
			// HSP_TODO: open an existing named event.
			throw std::runtime_error( "Open Event is not implemented" );
		}
		...
		int Wait( unsigned long msTimeout ) {
			int result = TimedWait( msTimeout );
			int status = kWaitResult_Error;
			switch ( result ) {
				case 0:
					status = kWaitResult_Success;
...
public:
	...
	bool Create( const wchar_t* inName ) {
#ifdef ASSERT
		ASSERT( !IsValid() );
#endif
	...
	bool Open( const wchar_t* inName ) {
		if ( IsValid() ) return false;

		try {
			m_pImpl = new Impl( true, inName );
		}
		catch ( ... ) {
			return false;
		}
	...
	int Wait( unsigned long nMilliseconds ) {
#ifdef ASSERT
		ASSERT( IsValid() );
#endif
		if ( !IsValid() ) return kWaitResult_Error;
...
public:
	...
	operator bool() const throw() { return IsAlive(); }

	bool IsAlive() const throw() { return (m_hThread != 0); }
	int GetExitCode() const { return m_ExitCode; }
	unsigned int GetThreadID() const { return m_ThreadID; }

	bool Create( IThreadFunction* pFunc, size_t cbStack = 0, const char* pszThreadName = nullptr ) throw() {
		m_pFunc       = pFunc;
		m_ThreadID    = 0;
		...
		m_hThread = (HANDLE) _beginthreadex( nullptr, 0, ThreadFunc_, this, 0, &m_ThreadID );
		if ( m_hThread == nullptr ) return false;
		...
		// Create the thread.
		const int	res = pthread_create( &m_hThread, pAttr, (void *(*)(void*)) ThreadFunc_, this );
	...
	int GetCore() throw() { return m_nCore; }
	void SetCore( int nCore ) throw() { m_nCore = nCore; }
	...
	NativeHandle GetNativeHandle() throw() { return m_hThread; }
...
public:
	BlockingQueue()
	{
		...
		res = pthread_cond_init( &m_Cond, nullptr );
		if ( res != 0 ) {
			pthread_mutex_destroy( &m_Mutex );
			throw std::runtime_error( "pthread_cond_init failed" );
		}
	...
	bool TryDequeue( T& t, unsigned long timeout = 0 ) {
		...
		while ( m_Items.empty() ) {
			...
			if ( ::WaitForSingleObject( m_Event, timeout ) != WAIT_OBJECT_0) return false;
			::EnterCriticalSection( &m_Mutex );
			timeout = 0;
		}
