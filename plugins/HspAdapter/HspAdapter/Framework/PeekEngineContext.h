...
public:
	...
	;	CPeekEngineContext( const CGlobalId& inId, CRemoteStatsPlugin* inRemotePlugin )
		:	CPeekContext( inId, inRemotePlugin )
	{}
	...
	//
	// Functions Invoked From the Engine
	//
#ifdef IMPLEMENT_CAPTURE_EVENTS
	virtual int		OnCaptureStarting() = 0;
	virtual int		OnCaptureStarted() = 0;
	virtual int		OnCaptureStopping() = 0;
	virtual int		OnCaptureStopped() = 0;
#endif	// IMPLEMENT_CAPTURE_EVENTS
...
//	virtual int		OnResetGraphsStats() = 0;
	...
	virtual	int		OnSummary( IHeUnknown* outSnapshot ) = 0;
};
