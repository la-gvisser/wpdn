...
typedef union _tEthernetAddress {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		return CALLTYPEHASH( inAlg, union _tEthernetAddress, bytes[0], size_t );
	}
} tEthernetAddress;
...
typedef union _tIpV6Address {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		return CALLTYPEHASH( inAlg, union _tIpV6Address, bytes[0], size_t );
	}
} tIpV6Address;
...
typedef struct _tEthernetHeader {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		// return CALLTYPEHASH( inAlg, struct _tEthernetHeader, Destination, size_t );
...
typedef struct _t80211Header {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		// return CALLTYPEHASH( inAlg, struct _t80211Header, VersionTypeSubType, size_t );
		return
...
typedef struct _tIpV4Header {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		// return CALLTYPEHASH( inAlg, struct _tIpV4Header, VersionLength, size_t );
		const size_t	nSize = sizeof( struct _tIpV4Header ) - (sizeof( tIpV4Address ) * 2);
		return
			(CALLTYPESIZEHASH( inAlg, struct _tIpV4Header, nSize, VersionLength, size_t )) ^
			Source.Hash( inAlg ) ^
			Destination.Hash( inAlg );
	}
} tIpV4Header;
...
typedef struct _tIpV6Header {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		//return CALLTYPEHASH( inAlg, struct _tIpV6Header, VersionClassLabel, size_t );
		const size_t	nSize = sizeof( struct _tIpV6Header ) - (sizeof( tIpV6Address ) * 2);
		return
			(CALLTYPESIZEHASH( inAlg, struct _tIpV6Header, nSize, VersionClassLabel, size_t )) ^
			Source.Hash( inAlg ) ^
			Destination.Hash( inAlg );
	}
} tIpV6Header;
...
typedef struct _tTcpHeader {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		// return CALLTYPEHASH( inAlg, struct _tTcpHeader, SourcePort, size_t );
		const size_t	nSize = sizeof( struct _tTcpHeader ) - (sizeof( UInt16 ) * 2);
		return
...
typedef struct _tUdpHeader {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		// return CALLTYPEHASH( inAlg, struct _tUdpHeader, SourcePort, size_t );
		const size_t	nSize = sizeof( struct _tUdpHeader ) - (sizeof( UInt16 ) * 2);
		return
...
typedef struct _tDnsHeader {
	...
	size_t	Hash( int inAlg = kPeekHash_FNV ) const {
		const size_t	nSize = sizeof( struct _tDnsHeader );
		return CALLTYPESIZEHASH( inAlg, struct _tDnsHeader, nSize, id, size_t );
	}
} tDnsHeader;
