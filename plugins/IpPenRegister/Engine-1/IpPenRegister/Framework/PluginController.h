...
protected:
	size_t	InternalAdd( IConsoleSend* inConsoleSend, COmniPluginPtr& outPlugin ) {
		...
		if ( outPlugin.IsValid() ) {
			outPlugin->SetConsoleSend( inConsoleSend );
		}
		return nIndex;
	}
	...
	bool	InternalFind( IConsoleSend* inConsoleSend, size_t& outIndex ) const {
		...
		for ( size_t i = 0; i < nCount; i++ ) {
			...
			if ( spPlugin.IsValid() ) {
				if ( spPlugin->GetConsoleSend() == inConsoleSend ) {
					outIndex = i;
					return true;
				}
			}
...
public:
	...
	void	Remove( IConsoleSend* inConsoleSend, CRemotePlugin* inRemotePlugin ) {
		size_t	nIndex( kIndex_Invalid );
		if ( InternalFind( inConsoleSend, nIndex ) ) {
			m_ayPlugins[nIndex].fOwners.Remove( inRemotePlugin );
			if ( m_ayPlugins[nIndex].fOwners.empty() ) {
