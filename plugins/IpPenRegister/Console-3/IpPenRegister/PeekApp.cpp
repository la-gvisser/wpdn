	...
	ClientAppDataPtr	ClientAppData )
{
	...
	try {
		...
		s_procAddTabWithGroup			= (AddTabWithGroupProc)			GetProcAddress2( theModule, kExportName_AddTabWithGroup );
		s_procSendPluginMessage			= (SendPluginMessageProc)		GetProcAddress2( theModule, kExportName_SendPluginMessage );

		// New to API 12
		s_procSaveContext				= (SaveContextProc)				GetProcAddress2( theModule, kExportName_SaveContext );
	}
	catch( ... ) {
...
{
	// get the IP header and data pointer and length
	UInt32	ayIPs[] = { ProtoSpecDefs::IPv6 };
	UInt32	nTemp;
	int		nResult;
	...
	*outIPHdrData = DoPacketGetHeaderLayer( *outDeliveryProtocol, inMediaType, inMediaSubType,
	...
	// get the IP header length
	UInt16	nIPHdrLength( sizeof( tIPv6Header ) );
	UInt16	nIPTotalLength( NETWORKTOHOST16( *(reinterpret_cast<const UInt16*>( (*outIPHdrData) + 4 ) ) ) );
	...
	UInt32					inFlags )
{
	...
	if ( (inFlags & kSelectPackets_ActionMask) == 0 ) {
		inFlags |= kSelectPacketsAction_SelectOnly;
	}
...
{
	PLUGIN_ASSERT( inString != NULL );
	if ( inString == NULL ) return PLUGIN_RESULT_ERROR;
	PLUGIN_ASSERT( inSummary != NULL );
	if ( inSummary == NULL ) return PLUGIN_RESULT_ERROR;
