...
{
	{
		...
		_ASSERTE( m_AddNotifyArrayPtr != NULL );

		ResetAddArray();
	}

	_ASSERTE( m_SendNotifyArrayPtr != NULL );
...
//	PeekLock lock( m_Mutex );
	...
	try {
		...
		_ASSERTE( m_hThreadStartEvent == NULL );
