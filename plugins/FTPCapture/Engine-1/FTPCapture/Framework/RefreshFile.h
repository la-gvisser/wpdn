...
protected:
	...
	void			ResetTimeInterval() {
		...
		if ( m_nRefreshTimeIntervalMS != kRefreshTimeNotUsed ) {
			m_nTimeIntervalBeginMS = CPeekTime::Now().i;
		}
	}
...
public:
	...
	void			Deactivate();
	static bool		DirectoryExists( const CPeekString& inDirectory ) throw( ... );

	static bool		IsPathValid( const CPeekString& inPath ) throw( ... );

	static bool		MakePath( const CPeekString& inPath ) throw( ... );

//	static UInt64	Now() { return ::GetTickCount(); }
	...
	bool		IsTimeExpired() {
		if ( (m_nRefreshTimeIntervalMS != kRefreshTimeNotUsed) &&
			 ...
			 (CPeekTime::Now().i > (m_nTimeIntervalBeginMS + m_nRefreshTimeIntervalMS)) ) {
			return true;
		}
		return false;
	}
	...
	UInt64		GetTimeRemainingUntilRefresh() const {
		return (CPeekTime::Now().i - (m_nTimeIntervalBeginMS + m_nRefreshTimeIntervalMS));
	}
	UInt64		GetTimeSinceActivated() const { return (CPeekTime::Now().i - m_nTimeActivatedMS); }
