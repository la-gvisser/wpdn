...
public:
	...
	void	AddMessage( const CPeekString& inMessage, UInt8 inSeverity = peekSeverityInformational ) {
		if ( inMessage.GetLength() ) {
			...
			strMessage << COmniPlugin::GetName() << L" - " << inMessage;
			Add( strMessage, inSeverity );
		}
	}

	void	AddTimedOutMessages( const CArrayString& inRemovedList, UInt8 inSeverity = peekSeverityInformational );
