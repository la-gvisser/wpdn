...
CPluginStatistics::CPluginStatistics()
{
	...
	m_strLabels[kStat_TimedOutControl]		= _T("Timed-out Ftp Sessions");
	m_strLabels[kStat_TimedOutData]			= _T("Timed-out Data Connections");
}
...
CPluginStatistics::Init(
	CFtpProcess* inFtpProcess )
{
	m_Stats.Add( new CPluginStatFunc( inFtpProcess->m_FtpSessions ) );		// kStat_ActiveUsers,
	m_Stats.Add( new CPluginStatFunc( inFtpProcess->m_DataChannels ) );		// kStat_ActiveDataChannels,
	m_Stats.Add( new CPluginStatValue() );									// kStat_Logons,
	m_Stats.Add( new CPluginStatValue() );									// kStat_Logoffs,
	m_Stats.Add( new CPluginStatFunc( inFtpProcess->m_FtpSessions, 1 ) );	// kStat_Received,
	m_Stats.Add( new CPluginStatFunc( inFtpProcess->m_FtpSessions, 2 ) );	// kStat_Stored,
