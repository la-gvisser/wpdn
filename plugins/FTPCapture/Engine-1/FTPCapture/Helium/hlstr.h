...
//	}
	...
	HeResult WriteToStream( Helium::IHeStream* pStream ) const throw()
	{
		...
		if ( HE_SUCCEEDED(hr) && (cbStrLen > 0) )
		{
			...
			hr = pStream->Write( (UInt16*) m_str, cbStrLen, &cb );
#else
	...
	HeResult ReadFromStream( Helium::IHeStream* pStream ) throw()
	{
		...
		// Save the stream pos.
		UInt64		nStreamPos;
		HeResult	hrSeek = pStream->Seek( 0, Helium::IHeStream::STREAM_SEEK_CUR, &nStreamPos );
		...
		// Reset the stream position to start of HEBSTR size on failure.
		if ( HE_FAILED(hr) && HE_SUCCEEDED(hrSeek) )
		{
			(void) pStream->Seek( nStreamPos, Helium::IHeStream::STREAM_SEEK_SET, NULL );
		}
