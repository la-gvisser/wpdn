...
public:
	;	CPeekEngineContext( const CGlobalId& inId )
		:	CPeekContext( inId )
	{
	}
	...
	//petertest - I added this one:
	void		DoNotify( int inSeverity, const CPeekString& inNotifyMessage ) {
		GetNotifyService().DoNotify( m_Id,  m_Id, 0, 0, inSeverity, inNotifyMessage, inNotifyMessage );
	}
	...
	void		DoInsertPacket( CPacket& inPacket ) {
		m_PeekContextProxy.GetDynamicPacketBuffer().PutPacket( inPacket );
	}
...
//	virtual int		OnResetGraphsStats() = 0;
	...
	virtual void	SetInterfacePointers( IHeUnknown* inSite, CRemotePlugin* inPlugin ) {
		...
		if ( inSite ) {
			m_EngineProxy.SetInterfacePointers( inSite, inPlugin );
		}
	}
