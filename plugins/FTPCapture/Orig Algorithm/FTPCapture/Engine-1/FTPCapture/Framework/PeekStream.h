...
public:
	...
	;			CPeekStream( Helium::IHeStream*	inStream ) { m_spStream = inStream; }
	virtual		~CPeekStream() {}
	...
	size_t		Append( CPeekStream& otherStream ) {
		...
		if ( nCount != nWritten ) Peek::Throw( E_FAIL );
		return nWritten;
	}

	bool		CopyTo( CPeekStream& inDest, size_t inCount, size_t& outRead, size_t& outWrite ) {
	...
	bool		CopyTo( CPeekStream& inDest, UInt32 inCount ) {
		size_t	nRead = 0;
		size_t	nWrite = 0;
	...
	bool		IsNotValid() const { return (m_spStream == NULL); }
	bool		IsValid() const { return (m_spStream != NULL); }
