	...
	const CPeekString&	inGroup ) const
{
	...
	while ( pHashItem != NULL ) {
		// Get the group.
		ASSERT( pHashItem->fIndex < m_Groups.GetCount() );
...
CPeekSummaryStats::IndexToGroup(
	...
	CGlobalId&		outId ) const
{
	ASSERT( inIndex < m_Groups.GetCount() );
	...
	size_t				inGroupId ) const
{
	...
	while ( pHashItem != NULL ) {
		ASSERT( pHashItem->fIndex < m_Labels.GetCount() );
		// Compare the label.
	...
	const void*			inData )
{
	...
	ASSERT( nLabelIndex != kIndex_Invalid );
