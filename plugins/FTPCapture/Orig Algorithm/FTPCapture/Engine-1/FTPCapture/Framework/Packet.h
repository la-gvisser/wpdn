...
public:
	...
	void		GetDataLayer( UInt16 intProtoSpecId, UInt32* outSourceProtoSpec, const UInt8** outData, UInt16* outBytesRemaining );
	void		GetHeaderLayer( UInt16 inProtoSpecId, UInt32* outSourceProtoSpec, const UInt8** outData, UInt16* outBytesRemaining );
...
protected:
	...
	static TMediaSubType	InternalGetMediaSubType( IPacket* inIPacket ) {
		TMediaSubType	tMediaSubType = kMediaSubType_Native;
		if ( inIPacket ) {
			Peek::ThrowIfFailed( inIPacket->GetMediaSubType( &tMediaSubType ) );
		}
		return tMediaSubType;
	}
	static TMediaType		InternalGetMediaType( IPacket* inIPacket ) {
		TMediaType	tMediaType = kMediaType_802_3;
		if ( inIPacket )  {
			Peek::ThrowIfFailed( inIPacket->GetMediaType( &tMediaType ) );
		}
		return tMediaType;
	}
...
protected:
	...
	void			SetPtr( void* inPtr ) { Reset(); m_spPacket = reinterpret_cast<IPacket*>( inPtr );}
...
public:
	...
	TMediaSubType	GetMediaSubType() const { return InternalGetMediaSubType( m_spPacket ); }
