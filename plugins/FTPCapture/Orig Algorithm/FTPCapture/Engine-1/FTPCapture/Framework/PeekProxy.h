...
protected:
	CHePtr<IConsoleSend>	m_spConsoleSend;

protected:
	IConsoleSend*	GetPtr() { return m_spConsoleSend; }
	void			ReleasePtr() { m_spConsoleSend.Release(); }
	void			SetPtr( IConsoleSend* inConsoleUI ) {
		m_spConsoleSend = inConsoleUI;
	}
	void			SetPtr( IHeUnknown* inUnknown ) {
		SetPtr( reinterpret_cast<IConsoleSend*>( inUnknown ) );
	}
...
public:
	...
	bool	IsValid() const { return (m_spConsoleSend != NULL); }
	bool	IsNotValid() const { return (m_spConsoleSend == NULL); }
		...
		UInt8* inMessage, UInt32 inTimeout )
	{
		...
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spConsoleSend->Send( inPluginId, inReceive,
		...
		UInt32 inTimeout )
	{
		UInt32	nTransactionId = 0;
		if ( IsValid() ) {
			Peek::ThrowIfFailed( m_spConsoleSend->SendPluginMessage( inPluginId,
