	...
	const CPeekString&	inAddress )
{
	...
	for ( i = 0; i < sizeof( tEthernetAddress ); i++ ) {
		...
		m_EthernetAddress.bytes[i] = static_cast<UInt8>( _tstoi( strToken ) );
	}
...
//petertest - add this to the wizard class
// -----------------------------------------------------------------------------
//		Format
// -----------------------------------------------------------------------------
...
CIpAddressPortPair::Set(
	const CIpHeader&	inIpHeader,
	const tTcpHeader*	inTcpHeader /* = NULL */ )
{
	ASSERT( inIpHeader.IsValid() );
