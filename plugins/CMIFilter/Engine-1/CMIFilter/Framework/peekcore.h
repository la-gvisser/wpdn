...
//	peekcore.h
// =============================================================================
//	Copyright (c) 2001-2014 WildPackets, Inc. All rights reserved.
...
public:
	...
	HE_IMETHOD GetHandlerIDs(/*out*/ Helium::HeID** ppHandlerIDs, /*out*/ UInt32* pIDCount) = 0;
...
/// \ingroup Plugins
/// \brief Plugin UI Handler types: About or Options.
...
/// \struct CapturePacketCounters
/// \brief Holds the various packet counters tracked by a capture.
/// \see IPeekCapture3
#include "hepushpack.h"
struct CapturePacketCounters
{
	UInt64		Received;	///< Number of packets received by the capture
	UInt64		Filtered;	///< Number of packets filtered by the capture
	UInt64		Dropped;	///< Number of packets dropped by the capture during the capture
} HE_PACK_STRUCT;
#include "hepoppack.h"

/// \internal
/// \interface IPeekCapture3
/// \ingroup Capture
/// \brief Additional methods for PeekCapture.
/// \see IPeekCapture, coclass PeekCapture
#define IPeekCapture3_IID \
{ 0x7D90E8DA, 0xBD3A, 0x433A, {0x8F, 0x74, 0x6D, 0x8C, 0x28, 0x96, 0x10, 0xA0} }

class HE_NO_VTABLE IPeekCapture3 : public Helium::IHeUnknown
{
public:
	HE_DEFINE_STATIC_IID_ACCESSOR(IPeekCapture3_IID);

	/// Get the date-time data started in nanoseconds since midnight 1/1/1601 UTC.
	HE_IMETHOD GetDataStartTime(/*out*/ UInt64* pnDataStartTime) = 0;
	/// Get remote session ID.
	HE_IMETHOD GetRemoteSessionID(/*out*/ UInt64* pnRemoteSessionID) = 0;
	/// Get the capture packet counters.
	HE_IMETHOD GetPacketCounters(/*out*/ CapturePacketCounters * pPacketCounters) = 0;
};
...
public:
	...
	HE_IMETHOD SetCaptureID(SInt32 nCaptureID) = 0;
	HE_IMETHOD SetCaptureSessionID(/*in*/ SInt32 nCaptureSessionID) = 0;
	HE_IMETHOD SetTimeFilter(/*in*/ UInt64 nStartTime, /*in*/ UInt64 nEndTime) = 0;
	HE_IMETHOD SetFilterMode(/*in*/ PeekFilterMode fm) = 0;
	HE_IMETHOD SetPacketFilter(/*in*/ IFilterPacket* pPacketFilter) = 0;
	HE_IMETHOD Load(/*in*/ BOOL bPacketsOnlyLoad) = 0;
	HE_IMETHOD Stop() = 0;
	HE_IMETHOD IsLoading(/*out*/ BOOL* pbLoading) = 0;
};

/// \internal
/// \enum RemoteAdapterType
/// \ingroup Capture
/// \brief AdapterType: file, pcap, pfring or napatech.
enum RemoteAdapterType
{
	remoteAdapterTypeUnknown	= 0x0000,	///< Unknown.
	remoteAdapterTypeFile		= 0x0001,	///< File adapter.
	remoteAdapterTypePcap		= 0x0002,	///< Pcap adapter.
	remoteAdapterTypeNapatech	= 0x0003,	///< Napatech adapter.
	remoteAdapterTypePfring		= 0x0004,	///< Pfring adapter.
	remoteAdapterTypePktgen		= 0x0005,	///< Packet generator adapter.
	remoteAdapterTypeSearch		= 0x0006,	///< Forensics search adapter.
	remoteAdapterTypeSNF		= 0x0007	///< Myricom SNF adapter.
};
...
public:
	...
	/// Get remote adapter type.
	HE_IMETHOD GetRemoteAdapterType(/*out*/ RemoteAdapterType* pAdapterType) = 0;
...
enum RemoteCaptureFlags
{
	...
	remoteCaptureVoipStats	= 0x0008,		///< Analyze voip statistics (media streams' jitter, latency & loss)
	remoteCaptureTopStats	= 0x0010		///< Analyze top usage statistics (top nodes, top protocols)
};
...
#define PEEK_E_INVALID_ADAPTER					((HeResult)	0x80040203L)	///< Invalid capture adapter specified.
	...
	/// Set adapter name string by its index.
	HE_IMETHOD SetAdapterName(/*in*/ UInt32 nAdapterIndex,
		/*in*/ Helium::HEBSTR bstrName) = 0;
	/// Get adapter type by its index.
	HE_IMETHOD GetAdapterType(/*in*/ UInt32 nAdapterIndex,
		/*in*/ RemoteAdapterType* pAdapterType) = 0;
	...
	/// (this is a hack for now. we sould be setting HW filters on an adapter not a capture)
	HE_IMETHOD SetHardwareFilter(/*in*/ UInt64 nCaptureID,
		/*in*/ UInt64 nSessionID,
		/*in*/ Helium::HEBSTR bstrFilter) = 0;
	...
	HE_IMETHOD StopCapture(/*in*/ UInt64 nCaptureID,
		/*in*/ UInt64 nSessionID) = 0;
	/// Flush capture data onto the disk.
	HE_IMETHOD FlushCapture(/*in*/ UInt64 nCaptureID,
		/*in*/ UInt64 nSessionID) = 0;
	/// Reserve storage units for capture.
	HE_IMETHOD ReserveCaptureUnits(/*in*/ UInt64 nCaptureID,
		/*in*/ UInt64 nVolumeID,
		/*in*/ UInt64 nReservedUnits) = 0;
	/// Release storage units from capture.
	HE_IMETHOD ReleaseCaptureUnits(/*in*/ UInt64 nCaptureID,
		/*in*/ UInt64 nVolumeID,
		/*in*/ UInt64 nReservedUnits) = 0;
