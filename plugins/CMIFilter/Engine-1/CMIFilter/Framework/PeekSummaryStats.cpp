	...
	const CPeekString&	inGroup )
{
	...
	if ( !inGroup.IsEmpty() ) {
		...
		return nLabelIndex;	// nullptr group name not allowed
	}
...
CPeekSummaryStats::InitData()
{
	...
	if ( m_spCurrentSnapshot == nullptr ) throw -1;
	...
	const void*			inData )
{
	...
	if ( m_spCurrentSnapshot != nullptr ) {
