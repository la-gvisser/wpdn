...
#include <wppoppack.h>
	...
	// Seconds between 1/1/1601 and 1/1/1970 (see KB167296:
	// http://support.microsoft.com/default.aspx?scid=KB;en-us;q167296).
	const UInt64 TIMECONV_FILE_TO_ANSI = 11644473600ull;
	const UInt64 TIMECONV_PEEK_TO_ANSI = TIMECONV_FILE_TO_ANSI;
	
	// 100 nanosecond units between 1/1/1601 and 1/1/1904.
	const UInt64 TIMECONV_PEEK_TO_MICROPEEK = 95616288000000000ull;
...
{
#ifdef _WIN32
	struct tm*	pAnsiTm = nullptr;
	errno_t err = localtime_s( pAnsiTm, &nAnsiTime );
	if ( err || pAnsiTm == nullptr) return 0;
#else
	struct tm*	pAnsiTm = localtime( &nAnsiTime );
	if ( pAnsiTm == nullptr ) return 0;
#endif // _WIN32
	...
	return (static_cast<UInt32>( theTime.WordPart & 0x0000FFFF ) |
			static_cast<UInt32>( (theDate.WordPart & 0x0000FFFF) << 16 ));
}
	...
	UInt16	inTime )
{
	...
	theAnsiTm.tm_wday = 0;
	theAnsiTm.tm_yday = 0;
	// -1 is "use system to compute if DST applies", 0 is "don't use DST"
	theAnsiTm.tm_isdst = -1;
