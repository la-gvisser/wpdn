...
// Version 8 with wireless, only used internally (never written to a file).
struct PeekPacket8Wireless
{
	UInt32		fProtoSpec;		// ProtoSpec ID.
	UInt16		fPacketLength;	// Total length of packet.
	UInt16		fSliceLength;	// Sliced length of packet.
	...
	UInt8		fStatus;		// Slicing, ...
	UInt64		fTimeStamp;		// Timestamp in nanoseconds UTC.
	Wireless80211PrivateHeader	fWirelessHeader;
};
typedef struct PeekPacket8Wireless PeekPacket8Wireless;
